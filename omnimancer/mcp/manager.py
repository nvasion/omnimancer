...
from ..core.models import (
    MCPConfig, MCPServerConfig, ToolDefinition, ToolCall, ToolResult,
    MCPError, MCPServerError
)
from ..utils.errors import (
    MCPConnectionError, MCPTimeoutError, MCPConfigurationError
)
...
class MCPManager:
    ...
    async def initialize_servers(self, new_server_config: Optional[Dict[str, MCPServerConfig]] = None) -> None:
        """
        ...
        disabled_servers = {name: config for name, config in self.mcp_config.servers.items() if not config.enabled}
        if disabled_servers:
        ...
        async def connect_server(server_name: str, server_config: MCPServerConfig) -> None:
            ...
            async with semaphore:
                try:
                    ...
                    logger.error(f"Configuration error for MCP server '{server_name}': {error_msg}")
                    for suggestion in suggestions:
                        logger.info(f"  Suggestion: {suggestion}")
                    failed_connections.append((server_name, "configuration_error", str(e)))
                    
                except MCPConnectionError as e:
                    error_msg, suggestions, should_retry = handle_mcp_error(e, server_name)
                    logger.warning(f"Connection failed for MCP server '{server_name}': {error_msg}")
                    
                    # Attempt one retry for connection errors
                    if should_retry:
                        try:
                            logger.info(f"Retrying connection to MCP server: {server_name}")
                            await asyncio.sleep(2.0)  # Brief delay before retry
                            client = MCPClient(server_config)
                            await client.connect()
                            self.clients[server_name] = client
                            successful_connections += 1
                            logger.info(f"Successfully connected to MCP server on retry: {server_name}")
                        except Exception as retry_error:
                            logger.error(f"Retry failed for MCP server '{server_name}': {retry_error}")
                            failed_connections.append((server_name, "connection_failed", str(e)))
                    else:
                        failed_connections.append((server_name, "connection_failed", str(e)))
                        
                except MCPTimeoutError as e:
                    error_msg, suggestions, _ = handle_mcp_error(e, server_name)
                    logger.warning(f"Timeout connecting to MCP server '{server_name}': {error_msg}")
                    failed_connections.append((server_name, "timeout", str(e)))
                    
                except Exception as e:
                    error_msg, suggestions, _ = handle_mcp_error(e, server_name)
                    logger.error(f"Unexpected error initializing MCP server '{server_name}': {error_msg}")
        ...
        logger.info(f"MCP initialization complete. Connected to {successful_connections}/{total_servers} servers.")
        
        if failed_connections:
            logger.warning(f"Failed to connect to {len(failed_connections)} MCP servers:")
        ...
        if successful_connections == 0 and total_servers > 0:
            logger.warning("No MCP servers connected. Tool functionality will be unavailable.")
        elif successful_connections < total_servers:
            logger.info(f"Partial MCP connectivity: {successful_connections} servers available for tool execution.")
        ...
        for client in self.clients.values():
            if client.is_connected:
                try:
                    ...
                    logger.warning(f"Failed to get tools from server '{client.server_name}': {e}")
    ...
    async def execute_tool(self, name: str, arguments: Dict) -> ToolResult:
        ...
        if not target_client:
            ...
            for client in self.clients.values():
                if client.is_connected:
                    available_tools.extend([tool.name for tool in client.tools.values()])
            ...
            if available_tools:
                ...
                raise MCPError(f"Tool '{name}' not found. No MCP servers are connected.")
        ...
        try:
            ...
            error_msg, suggestions, should_retry = handle_mcp_error(e, target_client.server_name)
            ...
            if not target_client.is_connected:
                logger.info(f"Server {target_client.server_name} disconnected, looking for alternatives...")
                
                # Try to find another server with the same tool
                alternative_client = None
                for client in self.clients.values():
                    if (client != target_client and 
                        client.is_connected and 
                        client.get_tool_definition(name)):
                        alternative_client = client
                        break
                
                if alternative_client:
                    logger.info(f"Found alternative server: {alternative_client.server_name}")
                    try:
                        return await alternative_client.call_tool_with_retry(name, arguments)
    ...
    def total_tool_count(self) -> int:
        ...
        return sum(len(client.tools) for client in self.clients.values() if client.is_connected)
    ...
    async def recover_failed_servers(self) -> Dict[str, bool]:
        ...
        for server_name, server_config in self.mcp_config.servers.items():
            ...
            needs_recovery = (
                client is None or 
                not client.is_connected or 
                not await client.health_check()
            )
    ...
    async def monitor_and_recover(self, check_interval: float = 30.0) -> None:
        ...
        """
        logger.info(f"Starting MCP server monitoring (check interval: {check_interval}s)")
        ...
        while self.initialized:
            try:
                ...
                if unhealthy_servers:
                    logger.warning(f"Unhealthy MCP servers detected: {unhealthy_servers}")
        ...
        try:
            ...
            for fallback_tool in fallback_tools:
                try:
                    ...
                    logger.warning(f"Fallback tool '{fallback_tool}' failed: {fallback_error}")
                    continue
