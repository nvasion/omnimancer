...
from ..core.models import (
    MCPServerConfig, ToolDefinition, ToolCall, ToolResult,
    MCPError, MCPServerError, MCPToolError
)
...
class MCPClient:
    ...
    async def connect(self, retry_count: int = 0, max_retries: int = 3) -> None:
        ...
        try:
            logger.info(f"Connecting to MCP server: {self.server_config.name} (attempt {retry_count + 1})")
            ...
            if self.process.poll() is not None:
                ...
                elif "module not found" in stderr_output.lower() or "importerror" in stderr_output.lower():
                    raise MCPConfigurationError(
            ...
            try:
                ...
                if retry_count < max_retries:
                    logger.warning(f"Connection timeout for {self.server_config.name}, retrying...")
            ...
            try:
                ...
                logger.warning(f"Tool discovery failed for {self.server_config.name}: {e}")
    ...
    async def list_tools(self) -> List[ToolDefinition]:
        ...
        if not self.connected:
            raise MCPServerError(f"Not connected to MCP server: {self.server_config.name}")
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> ToolResult:
        ...
        if name not in self.tools:
            raise MCPToolError(f"Tool '{name}' not found on server '{self.server_config.name}'")
    ...
    async def _initialize_connection(self) -> None:
        ...
        if protocol_version and protocol_version not in ["1.0", "2024-11-05"]:
            raise MCPConnectionError(f"Unsupported protocol version: {protocol_version}")
    ...
    async def _discover_tools(self) -> None:
        ...
        for tool_data in tools_data:
            tool_def = ToolDefinition(
                ...
                server_name=self.server_config.name,
                auto_approved=tool_data.get("name", "") in self.server_config.auto_approve
            )
            self.tools[tool_def.name] = tool_def
        
        logger.info(f"Discovered {len(self.tools)} tools from server '{self.server_config.name}'")
    ...
    async def _send_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time() - start_time < self.server_config.timeout:
                if self.process.stdout.readable():
                    ...
                    if line.strip():
                        ...
                        if "error" in response:
                            error_info = response["error"]
                            raise MCPServerError(error_info.get("message", "Unknown error"))
            ...
            raise MCPServerError(f"Request timeout after {self.server_config.timeout} seconds")
    ...
    async def reconnect(self) -> bool:
        ...
        """
        try:
            logger.info(f"Attempting to reconnect to MCP server: {self.server_config.name}")
        ...
        if 'required' in tool_def.parameters:
            ...
            for param in required_params:
                if param not in tool_call.arguments:
                    raise MCPToolError(f"Missing required parameter '{param}' for tool '{tool_call.name}'")
            ...
            for param_name, param_value in tool_call.arguments.items():
                if param_name in properties:
                    ...
                    if expected_type == 'number' and not isinstance(param_value, (int, float)):
                        raise MCPToolError(f"Invalid parameter type for '{param_name}': expected number, got {type(param_value).__name__}")
                    elif expected_type == 'string' and not isinstance(param_value, str):
                        raise MCPToolError(f"Invalid parameter type for '{param_name}': expected string, got {type(param_value).__name__}")
    ...
    async def call_tool_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if any(indicator in error_msg for indicator in [
                    "not found", "invalid parameter", "permission denied"
                ]):
                    raise
        ...
        if last_error:
            ...
            raise MCPToolError(f"Tool '{name}' execution failed after {max_retries} retries")