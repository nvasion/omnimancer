...
class BatchApprovalPanel:
    ...
    def render_batch_overview(self, batch_request: BatchApprovalRequest) -> Panel:
        ...
        overview_table.add_row("Created:", batch_request.created_at.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        if batch_request.expires_at:
            ...
            if time_remaining.total_seconds() > 0:
                overview_table.add_row("Expires:", f"{time_remaining.seconds // 60} minutes")
            else:
        ...
        if risk_counts:
            ...
            for risk_level, count in risk_counts.items():
                ...
                overview_table.add_row(f"  {risk_level.title()}:", f"[{color}]{count}[/{color}]")
    ...
    def render_operations_table(self, 
                              batch_request: BatchApprovalRequest,
                              page: int = 0,
                              show_approved: bool = True) -> Panel:
        """
        ...
        for i in range(start_idx, end_idx):
            ...
            # Determine status
            original_idx = i if show_approved else self._get_original_index(batch_request, i)
            ...
            # Risk assessment
            risk_display = self._format_risk_assessment(preview) if preview else "Unknown"
        ...
        page_info = f"Page {page + 1} of {(total_ops - 1) // self.config.items_per_page + 1}" if total_ops > self.config.items_per_page else ""
    ...
    def render_action_details(self, 
                            operation: Operation, 
                            preview: Optional[ChangePreview] = None) -> Panel:
        """
        ...
        info_table.add_row("Requires Approval:", "Yes" if operation.requires_approval else "No")
        ...
        details_group.append(Panel(info_table, title="Operation Details", border_style="blue"))
        ...
        if preview:
            ...
            preview_table.add_row("Change Type:", preview.change_type.value.replace('_', ' ').title())
            preview_table.add_row("Risk Assessment:", self._format_risk_assessment(preview))
            ...
            details_group.append(Panel(preview_table, title="Change Preview", border_style="yellow"))
    ...
    def render_batch_summary(self, batch_request: BatchApprovalRequest) -> Panel:
        ...
        summary_parts.append(Panel(type_table, title="Operation Types", border_style="cyan"))
        ...
        risk_panel = Panel(risk_table, title=f"Risk Assessment - Overall: {overall_risk}", border_style="red")
    ...
    def _calculate_risk_distribution(self, previews: List[ChangePreview]) -> Dict[str, int]:
    ...
    def _format_target(self, operation: Operation) -> str:
        ...
        return operation.description[:25] + "..." if len(operation.description or "") > 25 else (operation.description or "No target")
    ...
    def _get_original_index(self, batch_request: BatchApprovalRequest, filtered_index: int) -> int:
...
def create_batch_approval_panel(console: Optional[Console] = None,
                              config: Optional[BatchDisplayConfig] = None) -> BatchApprovalPanel:
    """
