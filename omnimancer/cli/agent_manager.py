...
from ..core.agent.agent_config import (
    CustomAgentConfig, AgentRepository, CustomAgentStatus,
    AgentTool, ModelSettings
)
...
class AgentManager:
    ...
    async def list_agents(self, 
                         ...
                         sort_by: str = "name") -> List[CustomAgentConfig]:
        """
        ...
        try:
            with Progress(
                ...
                if category:
                    try:
                        ...
                        filtered_agents = [a for a in filtered_agents if a.category == cat_filter]
                    except ValueError:
            ...
            if not filtered_agents:
                self.console.print("[yellow]No agents found matching the criteria.[/yellow]")
                return []
    ...
    async def search_agents(self, query: str, 
                          fields: Optional[List[str]] = None) -> List[CustomAgentConfig]:
        """
        ...
        try:
            ...
            if not results:
                self.console.print(f"[yellow]No agents found matching '{query}'.[/yellow]")
                return []
    ...
    async def load_agent(self, identifier: str, 
                        activate: bool = True) -> Optional[CustomAgentConfig]:
        """
        ...
        try:
            ...
            if agent.status not in [CustomAgentStatus.ACTIVE, CustomAgentStatus.DRAFT]:
                self.console.print(f"[yellow]Warning: Agent status is '{agent.status.value}'.[/yellow]")
            ...
            self.console.print(f"[green]✅ Agent '{agent.name}' loaded successfully![/green]")
            
            if activate:
                self.console.print(f"[cyan]🚀 Agent '{agent.name}' is now active for this session.[/cyan]")
    ...
    async def delete_agent(self, identifier: str, 
        ...
        try:
            ...
            # Confirm deletion
            if confirm:
                if not Confirm.ask(f"\\n[red]Are you sure you want to delete '{agent.name}'?[/red]", default=False):
                    self.console.print("[yellow]Deletion cancelled.[/yellow]")
                    return False
            
            # Check if agent is currently active
            if self._active_agent and self._active_agent.id == agent.id:
                self.console.print("[yellow]Warning: Deleting currently active agent.[/yellow]")
            ...
            if success:
                self.console.print(f"[green]✅ Agent '{agent.name}' deleted successfully.[/green]")
            else:
                self.console.print(f"[red]❌ Failed to delete agent '{agent.name}'.[/red]")
    ...
    async def create_agent(self, interactive: bool = True) -> Optional[CustomAgentConfig]:
        """
        ...
        try:
            if interactive:
                ...
                self.console.print("[yellow]Non-interactive creation not yet implemented.[/yellow]")
                return None
    ...
    async def clone_agent(self, identifier: str, new_name: str) -> Optional[CustomAgentConfig]:
        """
        ...
        try:
            ...
            if self.repository.get_by_name(new_name):
                self.console.print(f"[red]Agent named '{new_name}' already exists.[/red]")
                return None
            ...
            self.console.print(f"[green]✅ Agent '{new_name}' cloned from '{source_agent.name}'![/green]")
    ...
    async def export_agent(self, identifier: str, 
        ...
        try:
            ...
            if success:
                self.console.print(f"[green]✅ Agent '{agent.name}' exported to {export_path}[/green]")
            else:
                self.console.print(f"[red]❌ Failed to export agent '{agent.name}'.[/red]")
    ...
    async def import_agent(self, import_path: Path, 
                          new_name: Optional[str] = None) -> Optional[CustomAgentConfig]:
        """
        ...
        try:
            ...
            if not imported_agent:
                self.console.print(f"[red]Failed to import agent from {import_path}[/red]")
                return None
            ...
            self.console.print(f"[green]✅ Agent '{imported_agent.name}' imported successfully![/green]")
    ...
    def show_statistics(self):
        ...
        try:
            ...
            stats_table.add_row("Storage Location", str(repo_stats['storage_directory']))
            
            # Session stats
            stats_table.add_row("", "")  # Separator
            stats_table.add_row("Session Started", self._session_stats['session_start'].strftime('%Y-%m-%d %H:%M'))
            stats_table.add_row("Agents Loaded", str(self._session_stats['agents_loaded']))
            stats_table.add_row("Agents Activated", str(self._session_stats['agents_activated']))
            
            if self._active_agent:
                stats_table.add_row("Active Agent", self._active_agent.name)
            
            self.console.print(Panel(stats_table, title="📊 Agent Statistics", box=ROUNDED))
    ...
    def _show_summary_agent_list(self, agents: List[CustomAgentConfig]):
        ...
        for agent in agents:
            ...
            status_display = f"{status_emoji.get(agent.status, '')} {agent.status.value}"
    ...
    def _show_agent_details(self, agent: CustomAgentConfig):
        ...
        info_table.add_row("Last Modified", agent.updated_at.strftime('%Y-%m-%d %H:%M:%S'))
        info_table.add_row("Usage Count", str(agent.metadata.usage_count))
        
        if agent.metadata.last_used:
            info_table.add_row("Last Used", agent.metadata.last_used.strftime('%Y-%m-%d %H:%M:%S'))
        ...
        tools_text = "\\n".join([f"• {tool.value.replace('_', ' ').title()}" for tool in agent.enabled_tools])
        caps_text = "\\n".join([f"• {cap.value.replace('_', ' ').title()}" for cap in agent.capabilities])
        
        tools_panel = Panel(tools_text or "[dim]None[/dim]", title="🔧 Enabled Tools", box=ROUNDED)
        caps_panel = Panel(caps_text or "[dim]None[/dim]", title="⭐ Capabilities", box=ROUNDED)
        ...
        context_table.add_row("Context Window", f"{agent.context_parameters.context_window_size:,} tokens")
        context_table.add_row("Memory Enabled", "✅" if agent.context_parameters.conversation_memory else "❌")
        
        if agent.context_parameters.conversation_memory:
            context_table.add_row("Memory Limit", f"{agent.context_parameters.memory_limit} messages")
        
        context_table.add_row("Response Format", agent.context_parameters.response_format)
        context_table.add_row("Safety Level", agent.behavior_rules.safety_level.title())
        context_table.add_row("Reasoning Style", agent.behavior_rules.reasoning_style.replace('_', ' ').title())
        context_table.add_row("Creativity Level", agent.behavior_rules.creativity_level.title())
        
        self.console.print(Panel(context_table, title="💭 Context & Behavior", box=ROUNDED))
        ...
        if agent.context_parameters.system_prompt:
            ...
            self.console.print(Panel(prompt_preview, title="📝 System Prompt (Preview)", box=ROUNDED))
    ...
    def _sort_agents(self, agents: List[CustomAgentConfig], sort_by: str) -> List[CustomAgentConfig]:
...
async def list_available_agents(console: Optional[Console] = None) -> List[CustomAgentConfig]:
    """Quick function to list all available agents."""
    manager = create_agent_manager(console)
    return await manager.list_agents()


async def load_agent_by_name(name: str, console: Optional[Console] = None) -> Optional[CustomAgentConfig]:
