...
from ..core.config_provider import ConfigurationProvider, ConfigurationContext, ConfigurationMode
...
The entire process takes just **5-10 minutes** and you can change everything later.
    ...
    def _handle_existing_configuration(self) -> bool:
        ...
        self.console.print("You already have Omnimancer configured. What would you like to do?")
    ...
    def _add_providers_to_existing(self) -> bool:
        ...
        available_providers = [p for p in all_providers if p.name not in current_providers]
        
        if not available_providers:
            self.console.print("[yellow]All supported providers are already configured.[/yellow]")
            return True
        
        self.console.print("Available providers to add:")
        for i, provider in enumerate(available_providers, 1):
            self.console.print(f"  {i}. {provider.display_name} - {provider.description}")
        
        selections = Prompt.ask("Which providers would you like to add? (comma-separated numbers)").split(",")
    ...
    def _migrate_existing_configuration(self) -> bool:
        ...
        with self.console.status("[bold green]Analyzing your current configuration..."):
            analysis = self.config_provider.migration_helper.analyze_current_configuration()
        
        # Show analysis results
        self._show_migration_analysis(analysis)
        
        if not analysis.recommended_template:
            self.console.print("[yellow]No suitable template found for migration.[/yellow]")
            return self._run_fresh_setup()
        
        if not Confirm.ask(f"Migrate to '{analysis.recommended_template}' template?", default=True):
            return self._run_fresh_setup()
        
        # Create and execute migration plan
        with self.console.status("[bold green]Creating migration plan..."):
            migration_plan = self.config_provider.migration_helper.create_migration_plan(
        ...
        if Confirm.ask("Execute this migration plan?", default=True):
            return self._execute_migration(migration_plan, analysis.recommended_template)
    ...
    def _execute_migration(self, migration_plan, template_name: str) -> bool:
        ...
        with Progress(
            ...
            task = progress.add_task("Migrating configuration...", total=len(migration_plan.steps))
            
            success, messages = self.config_provider.migration_helper.execute_migration(migration_plan, template_name)
            
            for i, step in enumerate(migration_plan.steps):
                progress.update(task, advance=1, description=f"Step {i+1}: {step['title']}")
    ...
    def _start_fresh_configuration(self) -> bool:
        ...
        if Confirm.ask("This will replace your current configuration. Continue?", default=False):
            # Backup current config
            backup_path = self.config_manager.backup_config()
            self.console.print(f"[green]✓[/green] Current configuration backed up to: {backup_path}")
    ...
    def _handle_welcome_step(self, step: Dict[str, Any]) -> bool:
        ...
        self.console.print(f"\n{content.get('message', '')}")
        self.console.print(f"Estimated time: {content.get('estimated_time', 'Unknown')}")
    ...
    def _handle_single_choice_step(self, step: Dict[str, Any]) -> bool:
        ...
        default_idx = next(
            (i for i, choice in enumerate(choices) if choice.get("value") == option.get("default_value")),
            0
        )
    ...
    def _handle_provider_selection_step(self, step: Dict[str, Any]) -> bool:
        ...
        for i, provider_info in enumerate(providers):
            ...
            table.add_row(
                ...
                provider["description"][:60] + "..." if len(provider["description"]) > 60 else provider["description"],
                setup_difficulty,
                recommended
            )
    ...
    def _handle_api_keys_step(self, step: Dict[str, Any]) -> bool:
        ...
        enabled_providers = [
            key.replace("enable_", "") for key, value in self.user_answers.items() 
            if key.startswith("enable_") and value
        ]
        
        providers_needing_keys = [
            p for p in enabled_providers 
            if p not in ["claude-code", "ollama"]
        ]
        
        if not providers_needing_keys:
            self.console.print("[green]✓[/green] No API keys needed for your selected providers!")
            return True
    ...
    def _get_api_key_for_provider(self, provider_name: str) -> None:
        ...
        instructions = self.config_provider.get_provider_setup_instructions(provider_name)
        ...
        if api_key and api_key.lower() != "skip":
            ...
            self.console.print(f"[yellow]⚠[/yellow] {provider_name} will be disabled until API key is added")
    ...
    def _handle_review_step(self, step: Dict[str, Any]) -> bool:
        ...
        if not preview.get("setup_complete", False):
            ...
            if not Confirm.ask("Continue with incomplete configuration?", default=False):
                return False
    ...
    def _show_configuration_preview(self, preview: Dict[str, Any]) -> None:
        ...
        table.add_row("Enabled Providers", ", ".join(preview.get("enabled_providers", [])))
        table.add_row("Estimated Cost", preview.get("estimated_monthly_cost", "Unknown"))
        ...
        status_text = "Complete" if preview.get("setup_complete", False) else "Incomplete"
    ...
    def _finalize_configuration(self) -> bool:
        ...
        with Progress(
            ...
            success, config_dict, messages = self.config_provider.generate_simple_configuration(self.user_answers)
            ...
            if success:
                ...
                is_valid, errors = self.config_provider.validate_simple_answers(self.user_answers)
...
def run_config_setup_wizard(config_manager: Optional[ConfigManager] = None) -> bool:
    """
...
def run_quick_setup(use_case: str = "general", providers: List[str] = None) -> bool:
    ...
    recommendation = simple_interface.config_mode_manager.get_recommended_configuration(context)
    
    console.print(f"Using template: {recommendation.template_name}")
    
    # Apply template
    success, messages = simple_interface.config_mode_manager.apply_template_configuration(
