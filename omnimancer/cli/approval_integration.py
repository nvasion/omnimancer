...
from ..core.agent.approval_manager import EnhancedApprovalManager, ChangePreview, BatchApprovalRequest
...
class CLIApprovalIntegration:
    ...
    async def request_batch_approval(self, operations: List[Operation]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            batch_request = await self.approval_manager.request_batch_approval(operations)
    ...
    async def _handle_batch_approval(self, batch_request: BatchApprovalRequest) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if batch_decision.decision_type == "approve_all":
                ...
                return {
                    'approved_indices': batch_decision.approved_indices
                }
    ...
    async def _check_auto_approval(self, operation: Operation) -> Optional[bool]:
        ...
        try:
            ...
            has_permission = await self.permission_controller.check_operation_permission(
            ...
            if has_permission:
                ...
                self.console.print(f"[green]✅ Auto-approved: {operation.description} (remembered decision)[/green]")
                return True
    ...
    async def _store_approval_pattern(self, operation: Operation, approval_request: ApprovalRequest):
        """
        ...
        try:
            ...
            self.console.print(f"[blue]🧠 Stored approval pattern for similar '{operation.type.value}' operations[/blue]")
            
        except Exception as e:
            logger.error(f"Error storing approval pattern: {e}")
            self.console.print(f"[yellow]⚠️  Failed to store approval pattern: {e}[/yellow]")
    ...
    def _generate_operation_signature(self, operation: Operation) -> str:
        ...
        if operation.type in [OperationType.FILE_READ, OperationType.FILE_WRITE, OperationType.FILE_DELETE]:
    ...
    def _log_approval_decision(self, operation: Operation, approved: bool):
        ...
        log_entry = {
            'timestamp': operation.created_at.isoformat() if operation.created_at else None,
            'operation_type': operation.type.value,
    ...
    def _log_batch_approval_decision(self, operations: List[Operation], batch_request: BatchApprovalRequest):
    ...
    def _record_session_decision(self, operation: Operation, decision: ApprovalDecision):
    ...
    def get_session_statistics(self) -> Dict[str, Any]:
        ...
        total = len([entry for entry in self.approval_session_log 
                    if 'decision_type' in entry])
        approved = len([entry for entry in self.approval_session_log 
                       if entry.get('approved', False)])
        remembered = len([entry for entry in self.approval_session_log 
                         if entry.get('remember', False)])
        
        # Calculate average response time
        response_times = [entry.get('response_time_seconds', 0) 
        ...
        return {
            ...
            'recent_decisions': self.approval_session_log[-10:] if self.approval_session_log else []
        }
    ...
    def add_no_approval_flag_support(self, no_approval_enabled: bool = False):
        ...
        if no_approval_enabled:
            self.console.print("[yellow]⚠️  WARNING: Approval system bypassed with --no-approval flag[/yellow]")
            self.console.print("[yellow]   All operations will be executed without user confirmation[/yellow]")
    ...
    async def cleanup(self):
        ...
        try:
            ...
            if len(self.approval_session_log) > 0:
                ...
                logger.info(f"Approval session completed with {stats['total_decisions']} decisions")
            ...
            if self.permission_controller:
                ...
                if expired_count > 0:
                    logger.info(f"Cleaned up {expired_count} expired approvals during shutdown")
    ...
    async def _cleanup_approval_manager(self):
        ...
        try:
            ...
            pending_operations = getattr(self.approval_manager, 'pending_operations', {})
            if pending_operations:
                logger.warning(f"Cancelling {len(pending_operations)} pending approval operations")
    ...
    async def _cancel_pending_approvals(self):
        ...
        try:
            ...
            if hasattr(self.approval_manager, 'pending_batches'):
                ...
                if pending_batches:
                    logger.info(f"Cancelling {len(pending_batches)} pending batch approvals")
                    ...
                    for batch_id, batch_request in pending_batches.items():
                        try:
                            # Mark as cancelled
                            batch_request.status = getattr(batch_request, 'status', None)
                            ...
                            logger.error(f"Error cancelling batch {batch_id}: {batch_error}")
                    ...
                    if hasattr(self.approval_manager, 'completed_batches'):
                        for batch_id in cancelled_batches:
                            ...
                            self.approval_manager.completed_batches[batch_id] = batch_request
    ...
    async def handle_operation_cancellation(self, operation_id: str, reason: str = "User cancelled"):
        """
        ...
        try:
            ...
            logger.error(f"Error handling operation cancellation for {operation_id}: {e}")
...
def inject_approval_integration_into_agent_engine(
    agent_engine, 
    cli_approval_integration: CLIApprovalIntegration
):
    """
