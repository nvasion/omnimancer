...
from ..core.agent.agent_config import (
    CustomAgentConfig, ModelSettings, ContextParameters, BehaviorRules,
    AgentTool, CustomAgentStatus, CustomAgentValidator, AgentRepository
)
...
class AgentCreationWizard:
    ...
    async def run_wizard(self, base_config: Optional[CustomAgentConfig] = None) -> Optional[CustomAgentConfig]:
        """
    ...
    def _show_welcome(self):
        ...
        welcome_text = Text.assemble(
            ...
            "You can press ", ("Ctrl+C", "bold"), " at any time to cancel.\\n\\n",
            "Let's get started! 🚀"
        )
    ...
    async def _step_basic_info(self) -> bool:
        ...
        while True:
            name = Prompt.ask(
                ...
                default=self.current_config.name if self.current_config.name != "Untitled Agent" else ""
            ).strip()
            ...
            if len(name) > 100:
                self.console.print("[red]Agent name too long (max 100 characters).[/red]")
                continue
            
            # Check for name conflicts
            existing = self.repository.get_by_name(name)
            if existing:
                self.console.print(f"[yellow]An agent named '{name}' already exists.[/yellow]")
        ...
        if len(description) > 500:
            self.console.print("[yellow]Description truncated to 500 characters.[/yellow]")
    ...
    async def _step_select_template(self) -> bool:
        ...
        self.console.print("You can base your agent on an existing template for easier setup.")
        
        use_template = Confirm.ask("Would you like to use a base template?", default=False)
        ...
        try:
            ...
            if not templates:
                self.console.print("[yellow]No templates available. Continuing without template.[/yellow]")
                self.current_config.base_template_id = None
            ...
            for i, template in enumerate(templates, 1):
                description = template.description[:60] + ("..." if len(template.description) > 60 else "")
            ...
            while True:
                try:
                    ...
                    if choice == 0:
                        ...
                        self.console.print(f"[green]Selected template: {selected_template.name}[/green]")
                        break
                    else:
                        self.console.print(f"[red]Please enter a number between 0 and {len(templates)}.[/red]")
                except (ValueError, EOFError):
                    self.console.print("[red]Invalid input. Please enter a number.[/red]")
    ...
    async def _step_model_settings(self) -> bool:
        ...
        while True:
            try:
                ...
                if 1 <= choice <= len(providers):
                    ...
                    self.console.print(f"[red]Please enter a number between 1 and {len(providers)}.[/red]")
            except (ValueError, EOFError):
                self.console.print("[red]Invalid input. Please enter a number.[/red]")
        ...
        model_name = Prompt.ask(
            "Model name",
            default=current_settings.model_name
        ).strip()
            ...
            while True:
                try:
                    ...
                    if max_tokens > 0:
                        ...
                        self.console.print("[red]Maximum tokens must be positive.[/red]")
                except (ValueError, EOFError):
                    self.console.print("[red]Invalid input. Please enter a number.[/red]")
    ...
    async def _step_tools_capabilities(self) -> bool:
        ...
        for tool in all_tools:
            ...
            tool_table.add_row(tool.value.replace('_', ' ').title(), description, enabled)
        ...
        while True:
            ...
            if choice == "1":
                selected_tools = await self._interactive_tool_selection(all_tools, selected_tools)
            elif choice == "2":
        ...
        if suggested_capabilities:
            ...
            if Confirm.ask("Use suggested capabilities?", default=True):
                ...
                self.current_config.capabilities = await self._interactive_capability_selection(
    ...
    async def _step_context_parameters(self) -> bool:
        ...
        if context_size < 1000:
            self.console.print("[yellow]Warning: Very small context window may limit functionality.[/yellow]")
    ...
    async def _step_behavior_rules(self) -> bool:
        ...
        for option, description in confirmation_options:
            if Confirm.ask(f"Require confirmation for {description}?", 
    ...
    def _show_configuration_summary(self):
        ...
        basic_table.add_row("Base Template", config.base_template_id or "[dim]None[/dim]")
        ...
        model_table.add_row("Max Tokens", str(config.model_settings.max_tokens) if config.model_settings.max_tokens else "[dim]Unlimited[/dim]")
        
        self.console.print(Panel(model_table, title="Model Configuration", box=ROUNDED))
        
        # Tools and capabilities
        tools_text = ", ".join([tool.value.replace('_', ' ').title() for tool in config.enabled_tools])
        caps_text = ", ".join([cap.value.replace('_', ' ').title() for cap in config.capabilities])
        ...
        self.console.print(Panel(tools_table, title="Tools and Capabilities", box=ROUNDED))
    
    async def _interactive_tool_selection(self, all_tools: List[AgentTool], 
                                        current_selection: Set[AgentTool]) -> Set[AgentTool]:
        ...
        while True:
            ...
            for i, tool in enumerate(all_tools, 1):
                ...
                self.console.print(f"  {i}. {status} {tool.value.replace('_', ' ').title()}")
            ...
            try:
                ...
                if choice == len(all_tools) + 1:
                    ...
                    self.console.print(f"[red]Please enter a number between 1 and {len(all_tools) + 1}.[/red]")
            except (ValueError, EOFError):
                self.console.print("[red]Invalid input. Please enter a number.[/red]")
    ...
    async def _preset_tool_selection(self) -> Set[AgentTool]:
        ...
        presets = {
            ...
            "standard": {AgentTool.FILE_OPERATIONS, AgentTool.PROGRAM_EXECUTOR, AgentTool.SYSTEM_INFO},
            "web_enabled": {AgentTool.FILE_OPERATIONS, AgentTool.PROGRAM_EXECUTOR, AgentTool.WEB_CLIENT, AgentTool.SEARCH},
        ...
        for i, name in enumerate(preset_names, 1):
            tool_list = ", ".join([t.value.replace('_', ' ').title() for t in presets[name]])
        ...
        while True:
            try:
                ...
                if 1 <= choice <= len(preset_names):
                    ...
                    self.console.print(f"[red]Please enter a number between 1 and {len(preset_names)}.[/red]")
            except (ValueError, EOFError):
                self.console.print("[red]Invalid input. Please enter a number.[/red]")
    
    async def _interactive_capability_selection(self, all_capabilities: List[PersonaCapability],
                                              current_selection: Set[PersonaCapability]) -> Set[PersonaCapability]:
        """Interactive capability selection interface."""
        # Group capabilities for easier selection
        capability_groups = {
            "Core": [PersonaCapability.GENERAL_PURPOSE, PersonaCapability.REASONING, PersonaCapability.ANALYSIS],
            "Development": [PersonaCapability.CODE_GENERATION, PersonaCapability.FILE_OPERATIONS, PersonaCapability.TOOL_CALLING],
            "Research": [PersonaCapability.RESEARCH, PersonaCapability.WEB_SEARCH, PersonaCapability.LARGE_CONTEXT],
            "Creative": [PersonaCapability.CREATIVE_WRITING, PersonaCapability.HIGH_TEMPERATURE],
            "Performance": [PersonaCapability.FAST_RESPONSE, PersonaCapability.COST_EFFICIENT, PersonaCapability.BALANCED]
        }
        ...
        for group_name, group_caps in capability_groups.items():
            ...
            for cap in group_caps:
                if cap in all_capabilities:
                    ...
                    self.console.print(f"  {status} {cap.value.replace('_', ' ').title()}")
    ...
    def _suggest_capabilities_for_tools(self, tools: Set[AgentTool]) -> Set[PersonaCapability]:
...
async def create_agent_wizard(console: Optional[Console] = None) -> AgentCreationWizard:
    """Create and initialize an agent creation wizard."""
    return AgentCreationWizard(console=console)


async def quick_agent_creation(name: str, provider: str = "anthropic", 
                              model: str = "claude-3-haiku-20240307") -> Optional[CustomAgentConfig]:
