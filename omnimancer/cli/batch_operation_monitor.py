...
class BatchOperationMonitor:
    ...
    async def execute_batch(self, 
                          ...
                          operation_executor: callable) -> BatchExecutionSummary:
        """
        ...
        try:
            ...
            self.current_summary.error_summary.append(f"Batch execution error: {str(e)}")
            return self.current_summary
        finally:
    ...
    async def _execute_operations(self, 
        ...
        for i, operation in enumerate(batch_request.operations):
            ...
            if i not in approved_indices:
                self._record_operation_result(i, operation, OperationStatus.SKIPPED, 
    ...
    def _record_operation_result(self, 
                               operation_id: int,
                               operation: Operation,
                               status: OperationStatus,
    ...
    def _create_status_display(self) -> Panel:
        ...
        for result in self.operation_results.values():
            ...
            if result.error_message and result.status == OperationStatus.FAILED:
                recent_events.append(f"[red]Failed[/red]: {result.operation.type.value} - {result.error_message}")
            elif result.status == OperationStatus.COMPLETED:
                recent_events.append(f"[green]Completed[/green]: {result.operation.type.value}")
        ...
        # Recent events
        events_text = "\n".join(recent_events[-5:]) if recent_events else "[dim]No recent events[/dim]"
    ...
    async def _save_execution_results(self):
        ...
        json_data = {
            'summary': {
                ...
                'end_time': self.current_summary.end_time.isoformat() if self.current_summary.end_time else None,
                'duration_seconds': self.current_summary.duration.total_seconds() if self.current_summary.duration else None,
    ...
    def display_execution_summary(self, summary: Optional[BatchExecutionSummary] = None):
        ...
        summary_table.add_row("Execution Time:", str(summary.duration) if summary.duration else "Unknown")
        summary_table.add_row("Total Operations:", str(summary.total_operations))
        summary_table.add_row("Successful:", f"[green]{summary.successful_operations}[/green]")
        ...
        summary_table.add_row("Retried:", f"[yellow]{summary.retried_operations}[/yellow]")
        summary_table.add_row("Success Rate:", f"[green]{summary.success_rate:.1f}%[/green]")
        summary_table.add_row("Result:", self._format_batch_result(summary.result))
        
        summary_panel = Panel(
            summary_table,
            title="[bold]Batch Execution Summary[/bold]",
            border_style="green" if summary.result == BatchOperationResult.SUCCESS else "red"
        )
    ...
    def _display_operation_details(self):
        ...
        for result in sorted(self.operation_results.values(), key=lambda x: x.operation_id):
            status_display = self._format_operation_status(result.status)
            duration_display = f"{result.duration.total_seconds():.1f}s" if result.duration else "N/A"
            retry_display = str(result.retry_count) if result.retry_count > 0 else "-"
            error_display = result.error_message[:40] + "..." if result.error_message and len(result.error_message) > 40 else (result.error_message or "")
    ...
    def export_results(self, format_type: str = "json", output_path: Optional[Path] = None) -> Optional[Path]:
        """
        ...
        try:
            if format_type.lower() == "json":
                ...
                self.console.print(f"[red]Unsupported export format: {format_type}[/red]")
                return None
        except Exception as e:
    ...
    def _export_json(self, output_path: Path) -> Path:
        ...
        data = {
            'batch_summary': {
                ...
                'result': self.current_summary.result.value,
                'success_rate': self.current_summary.success_rate,
                'execution_time': str(self.current_summary.duration) if self.current_summary.duration else None,
                'operations_summary': {
    ...
    def _export_csv(self, output_path: Path) -> Path:
        ...
        with open(output_path, 'w', newline='') as f:
            ...
            # Write header
            writer.writerow([
                'Operation ID', 'Type', 'Description', 'Status', 
                'Duration (s)', 'Retry Count', 'Error Message'
            ])
...
def create_batch_monitor(console: Optional[Console] = None, **kwargs) -> BatchOperationMonitor:
    """
