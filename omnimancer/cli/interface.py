...
from ..ui.progress_indicator import ProgressIndicator, OperationType, set_progress_indicator
from .commands import Command, CommandType, SlashCommand, parse_command, get_command_registry
...
class DisplayManager:
    ...
    def __init__(self, console: Console):
        ...
        self._formats = {
            MessageType.SUCCESS: {"text": "[green]✓ {message}[/green]", "panel": None},
            MessageType.ERROR: {"text": None, "panel": {"title": "Error", "style": "red"}},
            MessageType.WARNING: {"text": "[yellow]⚠ {message}[/yellow]", "panel": None},
            MessageType.INFO: {"text": "[cyan]ℹ {message}[/cyan]", "panel": None}
        }
    ...
    def show_panel(self, content: str, title: str, style: str = "blue", icon: str = None) -> None:
...
class CompletionManager:
    ...
    def get_completions(self, command: str, arg_index: int, text: str, args: List[str]) -> List[str]:
        ...
        static_completions = {
            ...
            'history': {
                0: ['list', 'clear', 'export', 'import']
            },
            'config': {
                0: ['show', 'set', 'get', 'generate', 'validate', 'setup', 'mode', 'migrate', 'templates', 'reset']
            },
            'validate': {
                0: ['--fix', '--auto-fix']
            },
...
class CommandLineInterface:
    ...
    def __init__(self, engine: CoreEngine, no_approval: bool = False):
        ...
        verbose_mode = getattr(engine, 'verbose', False) or sys.argv.count('--verbose') > 0
    ...
    async def _async_start(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if not hasattr(config, 'providers') or not config.providers:
                    ...
                    try:
                        ...
                        setup_wizard = SetupWizard(self.engine.config_manager, provider_registry)
                        ...
                        if success:
                            ...
                            try:
                                ...
                                self.console.print(f"[yellow]Warning: Provider initialization still failed: {retry_e}[/yellow]")
                                self.console.print("[yellow]You can continue using Omnimancer and configure providers later with /setup[/yellow]")
                        else:
                        ...
                        self.console.print("[yellow]You can try running /setup manually later.[/yellow]")
                else:
                    self.console.print("[yellow]You can try running /setup to reconfigure providers or /validate to check your configuration.[/yellow]")
            ...
            try:
                ...
                self.agent_progress_ui = AgentProgressUI(self.agent_manager, self.console)
            ...
            try:
                if self.agent_manager:
                    await self.agent_manager.disable_agent_mode(wait_for_completion=False)
                if self.agent_progress_ui:
    ...
    def _initialize_console_with_fallback(self) -> Console:
        ...
        is_tty = hasattr(sys, 'stdout') and hasattr(sys.stdout, 'isatty') and sys.stdout.isatty()
    ...
    def _setup_approval_integration(self):
        ...
        try:
            from .approval_integration import create_cli_approval_integration, inject_approval_integration_into_agent_engine
            ...
            if agent_engine and approval_manager:
                ...
                logger.debug("No agent engine or approval manager available for integration")
    ...
    async def _complete_approval_integration_setup(self):
        ...
        if hasattr(self, '_approval_setup_params') and not self.approval_integration:
            try:
                ...
                logger.error(f"Failed to complete async approval integration setup: {e}")
    
    def _setup_file_interaction_integration(self):
        """Set up CLI file interaction integration for read-before-write operations."""
        try:
            from ..core.agent.read_before_write_ui import create_review_callback, create_confirmation_callback
            ...
            if agent_engine and hasattr(agent_engine, 'set_read_before_write_callback'):
                ...
                confirmation_callback = create_confirmation_callback(console=self.console)
                ...
                if hasattr(agent_engine.file_system, 'set_confirmation_callback'):
                    agent_engine.file_system.set_confirmation_callback(confirmation_callback)
...
IMPORTANT: Always use these exact markers when you want to perform operations. Do not just describe what you would do - actually use the markers to do it!"""
    ...
    async def _execute_continuous_workflow(self, original_message: str, initial_response) -> None:
        """
        ...
        while iteration_count < max_iterations:
            ...
            executed_response = await self._parse_and_execute_operations(current_response.content)
            ...
            had_operations = any(re.search(pattern, current_response.content, re.DOTALL) 
            ...
            done_indicators = [
                'task is complete', 'analysis complete', 'finished', 'done', 'complete',
                'workflow finished', "i'm done", 'task completed', 'everything looks good'
            ]
            ...
            if any(indicator in content_lower for indicator in done_indicators):
                ...
                self._show_assistant_message(next_response.content, next_response.model_used)
                break
                
            current_response = next_response
            
        if iteration_count >= max_iterations:
            self._show_warning("Workflow stopped after maximum iterations to prevent infinite loop")
    ...
    def _is_action_request(self, message: str) -> bool:
        ...
        action_verbs = [
            ...
            "test", "validate", "verify", "scan", "find", "search",
        ...
        question_patterns = [
            "how do i", "how can i", "what should i", "can you help",
            "could you help", "would you help", "please help"
        ]
        
        contains_action_question = any(pattern in normalized for pattern in question_patterns)
        
        # Determine if this is an action request
        is_action = any(imperative_patterns) or contains_action_verb or contains_action_question
        
        # Exclude pure questions without action intent
        pure_question_starters = ["what is", "what are", "who is", "who are", "when is", "when was", "where is", "why"]
        ...
        try:
            ...
            for match in re.finditer(file_write_pattern, response_content, re.DOTALL):
                ...
                if hasattr(self.engine, 'agent_engine'):
                    result = await self.engine.agent_engine.execute_with_approval(operation)
                    if result.success:
                        updated_response = updated_response.replace(match.group(0), 
        ...
        async def ai_processing_task():
            ...
            try:
                ...
                if response.is_success:
                    ...
                    if self.agent_manager and self.agent_manager.mode.value == 'on':
                        await self._execute_continuous_workflow(command.content, response)
                    else:
    ...
    async def _handle_dynamic_command(self, command: Command) -> None:
        ...
        try:
            ...
            if dynamic_cmd.handler:
                ...
                result = await self._execute_dynamic_handler(dynamic_cmd.handler, command.args)
                if result:
                    self.console.print(result)
            
            # Check if command has a script to execute
            elif dynamic_cmd.script_path:
                result = await self._execute_dynamic_script(dynamic_cmd.script_path, command.args)
                if result:
                    self.console.print(result)
            
            else:
                self._show_error(f"Dynamic command '{dynamic_cmd.name}' has no handler or script")
                
        except Exception as e:
            self._show_error(f"Error executing dynamic command '{dynamic_cmd.name}': {e}")
    
    async def _execute_dynamic_handler(self, handler: Callable, args: List[str]) -> Optional[str]:
    ...
    async def _execute_dynamic_script(self, script_path: Path, args: List[str]) -> Optional[str]:
...
Just type your message to start chatting with AI!"""
    ...
    def _show_command_help(self, command_name: str) -> None:
        ...
        if help_content:
            ...
            self._show_info("Available commands: /help, /quit, /clear, /status, /models, /providers, /switch, /tools, /mcp, /save, /load, /list, /config")
...
Best for: Free Claude access, privacy-focused usage, code tasks"""
        ...
        if help_content:
            ...
            self._show_info("Available providers: claude, openai, gemini, cohere, ollama")
...
Model available: {'Yes' if model_info else 'No'}"""
    ...
    def _show_assistant_message(self, message: str, model: str) -> None:
        ...
        for pattern in operation_patterns:
            escaped_message = re.sub(pattern, lambda m: m.group(0).replace('[', '\\[').replace(']', '\\]'), escaped_message)
    ...
    async def _get_enhanced_models_list(self, filter_type: str = None, filter_value: str = None) -> str:
        ...
        try:
            ...
            if filter_type == "provider" and filter_value:
                all_models = {k: v for k, v in all_models.items() if k.lower() == filter_value.lower()}
            elif filter_type == "free":
                filtered_models = {}
                for provider, models in all_models.items():
                    free_models = [m for m in models if getattr(m, 'is_free', False) or 
                                  (hasattr(m, 'cost_per_token') and m.cost_per_token == 0)]
                    if free_models:
                ...
                filtered_models = {}
                for provider, models in all_models.items():
                    latest_models = [m for m in models if getattr(m, 'latest_version', False)]
                    if latest_models:
            ...
            for provider_name in sorted(all_models.keys()):
                ...
                provider_status = "✓"
                current_marker = " (active)" if provider_name == current_provider_name else ""
                ...
                for model in sorted(models, key=lambda m: m.name):
                    ...
                    if model.name == current_model and provider_name == current_provider_name:
                        status = "●"  # Current
                    ...
                    # Cost (simplified)
                    is_free = getattr(model, 'is_free', False) or (hasattr(model, 'cost_per_token') and getattr(model, 'cost_per_token', 0) == 0)
                    if is_free:
                        cost = "FREE"
                    else:
                        input_cost = getattr(model, 'cost_per_million_input', None) or getattr(model, 'cost_per_token', 0) * 1000000
                    ...
                    for remove_str in [provider_name.title(), provider_name.upper(), model.name, 'via OpenRouter']:
                        desc = desc.replace(remove_str, '').strip(' -')
                    if len(desc) > 40:
                        desc = desc[:37] + "..."
                    
                    # Format line with fixed widths
                    output_lines.append(f"  {name:35} {status:6} {caps:4} {cost:8} {desc}")
    ...
    async def _handle_switch_command(self, command: Command) -> None:
        ...
        if len(args) < 1:
            ...
            providers_command = Command.create_slash_command(SlashCommand.PROVIDERS, [], "/providers")
            ...
            with self.console.status("[bold yellow]Switching...", spinner="dots"):
                ...
                if provider_name not in self.engine.providers:
                    if provider_name in available_providers:
                        self._show_error(f"Provider '{provider_name}' is available but not configured.")
                        self._show_info("Configure it in your settings first, then try switching again.")
                        return
                    else:
                        # Show suggestions for similar provider names
                        suggestions = [p for p in available_providers if provider_name in p.lower() or p.lower() in provider_name]
                        if suggestions:
                            self._show_error(f"Provider '{provider_name}' not found. Did you mean: {', '.join(suggestions)}?")
                        else:
                            self._show_error(f"Provider '{provider_name}' not found.")
                        from .commands import Command, CommandType
                        providers_command = Command(content="/providers", command_type=CommandType.SLASH_COMMAND, args=[])
                        await self._show_providers(providers_command)
                        return
                ...
                if model_name:
                    ...
                    custom_model_names = [m.name for m in custom_models if m.provider == provider_name]
                    
                    # Combine both lists
                    all_model_names = model_names + custom_model_names
                    
                    if model_name not in all_model_names:
                        self._show_error(f"Model '{model_name}' not available for provider '{provider_name}'.")
                        if all_model_names:
                            # Show suggestions for similar model names
                            suggestions = [m for m in all_model_names if model_name.lower() in m.lower() or m.lower() in model_name.lower()]
                            if suggestions:
                                self._show_info(f"Available models for {provider_name}: {', '.join(suggestions[:5])}")
                            else:
                                self._show_info(f"Available models for {provider_name}: {', '.join(all_model_names[:5])}")
                        return
                
                success = await self.engine.switch_model(provider_name, model_name)
                
            if success:
                current_model = self.engine.current_provider.model if self.engine.current_provider else "unknown"
                ...
                try:
                    ...
                    if model_info and model_info.cost_per_token:
                        model_details.append(f"Cost: ${model_info.cost_per_token:.6f}/token")
                    
                    detail_text = f" | {' | '.join(model_details)}" if model_details else ""
                except:
                    detail_text = ""
                
                capability_text = f" ({', '.join(capabilities)})" if capabilities else ""
                self._show_info(f"✓ Switched to {provider_name}:{current_model}{capability_text}{detail_text}")
                ...
                if provider_info.supports_tools():
                    try:
                        ...
                        if tools_info:
                            ...
                            self._show_info(f"🔧 {tool_count} MCP tools available for use")
                        else:
                            self._show_info("🔧 Tool calling supported (no MCP tools currently available)")
                    except:
                        pass
            else:
    ...
    async def _handle_save_command(self, command: Command) -> None:
        ...
        try:
            ...
            with self.console.status("[bold yellow]Saving conversation...", spinner="dots"):
    ...
    async def _handle_load_command(self, command: Command) -> None:
        ...
        try:
            with self.console.status("[bold yellow]Loading conversation...", spinner="dots"):
            ...
            if info:
                ...
                self._show_info(f"Messages: {info['message_count']}, Model: {info['current_model']}")
            else:
                self._show_info(f"Loaded conversation: {filename}")
    ...
    async def _handle_config_generate(self, args: list) -> None:
        ...
        try:
            ...
            with self.console.status("[bold green]Generating configuration...", spinner="dots"):
                ...
                if template:
                    ...
                    if template == "general":
                        ...
                        self._show_info(f"✅ Generated {template} template configuration: {output_path}")
                        self.console.print(f"[dim]General purpose configuration with OpenAI and Anthropic providers[/dim]")
                    else:
                        self._show_error(f"Template functionality simplified. Only 'general' template is available.")
                ...
                self.console.print("1. Edit the configuration file to add your API keys")
    ...
    async def _handle_config_validate(self, args: list) -> None:
        ...
        try:
            ...
            with self.console.status("[bold yellow]Validating configuration...", spinner="dots"):
                if provider_name:
    ...
    async def _validate_single_provider(self, provider_name: str) -> None:
        ...
        try:
            ...
            validation_results.append(("Configuration", True, "Provider initialized successfully"))
        except Exception as e:
            validation_results.append(("Configuration", False, f"Initialization failed: {e}"))
            
        # 2. API key validation (if applicable)
        try:
            if hasattr(provider, 'api_key') and provider.api_key:
                # Basic format validation
                if provider_name == "claude" and not provider.api_key.startswith("sk-ant-"):
                    validation_results.append(("API Key Format", False, "Should start with 'sk-ant-'"))
                elif provider_name == "openai" and not provider.api_key.startswith("sk-"):
                    validation_results.append(("API Key Format", False, "Should start with 'sk-'"))
                elif provider_name == "gemini" and not provider.api_key.startswith("AIza"):
                    validation_results.append(("API Key Format", False, "Should start with 'AIza'"))
                else:
                    validation_results.append(("API Key Format", True, "Format appears valid"))
            else:
                if provider_name not in ["ollama", "claude-code"]:
                    validation_results.append(("API Key", False, "API key not configured"))
                else:
                    validation_results.append(("API Key", True, "Not required for this provider"))
        except Exception as e:
        ...
        try:
            ...
            model_available = any(m.name == current_model and m.available for m in models)
            
            if model_available:
                validation_results.append(("Model Availability", True, f"Model '{current_model}' is available"))
            else:
                validation_results.append(("Model Availability", False, f"Model '{current_model}' not available"))
        except Exception as e:
            validation_results.append(("Model Availability", False, f"Check failed: {e}"))
        ...
        try:
            ...
            model_info = provider.get_model_info()
            if model_info:
                validation_results.append(("Connection", True, "Provider responds to requests"))
            else:
                validation_results.append(("Connection", False, "No response from provider"))
        except Exception as e:
        ...
        try:
            ...
            if capabilities:
                validation_results.append(("Capabilities", True, f"Supports: {', '.join(capabilities)}"))
            else:
                validation_results.append(("Capabilities", True, "Basic text generation"))
        except Exception as e:
    ...
    async def _validate_all_providers(self) -> None:
        ...
        if not self.engine.providers:
            self._show_error("No providers configured. Use '/setup' to configure providers.")
            return
        ...
        for provider_name in sorted(self.engine.providers.keys()):
            ...
            try:
                ...
                provider_status['issues'].append(f"Health check failed: {str(e)[:50]}...")
        ...
        for status in validation_summary:
            ...
            # Capabilities
            capabilities = " ".join(status['capabilities']) if status['capabilities'] else "—"
        ...
        if health_percentage == 100:
            self.console.print("[bold green]🎉 All providers are fully operational![/bold green]")
        elif health_percentage >= 75:
            self.console.print("[bold yellow]⚠️ Most providers are working, but some need attention.[/bold yellow]")
        else:
            self.console.print("[bold red]❌ Multiple providers have issues. Consider running setup wizard.[/bold red]")
        ...
        if issues_found:
            ...
            self.console.print("[dim]Use '/setup' to reconfigure problematic providers[/dim]")
    ...
    async def _handle_config_set(self, key: str, value: str) -> None:
        ...
        try:
            ...
            self._show_info("Note: Some settings may require restarting Omnimancer to take effect")
        except Exception as e:
    ...
    async def _handle_config_setup(self, args: list) -> None:
        ...
        try:
            from .config_setup_wizard import run_config_setup_wizard, run_quick_setup
            ...
            if quick:
                ...
                self.console.print("[blue]Starting Configuration Setup Wizard...[/blue]")
                success = run_config_setup_wizard(config_manager)
            
            if success:
                self.console.print("[green]✅ Configuration setup completed![/green]")
            else:
                self.console.print("[yellow]⚠️ Configuration setup was cancelled or failed.[/yellow]")
    ...
    async def _handle_config_mode(self, args: list) -> None:
        ...
        try:
            from ..core.config_provider import ConfigurationProvider, ConfigurationMode
            from ..core.config_manager import ConfigManager
            ...
            if not args:
                ...
                self.console.print(f"Current mode: [blue]{current_mode.value.title()}[/blue]")
                ...
                if mode in ["simple", "advanced", "guided"]:
                    ...
                    self.console.print(f"[green]✓[/green] Configuration mode set to: {mode.title()}")
                else:
                    self._show_error(f"Invalid mode: {mode}. Available modes: simple, advanced, guided")
    ...
    async def _handle_config_migrate(self, args: list) -> None:
        ...
        try:
            ...
            if not force and not Confirm.ask(f"Migrate to '{template}' template?", default=False):
                self.console.print("Migration cancelled.")
                return
            
            # Create and execute migration plan
            self.console.print("[blue]Creating migration plan...[/blue]")
            migration_plan = migration_helper.create_migration_plan(template, preserve_customizations=True)
            ...
            success, messages = migration_helper.execute_migration(migration_plan, template)
    ...
    async def _handle_config_templates(self, args: list) -> None:
        ...
        try:
            ...
            if not templates:
                self.console.print("[yellow]No templates found matching criteria.[/yellow]")
                return
    ...
    async def _handle_config_reset(self, args: list) -> None:
        ...
        try:
            ...
            if not force and not Confirm.ask("This will reset all configuration to defaults. Continue?", default=False):
                self.console.print("Reset cancelled.")
                return
            
            if backup:
                backup_path = config_manager.backup_config()
                self.console.print(f"[green]✓[/green] Configuration backed up to: {backup_path}")
    ...
    async def _handle_config_validate_enhanced(self, args: list) -> None:
        ...
        try:
            ...
            compat_status = "✅ Compatible" if validation_report.is_compatible else "❌ Issues Found"
            status_table.add_row("Compatibility", compat_status, f"Version: {validation_report.format_info.get('version', 'unknown')}")
            ...
            status_table.add_row("Health Score", health_status, "Overall configuration health")
            ...
            if fix and (not validation_report.is_valid or not validation_report.is_compatible):
                ...
                if success:
                    self.console.print("[green]✅ Configuration fixes applied successfully![/green]")
                else:
                    self.console.print("[yellow]⚠️ Some issues require manual attention.[/yellow]")
                ...
                if validation_report.migration_required:
                    self.console.print(f"Or run [cyan]/config migrate[/cyan] to migrate to simplified configuration.")
...
[dim]/config templates --use-case coding[/dim]
    ...
    def _show_migration_plan(self, migration_plan) -> None:
        ...
        risk_color = {"low": "green", "medium": "yellow", "high": "red"}.get(migration_plan.risk_level, "white")
...
{', '.join(template.get('recommended_for', []))}
    ...
    async def _handle_setup_command(self, command: Command) -> None:
        ...
        try:
            ...
            self.console.print("[bold blue]🚀 Starting Omnimancer Setup Wizard...[/bold blue]\n")
            ...
            if success:
                self.console.print("\n[bold green]✅ Setup completed successfully![/bold green]")
                self.console.print("[dim]You may need to restart Omnimancer for changes to take effect.[/dim]")
            else:
    ...
    async def _set_config_value(self, key: str, value: str) -> None:
        ...
        try:
            if key == "default_provider":
                ...
                self._show_error(f"Configuration key '{key}' cannot be set via CLI. Use provider-specific commands or edit config file.")
    ...
    async def _show_conversations(self) -> None:
        ...
        try:
            ...
            for conv in conversations:
                ...
                lines.append(f"     Messages: {conv['message_count']}, Model: {conv.get('current_model', 'Unknown')}")
                lines.append("")
    ...
    async def _get_enhanced_providers_list(self) -> str:
        ...
        try:
            ...
            console.print(table)
            summary_lines = ["\n" + output.getvalue()]
            summary_lines.extend(self._build_provider_summary(available_provider_names, current_provider_name))
    ...
    def _get_provider_notes(self, provider, provider_name: str) -> str:
        ...
        if provider:
            try:
                model_info = provider.get_model_info()
                if model_info:
                    if hasattr(model_info, 'cost_per_token') and model_info.cost_per_token == 0:
                        notes.append("Free")
                    elif hasattr(model_info, 'max_tokens') and model_info.max_tokens > 100000:
    ...
    def _build_provider_summary(self, available_provider_names: list, current_provider_name: str) -> list:
        ...
        # Setup guidance
        unconfigured = [name for name in available_provider_names if name not in self.engine.providers]
        if unconfigured:
            lines.extend([
                "\n💡 Setup Guidance:",
    ...
    async def _handle_mcp_command(self, command: Command) -> None:
        ...
        try:
            with self.console.status("[bold yellow]Processing MCP command...", spinner="dots"):
                result = await self.engine._handle_mcp_command(command)
            
            mcp_panel = Panel(
                result,
                title="MCP Command Result",
                border_style="cyan"
            )
        ...
        for i, arg in enumerate(args):
            if not arg.startswith('--'):
                ...
                try:
                    ...
                    self._show_error("Invalid interval value. Using default 300 seconds.")
        
        try:
            if monitor_mode:
                # Start continuous monitoring
                self._show_info(f"Starting health monitoring (interval: {interval}s). Press Ctrl+C to stop.")
                
                try:
                    while True:
                        # Perform health check
                        health_status = await self.engine.check_provider_health(provider_name)
                        
                        # Display results
                        lines = ["Provider Health Status:"]
                        for provider, status in health_status.items():
                            status_icon = "✅" if status['status'] == 'healthy' else "❌"
                            lines.append(f"  {status_icon} {provider}: {status['message']}")
                ...
                with self.console.status("[bold yellow]Checking provider health...", spinner="dots"):
                    health_status = await self.engine.check_provider_health(provider_name)
                    ...
                    for provider, status in health_status.items():
                        ...
                        if status.get('credentials_valid') is not None:
                            cred_status = "✅ Valid" if status['credentials_valid'] else "❌ Invalid"
                            lines.append(f"  Credentials: {cred_status}")
                        
                        if status.get('model_available') is not None:
                            model_status = "✅ Available" if status['model_available'] else "❌ Unavailable"
    ...
    async def _handle_repair_command(self, command: Command) -> None:
        ...
        try:
            with self.console.status("[bold yellow]Analyzing configuration...", spinner="dots"):
                config = self.engine.config_manager.get_config()
    ...
    async def _handle_diagnose_command(self, command: Command) -> None:
        ...
        try:
            with self.console.status("[bold green]Running system diagnostics...", spinner="dots"):
                diagnostics = {}
                ...
                if config_diag['valid']:
                    ...
                    diag_lines.append(f"  📊 Providers configured: {config_diag['providers_configured']}")
                    diag_lines.append(f"  🎯 Default provider: {config_diag['default_provider'] or 'None'}")
                else:
                    diag_lines.append(f"  ❌ Configuration error: {config_diag.get('error', 'Unknown')}")
                
                # Provider health
                diag_lines.append("\n🏥 Provider Health:")
                providers_diag = diagnostics['providers']
                if isinstance(providers_diag, dict) and 'error' not in providers_diag:
                    healthy_count = sum(1 for status in providers_diag.values() if status.get('status') == 'healthy')
                    total_count = len(providers_diag)
                    diag_lines.append(f"  📊 {healthy_count}/{total_count} providers healthy")
                    
                    if detailed:
                        for prov_name, status in providers_diag.items():
                            status_icon = "✅" if status.get('status') == 'healthy' else "❌"
                            diag_lines.append(f"    {status_icon} {prov_name}: {status.get('message', 'Unknown status')}")
                else:
                    diag_lines.append(f"  ❌ Health check failed: {providers_diag.get('error', 'Unknown error')}")
                
                # System info
                diag_lines.append("\n💻 System Information:")
                system_diag = diagnostics['system']
                diag_lines.append(f"  🐍 Python version: {system_diag['python_version']}")
                ...
                elif diagnostics['providers'] and isinstance(diagnostics['providers'], dict):
                    unhealthy_providers = [name for name, status in diagnostics['providers'].items() 
                                         if status.get('status') != 'healthy']
                    if unhealthy_providers:
                        diag_lines.append(f"  • Check credentials for: {', '.join(unhealthy_providers)}")
    ...
    async def _handle_validate_command(self, command: Command) -> None:
        ...
        try:
            with self.console.status("[bold yellow]Validating configurations...", spinner="dots"):
                if provider_name:
                    # Validate specific provider
                    config = self.engine.config_manager.get_config()
                    if provider_name not in config.providers:
                    ...
                    if provider_config.api_key:
                        try:
                            ...
                            if health_status[provider_name]['status'] != 'healthy':
                                validation_errors.append(f"Provider health check failed: {health_status[provider_name]['message']}")
                        except Exception as e:
                    ...
                    if validation_errors:
                        ...
                        if fix_mode:
                            self._show_info("Auto-fix mode not yet implemented for provider validation.")
                    else:
                        validation_panel = Panel(
                    ...
                    for prov_name, prov_config in config.providers.items():
                        ...
                        if prov_config.api_key:
                            try:
                                health_status = await self.engine.check_provider_health(prov_name)
                                if health_status[prov_name]['status'] != 'healthy':
                                    errors.append(f"Health check failed: {health_status[prov_name]['message']}")
                            except Exception as e:
                    ...
                    if fix_mode:
                        self._show_info("Use '/repair --fix' to automatically fix configuration issues.")
    ...
    async def _handle_agent_command(self, command: Command) -> None:
        ...
        if subcommand in ['list', 'use', 'current', 'info', 'history', 'recommend', 'compare', 'preview', 'discover', 'help'] and self.agent_persona_handler:
            await self.agent_persona_handler.handle_agent_command(command)
            return
        
        # Handle agent mode commands
        if not self.agent_manager:
            self._show_error("Agent mode is not available. Failed to initialize agent manager.")
            return
        ...
        try:
            if subcommand in ['on', 'enable']:
                ...
                success = await self.agent_manager.enable_agent_mode(auto_approve=auto_approve)
                ...
                if active_count > 0:
                    from rich.prompt import Confirm
                    if not Confirm.ask(f"There are {active_count} active operations. Disable anyway?"):
                        self._show_info("Agent mode remains enabled.")
                        return
                
                self._show_info("Disabling agent mode...")
                success = await self.agent_manager.disable_agent_mode(wait_for_completion=True)
                ...
                if success:
                    self._show_success("Agent mode paused. Use '/agent resume' to continue.")
                else:
                    self._show_error("Failed to pause agent mode. Agent may not be running.")
                ...
                if success:
                    ...
                    self._show_error("Failed to resume agent mode. Use '/agent on' to enable.")
            
            else:
                self._show_error(f"Unknown agent subcommand: {subcommand}")
                self._show_info("Available commands: on, off, status, dashboard, pause, resume")
        ...
        try:
            ...
            if not hasattr(self, 'status_integration') or not self.status_integration:
                try:
                    ...
                    from ..core.agent.cli_integration import create_cli_status_integration
                    ...
                    self._show_info("To enable status display, ensure you're using a status-integrated agent engine")
                    return
    ...
    async def _handle_approvals_command(self, command: Command) -> None:
        ...
        try:
            ...
            if not hasattr(self, 'approval_integration') or not self.approval_integration:
                # Try to initialize approval integration
                from .approval_integration import create_cli_approval_integration
                try:
                    self.approval_integration = await create_cli_approval_integration(
                        console=self.console
                    )
            ...
            if action == 'list':
                ...
                self._show_info("Valid actions: list, revoke, clear, stats, cleanup, batch-list, batch-show, batch-approve, batch-deny, batch-filter, batch-interactive")
    ...
    async def _list_stored_approvals(self, permission_controller) -> None:
        ...
        for signature, approval_data in approvals.items():
            ...
            display_signature = signature[:50] + "..." if len(signature) > 50 else signature
    ...
    async def _revoke_stored_approval(self, permission_controller, signature: str) -> None:
        """Revoke a specific stored approval."""
        success = permission_controller.revoke_approval(signature)
        
        if success:
            self.console.print(f"[green]✅ Revoked approval for signature: {signature[:50]}...[/green]")
        else:
            self.console.print(f"[red]❌ No approval found with signature: {signature[:50]}...[/red]")
    ...
    async def _cleanup_expired_approvals(self, permission_controller) -> None:
        ...
        if cleaned_count > 0:
            self.console.print(f"[green]✅ Cleaned up {cleaned_count} expired approval(s)[/green]")
        else:
    ...
    async def _list_batch_requests(self) -> None:
        ...
        try:
            ...
            for batch_id, batch_request in approval_manager.pending_batches.items():
                # Calculate basic stats
                total_ops = len(batch_request.operations)
                approved_ops = len(batch_request.approved_operations)
                
                # Assess overall risk
                risk_counts = self._calculate_batch_risk_distribution(batch_request.previews)
            ...
            self.console.print(f"[blue]ℹ️  Use `/approvals batch-interactive <batch_id>` for interactive approval[/blue]")
    ...
    async def _approve_batch(self, batch_id: str) -> None:
        ...
        try:
            ...
            confirm = input("Are you sure you want to approve this entire batch? (yes/no): ")
            ...
            approval_manager._process_batch_approval_result(batch_request, approval_result)
            
            self.console.print(f"[green]✅ Approved batch {batch_id[:8]}... with {total_ops} operations[/green]")
    ...
    async def _deny_batch(self, batch_id: str) -> None:
        ...
        try:
            ...
            total_ops = len(batch_request.operations)
            self.console.print(f"[red]❌ Denied batch {batch_id[:8]}... with {total_ops} operations[/red]")
    ...
    async def _filter_batch(self, batch_id: str, filter_args: list) -> None:
        ...
        try:
            ...
            from .batch_approval_filters import (
                BatchFilterManager, ActionTypeFilter, RiskLevelFilter, 
                TargetPatternFilter, StatusFilter, OperationType
            )
            ...
            while i < len(filter_args):
                if filter_args[i] == '--risk' and i + 1 < len(filter_args):
                    ...
                    op_types = [type_map[name] for name in type_names if name in type_map]
                    if op_types:
            ...
            # Apply filters
            filtered_ops, filtered_previews = filter_manager.apply_filters(batch_request)
            ...
            if filter_manager.get_filter_count() > 0:
                ...
                self.console.print(f"[dim]Active filters: {', '.join(filter_descriptions)}[/dim]\n")
    ...
    async def _interactive_batch_approval(self, batch_id: str) -> None:
        ...
        try:
            ...
            from .batch_approval_display import create_batch_approval_panel
            from .batch_approval_filters import BatchFilterManager, BatchSorter, SortCriteria, SortBy
            ...
            def create_display():
                ...
                sorted_ops, sorted_previews = sorter.sort_batch(
                    filtered_ops, filtered_previews, sort_criteria, batch_request
                )
                ...
                layout["main"].update(panel.render_operations_table(
                    temp_batch, page=current_page, show_approved=show_approved
                ))
            ...
            self.console.print("[dim]Use keyboard shortcuts to interact with the batch[/dim]\n")
            
            with Live(create_display(), console=self.console, refresh_per_second=2) as live:
                while True:
                    try:
                        key = input("\nPress key (or 'help' for commands): ").lower().strip()
                        ...
                        if key in ['q', 'quit', 'exit']:
                            ...
                            approval_result = {'approve_all': True}
                            approval_manager._process_batch_approval_result(batch_request, approval_result)
                            self.console.print("[green]✅ Approved entire batch[/green]")
                            break
                        elif key in ['d', 'deny']:
                            # Deny all
                            reason = input("Reason for denial: ").strip() or "User denied in interactive mode"
                            approval_result = {'deny_all': True, 'reason': reason}
                            approval_manager._process_batch_approval_result(batch_request, approval_result)
                            ...
                            self.console.print("  [green]a/approve[/green] - Approve entire batch")
                            self.console.print("  [red]d/deny[/red] - Deny entire batch") 
                            self.console.print("  [yellow]h/toggle[/yellow] - Toggle showing approved operations")
                            self.console.print("  [blue]q/quit[/blue] - Exit interactive mode")
                        else:
            ...
            if action == 'recent':
                ...
                if len(args) > 1:
                    try:
                        ...
                        self._show_error("Invalid count for recent commands. Using default (20).")
                ...
                for i, entry in enumerate(reversed(recent_commands), 1):
                    ...
                    cmd_display = entry.command[:80] + "..." if len(entry.command) > 80 else entry.command
                ...
                if len(args) < 2:
                    self._show_error("Search requires a query. Usage: /history search <query>")
                    return
...
Newest Entry: {stats['newest_entry'] or 'None'}"""
                ...
                if '--confirm' not in args:
                    self._show_info("To clear command history, use: /history clear --confirm")
                ...
                if len(args) < 2:
                    self._show_error("Export requires a filename. Usage: /history export <file> [format]")
                    return
                ...
                if format_type not in ['json', 'txt']:
                    self._show_error("Format must be 'json' or 'txt'. Defaulting to 'json'.")
                    format_type = 'json'
                
                success = self.history_manager.export_history(filename, format_type)
                if success:
                    self._show_success(f"History exported to {filename} in {format_type} format.")
                else:
                    self._show_error(f"Failed to export history to {filename}.")
            
            else:
                self._show_error(f"Unknown history action: {action}")
                self._show_info("Available actions: recent, search, stats, clear, export")
    ...
    def _create_completion_callback(self) -> Optional[Callable[[str], List[str]]]:
    ...
    def _get_completions(self, line_buffer: str, text: str) -> List[str]:
        ...
        try:
            ...
            if not parts or (len(parts) == 1 and line_buffer.endswith(' ') is False and text.startswith('/')):
            ...
            if parts:
                ...
                if command.startswith('/'):
                    return self._complete_command_arguments(command, parts[1:], text, line_buffer.endswith(' '))
    ...
    def _complete_command_arguments(self, command: str, args: List[str], text: str, at_end: bool) -> List[str]:
        ...
        try:
            ...
            return self.completion_manager.get_completions(command, arg_index, text, args)
        except Exception:
            return []
    ...
    def _get_provider_names(self, text: str) -> List[str]:
        ...
        try:
            if hasattr(self.engine, 'providers') and self.engine.providers:
                ...
                provider_names = ["openai", "claude", "gemini", "openrouter", "azure", "bedrock", "mistral", "perplexity", "cohere", "xai", "ollama", "vertex"]
    ...
    def _get_model_names(self, provider_name: str, text: str) -> List[str]:
        ...
        try:
            ...
            if hasattr(self.engine, 'providers') and provider_name in self.engine.providers:
                provider = self.engine.providers[provider_name]
    ...
    async def _handle_add_model_command(self, command: Command) -> None:
        ...
        try:
            ...
            args = command.args
            if len(args) < 2:
                self._show_error("Add-model command requires at least model ID and provider")
                return
            ...
            if description_parts:
                description = ' '.join(description_parts).strip('"\'')  # Remove quotes if present
            else:
                ...
                if '=' in arg:
                    ...
                    try:
                        if key == 'max_tokens':
                            ...
                            supports_tools = value.lower() in ['true', 'yes', '1']
                        elif key == 'supports_multimodal':
                            supports_multimodal = value.lower() in ['true', 'yes', '1']
                        elif key == 'is_free':
            ...
            self._show_success(f"Added custom model '{model_name}' for provider '{provider}'")
    ...
    async def _handle_remove_model_command(self, command: Command) -> None:
        ...
        try:
            ...
            success = self.engine.config_manager.remove_custom_model(model_name, provider)
            
            if success:
                self._show_success(f"Removed custom model '{model_name}' from provider '{provider}'")
            else:
                self._show_error(f"Custom model '{model_name}' for provider '{provider}' not found")
    ...
    async def _handle_list_custom_models_command(self, command: Command) -> None:
        ...
        try:
            ...
            table = Table(title="Custom Models", show_header=True, header_style="bold blue")
...
def main() -> None:
    ...
    @click.option('--no-approval', is_flag=True, help='Skip approval prompts and auto-approve all operations (DANGEROUS)')
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['--help', '-h']:
        click.echo("""Omnimancer - A unified CLI for multiple AI language models.
