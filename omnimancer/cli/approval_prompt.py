...
class ApprovalDecision:
    ...
    def is_approved(self) -> bool:
        ...
        return self.decision in [ApprovalDecisionType.APPROVED, ApprovalDecisionType.APPROVED_AND_REMEMBER, ApprovalDecisionType.REVIEW_AND_EDIT]
    
    @property
    def should_remember(self) -> bool:
        """Check if this decision should be remembered."""
        return self.decision == ApprovalDecisionType.APPROVED_AND_REMEMBER or self.remember
...
class CLIApprovalPrompt:
    ...
    def __init__(self, 
                 ...
                 default_timeout_seconds: int = 300):  # 5 minutes default
        """
    ...
    async def prompt_for_file_modification_approval(self,
    ...
    async def prompt_for_approval(self,
        ...
        try:
            ...
            if decision.is_approved:
                ...
                confirmation = self.formatter.format_approval_denied(
                    approval_request,
                    self._get_decision_reason(decision)
                )
            ...
            logger.info(f"Approval cancelled after {response_time:.2f} seconds: {approval_request.operation_type}")
            ...
            # Log timeout details
            logger.warning(f"Approval timeout after {timeout}s: {approval_request.operation_type}")
    ...
    async def prompt_for_batch_approval(self,
                                      batch_request: BatchApprovalRequest,
                                      timeout_seconds: Optional[int] = None) -> BatchApprovalDecision:
        """
    ...
    async def _get_user_decision_with_timeout(self, timeout_seconds: int) -> ApprovalDecision:
        """Get user decision with timeout handling."""
        try:
            # Use asyncio.wait_for for timeout
            response = await asyncio.wait_for(
                self._get_user_input_async("Enter your decision (y=approve/r=remember/e=edit/n=reject/q=quit): "),
            ...
            if decision_type is None:
                ...
                self.console.print("  [blue]r[/blue] = Remember (approve and auto-approve similar)")
                self.console.print("  [cyan]e[/cyan] = Edit (review and modify content)")
                self.console.print("  [red]n[/red] = No (reject)")
                self.console.print("  [yellow]q[/yellow] = Quit (cancel and exit to omnimancer prompt)")
    ...
    async def _get_batch_decision_with_timeout(self, 
                                             batch_request: BatchApprovalRequest,
                                             timeout_seconds: int) -> BatchApprovalDecision:
        """Get batch decision with timeout handling."""
        try:
            response = await asyncio.wait_for(
                self._get_user_input_async("Enter batch decision (all/none/select/individual/q): "),
                timeout=timeout_seconds
            )
            ...
            if response_lower in ['all', 'approve-all', 'yes-all']:
                ...
                return BatchApprovalDecision(
                    decision_type="deny_all"
                )
                
            elif response_lower in ['select', 'selective', 's']:
                return await self._handle_selective_approval(batch_request, timeout_seconds)
                
            elif response_lower in ['individual', 'one-by-one', 'i']:
                return await self._handle_individual_approval(batch_request, timeout_seconds)
                ...
                self.console.print("  [yellow]individual[/yellow] = Review each operation separately")
                self.console.print("  [yellow]q[/yellow] = Cancel batch")
                
                remaining_timeout = max(timeout_seconds - 15, 30)
                return await self._get_batch_decision_with_timeout(batch_request, remaining_timeout)
                
        except asyncio.TimeoutError:
            raise
    
    async def _handle_selective_approval(self, 
                                       batch_request: BatchApprovalRequest,
                                       timeout_seconds: int) -> BatchApprovalDecision:
        ...
        self.console.print("Enter numbers separated by spaces or commas (e.g., '1 3 5' or '1,3,5')")
        self.console.print("Or enter ranges (e.g., '1-3 5 7-9')")
        
        try:
            response = await asyncio.wait_for(
                self._get_user_input_async(f"Select operations (1-{operation_count}): "),
                timeout=timeout_seconds
            )
            ...
            if selected_indices is None:
                ...
                return await self._handle_selective_approval(batch_request, timeout_seconds - 10)
            
            return BatchApprovalDecision(
                decision_type="selective",
                approved_indices=selected_indices
            )
            
        except asyncio.TimeoutError:
            raise
    
    async def _handle_individual_approval(self, 
                                        batch_request: BatchApprovalRequest,
                                        timeout_seconds: int) -> BatchApprovalDecision:
        ...
        for i, (operation, preview) in enumerate(zip(batch_request.operations, batch_request.previews)):
    ...
    def _parse_operation_selection(self, response: str, max_operations: int) -> Optional[List[int]]:
    ...
    async def _confirm_remember_decision(self, approval_request: ApprovalRequest) -> bool:
        """Confirm user wants to remember this approval decision."""
        self.console.print()
        self.console.print("[blue]🧠 Remember Decision Confirmation[/blue]")
        self.console.print(f"You chose to remember this approval for similar '{approval_request.operation_type}' operations.")
        self.console.print("This means similar operations will be automatically approved without asking.")
        self.console.print()
        
        try:
            response = await asyncio.wait_for(
                self._get_user_input_async("Are you sure you want to remember this? (y/n): "),
            ...
            self.console.print("[yellow]Confirmation timed out - not remembering decision[/yellow]")
            return False
    ...
    def _display_batch_confirmation(self, 
                                  batch_decision: BatchApprovalDecision,
                                  batch_request: BatchApprovalRequest):
        """Display confirmation of batch decision."""
        if batch_decision.decision_type == "approve_all":
            self.console.print(f"[green]✅ Approved all {len(batch_request.operations)} operations[/green]")
            
        elif batch_decision.decision_type == "deny_all":
            self.console.print(f"[red]❌ Denied all {len(batch_request.operations)} operations[/red]")
            ...
            self.console.print(f"[blue]✅ Approved {approved_count} of {total_count} operations[/blue]")
            
            # Show which operations were approved
            if approved_count > 0:
                approved_nums = [str(i + 1) for i in batch_decision.approved_indices]
                self.console.print(f"   Approved operations: {', '.join(approved_nums)}")
                
        elif batch_decision.decision_type == "individual":
            approved_count = len(batch_decision.approved_indices)
            total_count = len(batch_request.operations)
            self.console.print(f"[blue]✅ Individual review complete: {approved_count} of {total_count} approved[/blue]")
            
        elif batch_decision.decision_type in ["cancelled", "timeout"]:
            self.console.print("[yellow]❌ Batch approval cancelled[/yellow]")
    
    async def _handle_cancellation_cleanup(self, approval_request: ApprovalRequest, reason: str):
        """
        ...
        try:
            ...
            logger.debug(f"Performing cancellation cleanup for {approval_request.operation_type} - Reason: {reason}")
            ...
            cancellation_data = {
                ...
                'risk_level': approval_request.risk_level.value if approval_request.risk_level else 'unknown'
            }
    ...
    def _display_file_modification_preview(self, review_data: Dict[str, Any], current_model: str):
    ...
    def _display_modification_header(self, review_data: Dict[str, Any], current_model: str):
        ...
        header_text = Text()
        header_text.append(f"{current_model} - File Modification Review\n", style="bold cyan")
    ...
    def _display_new_file_preview(self, review_data: Dict[str, Any]):
        ...
        try:
            syntax = Syntax(new_content[:2000], file_path.suffix.lstrip('.') or "text", 
            ...
            preview_content = new_content[:2000] + "..." if len(new_content) > 2000 else new_content
            preview_panel = Panel(
                preview_content,
                title="[bold green]New File Content (Preview)[/bold green]",
                border_style="green"
    ...
    def _display_current_content_summary(self, current_content: str):
        ...
        if total_lines <= 20:
            ...
            preview_lines = lines[:10] + [f"... ({total_lines - 20} lines omitted) ..."] + lines[-10:]
    ...
    def _display_side_by_side_preview(self, current_content: str, new_content: str):
        ...
        current_preview = current_content[:1000] + "..." if len(current_content) > 1000 else current_content
    ...
    def _display_content_stats(self, content: str, is_new: bool = False):
        ...
        title = "[bold blue]New File Statistics[/bold blue]" if is_new else "[bold blue]Content Statistics[/bold blue]"
    ...
    async def _get_file_modification_decision_with_timeout(self, review_data: Dict[str, Any], timeout_seconds: int) -> Dict[str, Any]:
        ...
        options_text.append("Remember (approve and auto-approve similar)\n", style="blue")
        ...
        options_text.append("Quit (cancel and exit to omnimancer prompt)\n", style="yellow")
        ...
        try:
            ...
            if response_lower in ['y', 'yes', 'approve', '1']:
                ...
                remaining_timeout = max(timeout_seconds - 10, 30)
                return await self._get_file_modification_decision_with_timeout(review_data, remaining_timeout)
    ...
    async def _handle_edit_content(self, review_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if edit_choice_lower in ["1", "inline"]:
                ...
                return await self._get_file_modification_decision_with_timeout(review_data, 300)
                
        except asyncio.TimeoutError:
            self.console.print("[yellow]Edit choice timed out - going back to approval options[/yellow]")
            return await self._get_file_modification_decision_with_timeout(review_data, 300)
    ...
    async def _inline_content_edit(self, review_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.console.print("Enter new content (type END_CONTENT on a new line to finish):")
        ...
        try:
            ...
            self.console.print("[yellow]Confirmation timed out - using modified content[/yellow]")
            return {
                'approved': True,
                'modified_content': modified_content,
    ...
    async def _external_editor_edit(self, review_data: Dict[str, Any]) -> Dict[str, Any]:
        """Handle external editor content editing."""
        self.console.print("[yellow]External editor functionality not yet implemented[/yellow]")
...
async def prompt_single_approval(approval_request: ApprovalRequest,
                               preview: Optional[ChangePreview] = None,
                               operation_data: Optional[Dict[str, Any]] = None,
                               console: Optional[Console] = None,
                               timeout_seconds: int = 300) -> ApprovalDecision:
    """
    ...
    Returns:
        ApprovalDecision with user's choice
    """
    prompt_handler = CLIApprovalPrompt(console=console, default_timeout_seconds=timeout_seconds)
...
async def prompt_batch_approval(batch_request: BatchApprovalRequest,
                              console: Optional[Console] = None,
                              timeout_seconds: int = 600) -> BatchApprovalDecision:
    """
    ...
    """
    prompt_handler = CLIApprovalPrompt(console=console, default_timeout_seconds=timeout_seconds)
    return await prompt_handler.prompt_for_batch_approval(batch_request, timeout_seconds)