...
@performance.command()
@click.option(
    "--start-monitoring", is_flag=True, help="Start performance monitoring services"
)
@click.option(
    "--format",
    "output_format",
...
@click.option(
    "--view",
    type=click.Choice(
        ["overview", "tokens", "performance", "resources", "alerts", "suggestions"]
    ),
    default="overview",
...
def _display_status_table(console: Console, status_info: Dict[str, Any]) -> None:
    ...
    table.add_row(
        "Optimization Engine", optimization_status, "Alert and suggestion generation"
    )
...
def _display_token_usage_table(
    ...
    table.add_row(
        "Total Tokens", f"{total_tokens:,}", f"{total_tokens/(hours*60):.0f}/min"
    )
    table.add_row(
        "Total Requests", f"{total_requests:,}", f"{total_requests/(hours*60):.1f}/min"
    )
    table.add_row(
...
def _display_analysis_results(console: Console, results: Dict[str, Any]) -> None:
    ...
    table.add_row(
        "Peak Usage Time", str(usage_patterns.get("peak_usage_time", "Unknown"))
    )
