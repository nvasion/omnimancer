...
from ...core.agent.performance_monitor import (
    PerformanceMonitor, get_performance_monitor, initialize_performance_monitoring
)
from ...core.agent.token_tracker import TokenUsageTracker, get_token_tracker
from ...core.agent.metrics_collector import (
    PerformanceMetricsCollector, get_metrics_collector, initialize_metrics_collection
)
from ...core.agent.optimization_engine import (
    OptimizationEngine, get_optimization_engine, initialize_optimization_engine,
    OptimizationCategory, AlertLevel
)
...
@performance.command()
@click.option('--start-monitoring', is_flag=True, help='Start performance monitoring services')
...
def status(ctx, start_monitoring, output_format):
    ...
    try:
        ...
        status_info = {
            "monitoring_active": performance_monitor.is_monitoring if hasattr(performance_monitor, 'is_monitoring') else False,
            "metrics_collecting": metrics_collector.is_collecting if hasattr(metrics_collector, 'is_collecting') else False,
            "optimization_running": optimization_engine.is_running if hasattr(optimization_engine, 'is_running') else False,
...
@performance.command()
@click.option('--view', type=click.Choice(['overview', 'tokens', 'performance', 'resources', 'alerts', 'suggestions']),
              default='overview', help='Dashboard view to display')
...
@performance.command()
@click.option('--category', type=click.Choice(['cost_reduction', 'performance_improvement', 
              'resource_efficiency', 'reliability_enhancement', 'user_experience']),
...
def suggestions(ctx, category, output_format):
    ...
    try:
        ...
        if category:
            ...
            current_suggestions = optimization_engine.get_current_suggestions(category_enum)
        else:
...
def export(ctx, output, output_format, days):
    ...
    try:
        ...
        # Export metrics
        metrics_collector = get_metrics_collector()
        metrics_path = metrics_collector.export_metrics(time_window, f"metrics_{output_path.stem}.json")
...
def _display_status_table(console: Console, status_info: Dict[str, Any]) -> None:
    ...
    # Monitoring status
    monitoring_status = "🟢 Active" if status_info.get("monitoring_active") else "🔴 Inactive"
    table.add_row("Performance Monitor", monitoring_status, "Real-time monitoring")
    
    # Metrics collection
    metrics_status = "🟢 Active" if status_info.get("metrics_collecting") else "🔴 Inactive"
    table.add_row("Metrics Collection", metrics_status, "System metrics gathering")
    
    # Optimization engine
    optimization_status = "🟢 Running" if status_info.get("optimization_running") else "🔴 Stopped"
    table.add_row("Optimization Engine", optimization_status, "Alert and suggestion generation")
...
def _display_token_usage_table(console: Console, summary: Dict[str, Any], hours: int) -> None:
    ...
    table.add_row("Total Tokens", f"{total_tokens:,}", f"{total_tokens/(hours*60):.0f}/min")
    table.add_row("Total Requests", f"{total_requests:,}", f"{total_requests/(hours*60):.1f}/min")
    table.add_row("Total Cost", f"${total_cost:.4f}", f"${total_cost/(hours*60):.5f}/min")
...
def _display_suggestions_table(console: Console, 
                             optimization_suggestions: List[Any],
                             token_suggestions: List[Dict[str, Any]]) -> None:
    ...
    for suggestion in optimization_suggestions:
        table.add_row(
            suggestion.optimization_type.value if hasattr(suggestion, 'optimization_type') else "General",
            suggestion.title if hasattr(suggestion, 'title') else "N/A",
            suggestion.potential_improvement if hasattr(suggestion, 'potential_improvement') else "Unknown",
            suggestion.implementation_difficulty if hasattr(suggestion, 'implementation_difficulty') else "Unknown"
        )
...
def _display_alerts_table(console: Console, alerts: List[Any]) -> None:
    ...
    for alert in alerts:
        ...
        table.add_row(
            ...
            alert.timestamp.strftime("%H:%M:%S") if hasattr(alert, 'timestamp') else "Unknown"
        )
...
def _display_analysis_results(console: Console, results: Dict[str, Any]) -> None:
    ...
    # Summary table
    table = Table(title=f"Performance Analysis ({results.get('analysis_period_days', 7)} days)")
    table.add_column("Metric", style="cyan")
    ...
    table.add_row("Avg Tokens/Request", f"{usage_patterns.get('average_tokens_per_request', 0):.0f}")
    table.add_row("Peak Usage Time", str(usage_patterns.get('peak_usage_time', 'Unknown')))
