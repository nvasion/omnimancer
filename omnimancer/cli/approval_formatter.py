...
class CLIApprovalFormatter:
    ...
    def format_approval_dialog(self, 
                             approval_request: ApprovalRequest,
                             preview: Optional[ChangePreview] = None,
                             operation_data: Optional[Dict[str, Any]] = None) -> Group:
        """
        ...
        # Operation details
        components.append(self._format_operation_details(approval_request, operation_data))
    ...
    def format_batch_approval_dialog(self, batch_request: BatchApprovalRequest) -> Group:
        """
        ...
        components.append(Rule(style="dim"))
        components.append(self._format_batch_approval_options(len(batch_request.operations)))
        
        return Group(*components)
    
    def _format_header(self, 
                      approval_request: ApprovalRequest, 
                      preview: Optional[ChangePreview]) -> Panel:
        ...
        risk_text = Text(f"{risk_icon} Risk Level: {approval_request.risk_level.value.upper()}", 
                        style=risk_color)
    ...
    def _format_operation_details(self, 
                                 approval_request: ApprovalRequest,
                                 operation_data: Optional[Dict[str, Any]]) -> Panel:
        ...
        table.add_row("Request Time", approval_request.requested_at.strftime("%H:%M:%S"))
    ...
    def _format_diff_display(self, preview: ChangePreview) -> Panel:
        ...
        if len(diff_text) > 2000:
            ...
            if len(lines) > 50:
                truncated_lines = lines[:25] + ['...', '# Diff truncated - too long to display', '...'] + lines[-25:]
    ...
    def _format_impact_assessment(self, preview: ChangePreview) -> Panel:
        ...
        # File system impact
        if preview.change_type in [ChangeType.FILE_CREATE, ChangeType.FILE_MODIFY, ChangeType.FILE_DELETE]:
            if 'path' in preview.metadata:
                path = preview.metadata['path']
                impact_items.append(f"• File system: {path}")
        
        if preview.change_type in [ChangeType.DIRECTORY_CREATE, ChangeType.DIRECTORY_DELETE]:
            if 'path' in preview.metadata:
                path = preview.metadata['path']
                item_count = preview.metadata.get('item_count', 0)
    ...
    def _format_time_info(self, approval_request: ApprovalRequest) -> Panel:
        ...
        if approval_request.expires_at:
            ...
            if time_remaining and time_remaining.total_seconds() > 0:
                ...
                time_content.append(Text(f"Expires: {expires_time} ({minutes_left}m remaining)", style="yellow"))
            else:
                time_content.append(Text(f"Expires: {expires_time} (EXPIRED)", style="red"))
    ...
    def _format_approval_options(self) -> Panel:
        ...
        options_text = [
            ...
            Text(),
            Text("• [bold green]y[/bold green] or [bold green]yes[/bold green] - Approve this operation", style="green"),
            Text("• [bold blue]r[/bold blue] or [bold blue]remember[/bold blue] - Approve and remember for similar operations", style="blue"), 
            Text("• [bold red]n[/bold red] or [bold red]no[/bold red] - Deny this operation", style="red"),
            Text("• [bold yellow]q[/bold yellow] or [bold yellow]quit[/bold yellow] - Cancel and exit", style="yellow"),
            Text(),
    ...
    def _format_batch_header(self, batch_request: BatchApprovalRequest) -> Panel:
        ...
        header_content = Group(
            ...
            Text(f"Multiple operations require approval ({summary['total_operations']} operations)", style="dim white"),
            Text(),
            Text(f"🔢 Operations: {summary['total_operations']}", style="blue"),
            Text(f"⏰ Created: {batch_request.created_at.strftime('%H:%M:%S')}", style="white")
        )
    ...
    def _format_single_batch_operation(self, 
                                     approval_request: ApprovalRequest,
                                     preview: ChangePreview,
                                     operation_number: int) -> Group:
        """Format a single operation within a batch."""
        components = []
        
        # Operation header
        icon = self.change_type_icons.get(preview.change_type, "❓")
        components.append(Text(f"{icon} {approval_request.description}", style="bold white"))
    ...
    def _format_batch_approval_options(self, operation_count: int) -> Panel:
        ...
        options_text = [
            ...
            Text(),
            Text("• [bold green]all[/bold green] - Approve all operations", style="green"),
            Text("• [bold red]none[/bold red] - Deny all operations", style="red"),
            Text(f"• [bold blue]select[/bold blue] - Choose specific operations (1-{operation_count})", style="blue"),
            Text("• [bold yellow]individual[/bold yellow] - Review each operation separately", style="yellow"),
            Text("• [bold yellow]q[/bold yellow] or [bold yellow]quit[/bold yellow] - Cancel batch", style="yellow"),
            Text(),
            Text("Press Ctrl+C to cancel at any time.", style="dim")
        ]
    ...
    def format_approval_denied(self, approval_request: ApprovalRequest, reason: str = "") -> Panel:
    ...
    def format_approval_approved(self, approval_request: ApprovalRequest, remember: bool = False) -> Panel:
        ...
        if remember:
            content.extend([
                Text(),
                Text("🧠 Similar operations will be auto-approved in the future", style="blue")
            ])
