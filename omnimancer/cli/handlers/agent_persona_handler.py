...
from ...core.agent.persona import (
    PersonaManager, AgentPersona, PersonaStatus, PersonaCategory,
    get_persona_manager
)
from ...core.agent.agent_switcher import (
    AgentSwitcher, SessionState, SwitchState,
    get_agent_switcher
)
...
class AgentPersonaHandler:
    ...
    async def handle_list_personas(self) -> None:
        ...
        for persona in sorted_personas:
            ...
            table.add_row(
                ...
                status,
                persona.description[:50] + "..." if len(persona.description) > 50 else persona.description
            )
        ...
        self.console.print("[dim]Use '/agent info <persona_id>' for detailed information[/dim]")
    ...
    async def handle_use_persona(self, persona_id: str) -> None:
        ...
        success, message = self.agent_switcher.switch_persona(
            persona_id.lower(),
            reason="User requested switch via CLI"
        )
        ...
        if success:
            ...
            if persona.capabilities:
                caps_text = ", ".join([cap.value.replace("_", " ").title() 
    ...
    async def handle_current_persona(self) -> None:
        ...
        if not active_persona:
            ...
            self.console.print("[dim]Use '/agent use <persona_id>' to activate a persona[/dim]")
            return
        ...
        if active_persona.metadata:
            info_lines.append("")
            info_lines.append(f"[dim]Usage Count: {active_persona.metadata.usage_count}[/dim]")
            if active_persona.metadata.last_used:
                info_lines.append(f"[dim]Last Used: {active_persona.metadata.last_used.strftime('%Y-%m-%d %H:%M')}[/dim]")
    ...
    async def handle_persona_info(self, persona_id: str) -> None:
        ...
        if persona.configuration:
            ...
            if hasattr(persona.configuration, 'fallback_providers') and persona.configuration.fallback_providers:
                fallback_list = persona.configuration.fallback_providers
                if isinstance(fallback_list, (list, tuple)):
                    config_info.append(f"[yellow]Fallback Providers:[/yellow] {', '.join(fallback_list)}")
                else:
                    config_info.append(f"[yellow]Fallback Providers:[/yellow] {str(fallback_list)}")
        ...
        if persona.metadata:
            ...
            if persona.metadata.created_at:
                meta_info.append(f"[yellow]Created:[/yellow] {persona.metadata.created_at.strftime('%Y-%m-%d %H:%M')}")
            
            if persona.metadata.last_used:
                meta_info.append(f"[yellow]Last Used:[/yellow] {persona.metadata.last_used.strftime('%Y-%m-%d %H:%M')}")
            
            if persona.metadata.tags:
                meta_info.append(f"[yellow]Tags:[/yellow] {', '.join(persona.metadata.tags)}")
    ...
    async def handle_persona_status(self) -> None:
        ...
        if active_persona:
            status_lines.append(f"  {active_persona.icon} {active_persona.name} ({active_persona.id})")
            status_lines.append(f"  Status: {self._format_status(active_persona.status)}")
        else:
        ...
        if session_state:
            status_lines.append(f"  Session ID: {session_state.session_id or '[dim]None[/dim]'}")
            status_lines.append(f"  History Items: {len(session_state.conversation_history)}")
            status_lines.append(f"  Active Tools: {len(session_state.active_tools)}")
            status_lines.append(f"  Active Operations: {len(session_state.active_operations)}")
            if session_state.last_modified:
                status_lines.append(f"  Last Modified: {session_state.last_modified.strftime('%H:%M:%S')}")
        else:
    ...
    async def handle_switch_history(self) -> None:
        ...
        for idx, context in enumerate(history[-10:], 1):  # Show last 10 switches
            from_name = context.from_persona.name if context.from_persona else "[dim]None[/dim]"
            ...
            reason = context.switch_reason[:30] + "..." if len(context.switch_reason) > 30 else context.switch_reason
...
"""
        ...
        if not user_query:
            ...
            for task_desc, persona_id in recommendations:
                ...
                if persona:
                    ...
                    self.console.print(f"  → Use: [cyan]/agent use {persona_id}[/cyan] ({persona.name})")
                    self.console.print("")
            ...
            if any(word in query_lower for word in ["code", "program", "debug", "develop", "build", "fix"]):
                if "coding" in personas:
                    recommendations.append((personas["coding"], "Optimized for programming and development tasks"))
            
            if any(word in query_lower for word in ["research", "find", "search", "learn", "study", "investigate"]):
                if "research" in personas:
                    recommendations.append((personas["research"], "Enhanced with web search capabilities"))
            
            if any(word in query_lower for word in ["write", "creative", "story", "content", "blog", "article"]):
                if "creative" in personas:
                    recommendations.append((personas["creative"], "Specialized for creative writing tasks"))
            
            if any(word in query_lower for word in ["fast", "quick", "speed", "urgent", "simple"]):
                if "performance" in personas:
                    recommendations.append((personas["performance"], "Optimized for speed and efficiency"))
            
            # If no specific matches, recommend general
            if not recommendations and "general" in personas:
                recommendations.append((personas["general"], "Balanced for general-purpose tasks"))
            
            if recommendations:
                for persona, reason in recommendations[:3]:  # Show top 3
                    self.console.print(f"[green]✓[/green] [bold]{persona.icon} {persona.name}[/bold]")
                    self.console.print(f"  {reason}")
                    self.console.print(f"  Use: [cyan]/agent use {persona.id}[/cyan]")
                    self.console.print("")
            else:
                self.console.print("[dim]No specific recommendations found. Try /agent discover for interactive selection.[/dim]")
        
        # Show additional help
        self.console.print("[dim]💡 Tip: Use '/agent compare' to see capability differences[/dim]")
        self.console.print("[dim]💡 Tip: Use '/agent preview <persona>' to test without switching[/dim]")
    ...
    async def handle_compare_personas(self) -> None:
        ...
        for persona in sorted_personas:
            ...
            web_search = "✅" if (config and getattr(config, 'web_search_enabled', False)) else "❌"
            file_ops = "✅" if (config and getattr(config, 'file_operations_enabled', False)) else "❌"
            
            # Determine speed based on persona type
            speed_rating = "⚡" if persona.id == "performance" else "🐢" if persona.id == "research" else "⚖️"
    ...
    async def handle_preview_persona(self, persona_id: str) -> None:
        ...
        self.console.print(f"[dim]This is how '{persona.name}' would behave if activated[/dim]\n")
        ...
        if persona.capabilities:
            caps_text = ", ".join([cap.value.replace("_", " ").title() for cap in persona.capabilities])
            caps_panel = Panel(
                caps_text,
                title="Capabilities",
                border_style="green"
        ...
        if persona.id in behavior_predictions:
            ...
            behavior_lines.append("• Behavior will depend on configured template and settings")
        ...
        for category, persona_list in categories.items():
            ...
            for persona in sorted(persona_list, key=lambda p: p.name):
                ...
                if persona.metadata and persona.metadata.usage_count > 0:
                    card_lines.append(f"[dim]Used {persona.metadata.usage_count} times[/dim]")
                ...
                persona_panel = Panel(
                    "\n".join(card_lines),
                    title=f"{persona.id}",
                    border_style="blue" if persona == self.persona_manager.active_persona else "dim",
                    width=50
                )
