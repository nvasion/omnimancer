...
class AgentCLIHandler:
    ...
    def __init__(self, agent_engine: Optional[AgentEngine] = None, 
        ...
        try:
            ...
            try:
                ...
                if self.verbose:
                    self.console.print(f"[yellow]Warning: PersonaManager initialization failed: {e}[/yellow]")
            ...
            if self.verbose:
                self.console.print("[dim]Agent CLI Handler initialized successfully[/dim]")
                
        except Exception as e:
            logger.error(f"Failed to initialize AgentCLIHandler: {e}")
            if console:
                console.print(f"[red]Error: Failed to initialize agent system: {e}[/red]")
    ...
    async def handle_command(self, args: List[str]) -> None:
        ...
        try:
            if self.verbose:
                self.console.print(f"[dim]Executing agents subcommand: {subcommand}[/dim]")
    ...
    async def handle_list(self, args: List[str]) -> None:
        ...
        try:
            ...
            for agent_id, persona in personas.items():
                ...
                provider_name = config.provider_type.value if config.provider_type else "Unknown"
                
                table.add_row(
                    agent_id,
                    persona.name,
                    persona.description[:37] + "..." if len(persona.description) > 40 else persona.description,
                    provider_name,
    ...
    async def handle_enable(self, args: List[str]) -> None:
        ...
        try:
            if self.verbose:
                self.console.print(f"[dim]Checking if agent '{agent_id}' exists...[/dim]")
            ...
            if not persona:
                ...
                suggestions = [agent for agent in available_agents if agent_id in agent or agent in agent_id]
                ...
                raise AgentNotFoundError(f"Agent '{agent_id}' not found. {suggestion_text}")
            ...
            if self.approval_manager:
                try:
                    ...
                    if not await self._check_permission_for_action("agent_enable", agent_id):
                        raise PermissionDeniedError(f"You don't have permission to enable agent '{agent_id}'")
                except Exception as perm_e:
                    if self.verbose:
                        self.console.print(f"[dim]Permission check failed: {perm_e}[/dim]")
                    # Continue if permission system is not available
            
            # Show progress indicator for activation
            with Status(f"[bold blue]Activating {persona.name}...", console=self.console):
                if self.verbose:
                    self.console.print(f"[dim]Calling PersonaManager.activate_persona('{agent_id}')...[/dim]")
                ...
                if not success:
                    raise ConfigurationError(f"Failed to activate persona '{agent_id}'. Check system configuration.")
            ...
            config = persona.configuration
            capabilities_list = [cap.value.replace('_', ' ').title() for cap in persona.capabilities]
            ...
            logger.error(f"Unexpected error enabling agent '{agent_id}': {e}", exc_info=True)
    ...
    async def handle_disable(self, args: List[str]) -> None:
        ...
        try:
            ...
            if args:
                ...
                if self.verbose:
                    self.console.print("[dim]Looking for currently active persona...[/dim]")
            ...
            if self.approval_manager:
                try:
                    if not await self._check_permission_for_action("agent_disable", agent_id):
                        raise PermissionDeniedError(f"You don't have permission to disable agent '{agent_id}'")
                except Exception as perm_e:
                    if self.verbose:
                        self.console.print(f"[dim]Permission check failed: {perm_e}[/dim]")
                    # Continue if permission system is not available
            ...
            if active_operations > 0:
                ...
                if not Confirm.ask(f"Disable the active agent '{persona.name}'?", default=True):
                    self._show_info("Agent remains enabled.")
                    return
            
            # Show progress indicator for deactivation
            with Status(f"[bold red]Deactivating {persona.name}...", console=self.console):
                if self.verbose:
                    self.console.print(f"[dim]Calling PersonaManager.deactivate_persona()...[/dim]")
                ...
                if not success:
                    raise ConfigurationError(f"Failed to deactivate persona '{agent_id}'. Check system state.")
    ...
    async def handle_status(self, args: List[str]) -> None:
        ...
        try:
            if args:
                ...
                status_color, status_text = status_mapping.get(persona.status, ("gray", "Unknown"))
                config = persona.configuration
                capabilities_list = [cap.value.replace('_', ' ').title() for cap in persona.capabilities]
    ...
    async def handle_config(self, args: List[str]) -> None:
        ...
        """
        if not args:
            self._show_error("Config command requires an agent ID. Use '/agents list' to see available agents.")
            return
        ...
        try:
            ...
            config = persona.configuration
            capabilities_list = [cap.value.replace('_', ' ').title() for cap in persona.capabilities]
            status_text = "Active" if persona.is_active else "Available" if persona.status == PersonaStatus.AVAILABLE else persona.status.value.title()
    ...
    async def handle_switch(self, args: List[str]) -> None:
        ...
        """
        if not args:
            self._show_error("Switch command requires an agent ID. Use '/agents list' to see available agents.")
            return
    ...
    async def handle_current(self) -> None:
        ...
        if active_persona:
            config = active_persona.configuration
            self.console.print(f"[bold green]Current active agent:[/bold green] {active_persona.name} ({active_persona.id})")
            self.console.print(f"[bold]Provider:[/bold] {config.provider_type.value if config.provider_type else 'Unknown'}")
    ...
    async def handle_info(self, args: List[str]) -> None:
        ...
        """
        if not args:
            self._show_error("Info command requires an agent ID. Use '/agents list' to see available agents.")
            return
...
[cyan]/agents info research[/cyan]           - Get details about the research agent"""
        ...
        self.console.print(Panel(help_text, title="🤖 Agent Management Help", border_style="blue"))
    ...
    async def _check_permission_for_action(self, action: str, target: str) -> bool:
        ...
        try:
            ...
            if self.verbose:
                self.console.print(f"[dim]Checking permission for action '{action}' on target '{target}'[/dim]")
