...
class PermissionsHandler:
    ...
    async def handle_view_permissions(self, args: List[str] = None) -> None:
        ...
        try:
            ...
            if not category_filter or category_filter == "all":
                self._display_permission_rules(security_settings.get("rules", []), category_filter)
    ...
    async def handle_add_permission_rule(self, args: List[str]) -> None:
        ...
        try:
            ...
            if rule_type not in valid_types:
                self._show_error(f"Invalid rule type. Valid types: {', '.join(valid_types)}")
                return
            
            if permission_level not in valid_levels:
                self._show_error(f"Invalid permission level. Valid levels: {', '.join(valid_levels)}")
                return
            ...
            if success:
                self._show_success(f"Added permission rule: {pattern} ({rule_type}, {permission_level})")
            else:
        ...
        try:
            if action == "stats":
                ...
                if not export_path:
                    ...
                    self._show_info("Usage: /permissions audit export <file_path> [--format=json|csv] [--limit=N]")
                    return
                
                success = self.export_audit_trail(export_path, export_format, limit)
                if success:
                    self._show_success(f"Audit trail exported to {export_path} ({export_format} format)")
                else:
                ...
                if len(audit_entries) > 5:
                    approved_count = len([e for e in audit_entries if e.get('decision') == 'approved'])
                    denied_count = len([e for e in audit_entries if e.get('decision') == 'denied'])
                    avg_risk = sum(e.get('risk_level', 0) for e in audit_entries) / len(audit_entries)
                    
                    self._show_info(f"Summary: {approved_count} approved, {denied_count} denied, avg risk: {avg_risk:.1f}")
                    self._show_info("Use '/permissions audit stats' for detailed statistics")
    ...
    async def handle_clear_learned_permissions(self, args: List[str]) -> None:
        ...
        if not confirm:
            self._show_warning("This will clear learned permissions. Use --confirm to proceed.")
            return
        
        try:
            count = self._clear_learned_permissions(pattern)
            if pattern:
                self._show_success(f"Cleared {count} learned permissions matching '{pattern}'")
            else:
    ...
    async def handle_import_permissions(self, args: List[str]) -> None:
        ...
        try:
            ...
            if success:
                ...
                self._show_success(f"Permissions imported and {action} current configuration")
            else:
    ...
    def _get_current_security_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.permission_controller:
                ...
                settings["learned"] = self.permission_controller.get_learned_permissions()
            ...
            return {
                ...
                "rules": [
                    {"pattern": "*.py", "type": "file", "level": "read", "id": "1"},
                    {"pattern": "/tmp/*", "type": "file", "level": "write", "id": "2"},
                    {"pattern": "git", "type": "command", "level": "execute", "id": "3"},
                ],
                "learned": [
                    {"pattern": "test_*.py", "decision": "approved", "count": 5, "last_used": "2024-01-15T10:30:00"},
                    {"pattern": "npm install", "decision": "approved", "count": 3, "last_used": "2024-01-14T14:15:00"},
                ]
            }
    
    def _display_permissions_overview(self, settings: Dict[str, Any], verbose: bool = False) -> None:
        ...
        level_text = Text(level.replace("_", " ").title(), style=f"bold {level_colors.get(level, 'white')}")
    ...
    def _display_permission_rules(self, rules: List[Dict], category_filter: Optional[str] = None) -> None:
    ...
    def _display_learned_permissions(self, learned: List[Dict], verbose: bool = False) -> None:
    ...
    def _set_security_level(self, level: str) -> bool:
        ...
        try:
            ...
            if level == "auto_approve":
                success = self.security_manager.update_security_policy('require_approval_for_high_risk', False)
            elif level == "ask_always":
                success = self.security_manager.update_security_policy('require_approval_for_high_risk', True)
                # Also set a policy to disable auto approval if it doesn't exist
                if 'enable_auto_approval' not in self.security_manager.security_policies:
                    self.security_manager.security_policies['enable_auto_approval'] = False
                else:
                    success = success and self.security_manager.update_security_policy('enable_auto_approval', False)
            elif level == "ask_but_remember":
                success = self.security_manager.update_security_policy('require_approval_for_high_risk', True)
                # Set enable auto approval policy
                if 'enable_auto_approval' not in self.security_manager.security_policies:
                    self.security_manager.security_policies['enable_auto_approval'] = True
                else:
                    success = success and self.security_manager.update_security_policy('enable_auto_approval', True)
            else:
                return False
    ...
    def _add_permission_rule(self, pattern: str, rule_type: str, level: str) -> bool:
        ...
        try:
            ...
            if rule_type == "file":
                ...
                if self.security_manager:
                    if level == "allowed":
                        allowed_domains = self.security_manager.security_policies.get('allowed_network_domains', [])
                        if pattern not in allowed_domains:
                            allowed_domains.append(pattern)
                        self.security_manager.update_security_policy('allowed_network_domains', allowed_domains)
                    else:
                        blocked_domains = self.security_manager.security_policies.get('blocked_network_domains', [])
                        if pattern not in blocked_domains:
                            blocked_domains.append(pattern)
                        self.security_manager.update_security_policy('blocked_network_domains', blocked_domains)
            else:
                return False
    ...
    def _get_audit_entries(self, limit: int, filter_type: Optional[str], date_filter: Optional[str]) -> List[Dict]:
        ...
        try:
            ...
            from ...core.security.audit_logger import AuditEventType, AuditLevel
            ...
            for event in recent_events:
                ...
                if event.event_type in [AuditEventType.PERMISSION_CHECK, AuditEventType.COMMAND_EXECUTED, 
                                       AuditEventType.FILE_ACCESS, AuditEventType.APPROVAL_GRANTED]:
                    decision = "approved"
                elif event.event_type in [AuditEventType.PERMISSION_DENIED, AuditEventType.COMMAND_BLOCKED,
                                         AuditEventType.FILE_ACCESS_DENIED, AuditEventType.APPROVAL_DENIED]:
                    decision = "denied"  
                else:
                    decision = "info"
    ...
    def export_audit_trail(self, file_path: str, format_type: str = "json", limit: int = 1000) -> bool:
    ...
    def _export_audit_csv(self, file_path: str, entries: List[Dict]) -> bool:
        ...
        try:
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['timestamp', 'operation', 'decision', 'user', 'risk_level', 
                             'event_type', 'level', 'session_id', 'operation_id']
    ...
    def get_audit_statistics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_entries = self._get_audit_entries(500, None, None)  # Analyze last 500 entries
            ...
            approved_count = len([e for e in recent_entries if e.get('decision') == 'approved'])
            denied_count = len([e for e in recent_entries if e.get('decision') == 'denied'])
            
            approval_rate = (approved_count / total_entries) * 100 if total_entries > 0 else 0
            denial_rate = (denied_count / total_entries) * 100 if total_entries > 0 else 0
            ...
            return {
                ...
                "time_period": "Last 500 entries" if total_entries >= 500 else f"All {total_entries} entries"
            }
        ...
        try:
            ...
            if stats['common_operations']:
                ...
                for op_type, count in stats['common_operations']:
                    ...
                    stats_lines.append(f"  • {op_type}: {count} times ({percentage:.1f}%)")
                stats_lines.append("")
            ...
            if stats['risk_distribution']:
                ...
                for category, count in stats['risk_distribution'].items():
                    ...
                    if "Critical" in category:
                        ...
                        color_code = "[green]"
                    stats_lines.append(f"  • {color_code}{category}[/]: {count} entries ({percentage:.1f}%)")
    ...
    def _set_learning_enabled(self, enabled: bool) -> bool:
        ...
        try:
            ...
            success = self.security_manager.update_security_policy('enable_permission_learning', enabled)
            
            # Also set it directly in security policies if it doesn't exist
            if 'enable_permission_learning' not in self.security_manager.security_policies:
                self.security_manager.security_policies['enable_permission_learning'] = enabled
                success = True
    ...
    def _show_learning_status(self) -> None:
        ...
        try:
            ...
            if self.security_manager:
                learning_enabled = self.security_manager.security_policies.get('enable_permission_learning', False)
        ...
        try:
            ...
            if self.permission_controller:
                ...
                if stored_approvals:
                    ...
                    for op_type, count in sorted(pattern_counts.items(), key=lambda x: x[1], reverse=True)[:3]:
                        most_common.append(f"  • {op_type.replace('_', ' ').title()} operations ({count} patterns)")
            
            # Calculate accuracy (mock calculation)
            accuracy = 85 + min(15, unique_patterns * 2)  # Higher patterns = higher accuracy
        ...
        try:
            ...
            if pattern:
                ...
                for signature, approval_data in stored_approvals.items():
                    ...
                    if (pattern in signature or 
                        pattern in approval_data.get('operation_type', '')):
    ...
    def _export_permissions(self, file_path: str, include_learned: bool) -> bool:
        ...
        try:
            ...
            if self.security_manager:
                export_data["security_policies"] = self.security_manager.security_policies.copy()
    ...
    def _import_permissions(self, file_path: str, merge: bool) -> bool:
        ...
        try:
                        ...
                        try:
                            ...
                            loop.run_until_complete(
                                self.permission_controller.grant_operation_permission(
                                    ...
                                    expires_hours=24 * 365  # 1 year for imported permissions
                                )
                            )
                            loop.close()
                        except Exception:
            ...
            if "security_policies" in import_data and self.security_manager:
                for policy_name, policy_value in import_data["security_policies"].items():
                    self.security_manager.update_security_policy(policy_name, policy_value)
