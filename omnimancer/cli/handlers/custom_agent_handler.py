...
"""
        ...
        self.console.print(Panel(help_text, title="🤖 Custom Agent Commands", expand=False))
    ...
    async def _handle_create_agent(self, args: List[str]):
        ...
        if "--quick" in args:
            interactive = False
            self.console.print("[yellow]Quick creation not yet implemented. Using interactive mode.[/yellow]")
...
def create_custom_agent_handler(console: Optional[Console] = None) -> CustomAgentHandler:
    """Create and initialize a custom agent handler."""
    return CustomAgentHandler(console=console)


async def handle_agent_command(command: Command, console: Optional[Console] = None) -> None:
