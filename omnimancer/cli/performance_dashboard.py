...
from ..core.agent.performance_monitor import (
    ...
    PerformanceSnapshot, PerformanceAlert, OptimizationSuggestion
)
from ..core.agent.token_tracker import TokenUsageTracker, get_token_tracker, UsagePattern
from ..core.agent.metrics_collector import (
    PerformanceMetricsCollector, get_metrics_collector,
    AggregationMethod, MetricCategory
)
...
class PerformanceDashboard:
    ...
    def show_overview(self) -> None:
        ...
        try:
            ...
            # Create layout
            layout = self._create_overview_layout(dashboard_data, metrics_data, token_summary)
            self.console.print(layout)
            
            # Show recent alerts
            recent_alerts = dashboard_data.get("active_alerts", [])[-3:]  # Last 3 alerts
            if recent_alerts:
                self.console.print(Rule("Recent Alerts"))
                self._display_alerts_summary(recent_alerts)
            
            # Show suggestions
            suggestions = dashboard_data.get("suggestions", [])[-2:]  # Last 2 suggestions
            if suggestions:
    ...
    def show_detailed_view(self, view_name: str = "overview") -> None:
        ...
        if view_name not in self.available_views:
            ...
            self.console.print(f"Available views: {', '.join(self.available_views.keys())}")
            return
    ...
    def start_live_dashboard(self, view: str = "overview") -> None:
        ...
        try:
            with Live(
                ...
                self.console.print("[yellow]Live dashboard started. Press Ctrl+C to exit.[/yellow]")
    ...
    def _generate_live_layout(self) -> Layout:
        ...
        if self.current_view == "overview":
            ...
            metrics_data = self.metrics_collector.get_dashboard_metrics()
            token_summary = self.token_tracker.get_usage_summary(timedelta(hours=1))
            return self._create_overview_layout(dashboard_data, metrics_data, token_summary)
        else:
            # For other views, just render statically
            layout = Layout()
            layout.split_column(
                Layout(self._render_header(), size=3),
                Layout(Panel(self.available_views[self.current_view](), title=f"{self.current_view.title()} View"))
            )
            return layout
    
    def _create_overview_layout(self, dashboard_data: Dict, metrics_data: Dict, token_summary: Dict) -> Layout:
    ...
    def _render_header(self) -> Panel:
        ...
        subtitle_text = Text(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", style="dim")
        header_content = Align.center(f"{title_text}\n{subtitle_text}")
        
        return Panel(header_content, style="blue", box=box.ROUNDED)
    
    def _create_performance_panel(self, dashboard_data: Dict, metrics_data: Dict) -> Panel:
        ...
        response_status = self._get_status_indicator(response_time, 1000, 5000)  # Good < 1s, Warning < 5s
        ...
        success_status = self._get_status_indicator(success_rate, 95, 90, reverse=True)  # Good > 95%
        table.add_row("Success Rate", f"{success_rate:.1f}%", success_status)
        
        # Active operations
        active_ops = current_snapshot.get("active_operations", 0)
        ops_status = self._get_status_indicator(active_ops, 5, 15)  # Good < 5, Warning < 15
    ...
    def _create_resources_panel(self, metrics_data: Dict) -> Panel:
        ...
        cpu_status = self._get_status_indicator(cpu_percent, 50, 80)  # Good < 50%, Warning < 80%
        table.add_row("CPU Usage", f"{cpu_percent:.1f}%", cpu_status)
        
        # Memory usage
        memory_mb = resources.get("memory_mb", 0)
        memory_gb = memory_mb / 1024
        memory_status = self._get_status_indicator(memory_mb, 512, 1024)  # Good < 512MB, Warning < 1GB
    ...
    def _get_status_indicator(self, value: float, good_threshold: float, warning_threshold: float, reverse: bool = False) -> str:
    ...
    def _render_token_usage(self) -> None:
        ...
        table.add_row("Total Tokens", f"{summary_1h.get('total_tokens', 0):,}", f"{summary_24h.get('total_tokens', 0):,}")
        table.add_row("Total Requests", f"{summary_1h.get('total_requests', 0):,}", f"{summary_24h.get('total_requests', 0):,}")
        table.add_row("Total Cost", f"${summary_1h.get('total_cost', 0):.3f}", f"${summary_24h.get('total_cost', 0):.3f}")
        ...
        if summary_24h.get("provider_breakdown"):
            provider_table = Table(title="Provider Breakdown (24h)", show_header=True, header_style="bold magenta")
    ...
    def _render_performance_metrics(self) -> None:
        ...
        perf_table.add_row("Average Response Time", f"{response_time:.0f} ms", self._get_status_indicator(response_time, 1000, 5000))
        perf_table.add_row("Success Rate", f"{success_rate:.1f}%", self._get_status_indicator(success_rate, 95, 90, reverse=True))
    ...
    def _render_resource_usage(self) -> None:
        ...
        resource_table = Table(title="Current Resource Usage", show_header=True, header_style="bold magenta")
        ...
        resource_table.add_row("CPU", f"{cpu_percent:.1f}%", self._get_status_indicator(cpu_percent, 50, 80))
        resource_table.add_row("Memory", f"{memory_mb:.0f} MB", self._get_status_indicator(memory_mb, 512, 1024))
    ...
    def _display_alerts_summary(self, alerts: List[Dict]) -> None:
        ...
        for alert in alerts:
            ...
            self.console.print(f"[{level_color}]● {alert.get('title', 'Unknown Alert')}[/{level_color}]")
    
    def _display_suggestions_summary(self, suggestions: List[Dict]) -> None:
        """Display suggestions summary."""
        for suggestion in suggestions:
            self.console.print(f"[yellow]💡 {suggestion.get('title', 'Optimization Available')}[/yellow]")
    ...
    def export_report(self, output_path: Optional[Path] = None) -> Path:
        ...
        report_data = {
            ...
            "dashboard_data": self.performance_monitor.get_performance_dashboard_data(),
            "metrics_data": self.metrics_collector.get_dashboard_metrics(),
            "token_summary_1h": self.token_tracker.get_usage_summary(timedelta(hours=1)),
            "token_summary_24h": self.token_tracker.get_usage_summary(timedelta(hours=24)),
        ...
        self.console.print(f"[green]Performance report exported to {output_path}[/green]")
        return output_path
