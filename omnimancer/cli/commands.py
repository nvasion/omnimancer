...
def parse_command(user_input: str) -> Command:
    ...
    if len(sanitized_input) > MAX_MESSAGE_LENGTH:
        raise ValueError(f"Message too long. Maximum length is {MAX_MESSAGE_LENGTH} characters.")
...
def _validate_command_args(command: SlashCommand, args: List[str]) -> List[str]:
    ...
    if command == SlashCommand.SWITCH:
        if len(args) < 1:
            raise ValueError("Switch command requires at least one argument: provider name")
        if len(args) > 2:
            raise ValueError("Switch command accepts at most two arguments: provider and model")
        ...
        if len(args) > 2:
            raise ValueError("MCP command accepts at most two arguments: action and server name")
        
        if args:
            action = args[0].lower()
            valid_actions = ["status", "reload", "connect", "disconnect", "health"]
            if action not in valid_actions:
                raise ValueError(f"Invalid MCP action. Valid actions: {', '.join(valid_actions)}")
    
    elif command in [SlashCommand.HELP, SlashCommand.CLEAR, SlashCommand.QUIT, SlashCommand.STATUS]:
        if args:
        ...
        if len(args) > 2:
            raise ValueError("Models command accepts at most two arguments: filter type and filter value")
        
        if args:
            filter_type = args[0].lower()
            valid_filters = ["provider", "capability", "price", "performance", "free", "latest"]
            if filter_type not in valid_filters:
                raise ValueError(f"Invalid filter type. Valid filters: {', '.join(valid_filters)}")
            ...
            if len(args) == 2:
                if filter_type == "capability":
                    ...
                    if args[1].lower() not in valid_capabilities:
                        raise ValueError(f"Invalid capability. Valid capabilities: {', '.join(valid_capabilities)}")
        ...
        if len(args) > 2:
            raise ValueError("Validate command accepts at most two arguments: provider and --fix flag")
        
        if args:
            # First arg should be provider name or --fix
            if args[0] not in ["--fix", "--auto-fix"] and not args[0].replace('_', '').replace('-', '').isalnum():
        ...
        if args:
            ...
            if not args[0].startswith('--') and not args[0].replace('_', '').replace('-', '').isalnum():
                raise ValueError("Invalid provider name format")
        ...
        if len(args) > 2:
            raise ValueError("Diagnose command accepts at most two arguments: provider and --detailed flag")
        ...
        if args:
            ...
            if action not in valid_actions:
                raise ValueError(f"Invalid history action. Valid actions: {', '.join(valid_actions)}")
    
    elif command == SlashCommand.ADD_MODEL:
        # /add-model <name> <provider> [description] [other_params...]
        if len(args) < 2:
            raise ValueError("Add-model command requires at least two arguments: model name and provider")
        if len(args) > 10:
        ...
        if len(args) != 2:
            raise ValueError("Remove-model command requires exactly two arguments: model name and provider")
        ...
        if len(args) > 3:
            raise ValueError("Agents command accepts at most three arguments: action, agent_name, and options")
        
        if args:
            action = args[0].lower()
            valid_actions = ["list", "enable", "disable", "status", "config", "switch", "current", "info"]
            if action not in valid_actions:
                raise ValueError(f"Invalid agents action. Valid actions: {', '.join(valid_actions)}")
            
            # Some actions require an agent name
            if action in ["enable", "disable", "config", "switch", "info"] and len(args) < 2:
                raise ValueError(f"Action '{action}' requires an agent name")
    
    elif command == SlashCommand.APPROVALS:
        # /approvals [action] [signature]
        if len(args) > 2:
            raise ValueError("Approvals command accepts at most two arguments: action and signature")
        
        if args:
            action = args[0].lower()
            valid_actions = ["list", "revoke", "clear", "stats", "cleanup"]
            if action not in valid_actions:
                raise ValueError(f"Invalid approvals action. Valid actions: {', '.join(valid_actions)}")
        ...
        if args:
            action = args[0].lower()
            valid_actions = ["view", "set-level", "add-rule", "remove-rule", "audit", "learn", "clear-learned", "export", "import"]
            if action not in valid_actions:
                raise ValueError(f"Invalid permissions action. Valid actions: {', '.join(valid_actions)}")
            
            # Actions that require additional arguments
            if action == "set-level" and len(args) < 2:
                raise ValueError("Set-level action requires a security level (auto_approve, ask_always, ask_but_remember)")
            elif action in ["add-rule", "remove-rule"] and len(args) < 2:
                raise ValueError(f"{action.capitalize()} action requires at least one additional argument")
            ...
            if action == "set-level" and len(args) >= 2:
                ...
                if level not in valid_levels:
                    raise ValueError(f"Invalid security level. Valid levels: {', '.join(valid_levels)}")
    
    # Commands that don't accept arguments
    elif command in [SlashCommand.CLEAR, 
                     SlashCommand.QUIT, SlashCommand.STATUS, SlashCommand.PROVIDERS, 
                     SlashCommand.TOOLS, SlashCommand.SETUP]:
        if args:
