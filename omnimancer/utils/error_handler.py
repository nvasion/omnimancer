...
from .errors import (
    OmnimancerError, ProviderError, AuthenticationError, RateLimitError,
    NetworkError, ModelNotFoundError, ProviderUnavailableError,
    ToolExecutionError, QuotaExceededError, ProviderConfigurationError,
    MCPError, MCPServerError, MCPConnectionError, MCPTimeoutError
)
...
class ErrorHandler:
        ...
        if isinstance(error, AuthenticationError):
            ...
            if available_providers:
                ...
                if working_providers:
                    suggestions.append(f"Try switching to: {', '.join(working_providers)}")
            ...
            if available_providers:
                working_providers = self._get_working_providers(available_providers)
                if working_providers:
                    suggestions.append(f"Use alternative provider: {', '.join(working_providers)}")
            ...
            if hasattr(error, 'get_suggestions'):
                ...
                if model_suggestions:
                    suggestions.append(f"Try these models: {', '.join(model_suggestions)}")
            ...
            if hasattr(error, 'fallback_providers') and error.fallback_providers:
                suggestions.append(f"Use fallback: {', '.join(error.fallback_providers)}")
            ...
            if available_providers:
                working_providers = self._get_working_providers(available_providers)
                if working_providers:
                    suggestions.append(f"Try alternative: {', '.join(working_providers)}")
    ...
    def handle_mcp_error(
        ...
        if isinstance(error, MCPConnectionError):
            ...
            if server_name:
                suggestions.append(f"Consider temporarily disabling '{server_name}' server")
    ...
    def should_retry_operation(
        ...
        elif isinstance(error, (AuthenticationError, QuotaExceededError, ModelNotFoundError)):
    ...
    def _get_working_providers(self, available_providers: List[str]) -> List[str]:
        ...
        for provider in available_providers:
            ...
            if (not status.get('last_error') or
                (status.get('last_success') and 
                 status.get('last_success') > status.get('last_error')) or
                status.get('status') in ['network_error', 'quota_limited']):
...
def handle_provider_error(error: Exception, provider_name: str, available_providers: List[str] = None):
    """Convenience function for handling provider errors."""
    return error_handler.handle_provider_error(error, provider_name, available_providers)


def handle_mcp_error(error: Exception, server_name: str = None, available_servers: List[str] = None):
...
def get_graceful_degradation_options(failed_provider: str, available_providers: List[str], required_capabilities: List[str] = None):
    """Convenience function for graceful degradation."""
    return error_handler.get_graceful_degradation_options(failed_provider, available_providers, required_capabilities)