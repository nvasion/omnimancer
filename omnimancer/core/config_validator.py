...
class ConfigValidator:
    ...
    def validate_config(self, config: Config, changes: Optional[Dict[str, Any]] = None) -> List[str]:
        """
    ...
    def _validate_config(self, config) -> List[str]:
        ...
        if hasattr(config, '_mock_name') or str(type(config)).find('Mock') != -1:
            ...
            if hasattr(config, 'default_provider'):
                ...
                elif hasattr(config, 'providers') and config.default_provider not in config.providers:
                    errors.append(f"Default provider '{config.default_provider}' is not configured")
        ...
        if not config.default_provider or config.default_provider == "":
            errors.append("No default provider configured")
        elif config.default_provider and config.default_provider not in config.providers:
            errors.append(f"Default provider '{config.default_provider}' is not configured")
        ...
        for provider_name, provider_config in config.providers.items():
            provider_errors = self.validate_provider_config(provider_name, provider_config)
            errors.extend(provider_errors)
        ...
        try:
            ...
            if not storage_path.parent.exists():
                errors.append(f"Storage path parent directory does not exist: {storage_path.parent}")
    ...
    def validate_provider_config(self, provider_name: str, provider_config) -> List[str]:
        """
        ...
        if hasattr(provider_config, '_mock_name') or str(type(provider_config)).find('Mock') != -1:
    ...
    def validate_mcp_config(self, mcp_config: MCPConfig) -> List[str]:
        ...
        try:
            # Handle mock objects
            if hasattr(mcp_config, '_mock_name') or str(type(mcp_config)).find('Mock') != -1:
                ...
                if hasattr(mcp_config, 'servers'):
                    for server_name, server_config in mcp_config.servers.items():
                        if hasattr(server_config, 'name') and (not server_config.name or server_config.name == ""):
                            errors.append(f"MCP server '{server_name}' has no name")
                        if hasattr(server_config, 'command') and (not server_config.command or server_config.command == ""):
                ...
                if hasattr(mcp_config, 'auto_approve_timeout'):
                    ...
                    if isinstance(timeout_val, (int, float)) and timeout_val <= 0:
                        errors.append(f"Invalid MCP auto_approve_timeout: {timeout_val}")
                ...
                if hasattr(mcp_config, 'max_concurrent_servers'):
                    ...
                    if isinstance(servers_val, (int, float)) and servers_val <= 0:
                        errors.append(f"Invalid MCP max_concurrent_servers: {servers_val}")
            ...
            if hasattr(mcp_config, 'servers'):
                for server_name, server_config in mcp_config.servers.items():
                    ...
                    try:
                        if hasattr(server_config, 'timeout') and server_config.timeout <= 0:
    ...
    def _validate_perplexity_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Perplexity model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_xai_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown xAI model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_mistral_config(self, config: ProviderConfig) -> List[str]:
        ...
        valid_model_patterns = ["mistral-small", "mistral-medium", "mistral-large", "mistral-7b", "mistral-8x7b"]
        model_valid = any(pattern in config.model for pattern in valid_model_patterns)
        if not model_valid:
            errors.append(f"Unknown Mistral model '{config.model}'. Valid model patterns: {', '.join(valid_model_patterns)}")
    ...
    def _validate_vertex_config(self, config: ProviderConfig) -> List[str]:
        ...
        if not config.api_key and not config.vertex_credentials_path:
            errors.append("Vertex AI provider requires either an API key or credentials path")
    ...
    def _validate_claude_code_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.claude_code_mode and config.claude_code_mode not in valid_modes:
            errors.append(f"Unknown Claude-code mode '{config.claude_code_mode}'. Valid modes: {', '.join(valid_modes)}")
    ...
    def _validate_chat_settings(self, chat_settings) -> List[str]:
        ...
        if hasattr(chat_settings, '_mock_name') or str(type(chat_settings)).find('Mock') != -1:
            return errors
        
        try:
            if hasattr(chat_settings, 'temperature') and chat_settings.temperature is not None:
                if not (0 <= chat_settings.temperature <= 2):
                    errors.append("Temperature must be between 0 and 2")
            
            if hasattr(chat_settings, 'max_tokens') and chat_settings.max_tokens is not None:
...
class ValidationResult:
    ...
    def __init__(self, is_valid: bool, errors: List[str], warnings: List[str] = None, config=None):
