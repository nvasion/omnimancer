...
class ConfigSecurityValidator:
    ...
    def validate_security(self, config: Dict[str, Any], strict_mode: bool = True) -> Tuple[bool, List[str], List[str]]:
        """
    ...
    def _validate_recommended_settings(self, config: Dict[str, Any]) -> List[str]:
        ...
        for setting_path, recommended_value in self.recommended_security_settings.items():
            current_value = self._get_nested_value(config, setting_path)
    ...
    def _validate_dangerous_combinations(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            for provider_name, provider_config in config["providers"].items():
                ...
                if api_key and isinstance(api_key, str):
                    ...
                    if api_key in ["test", "demo", "placeholder", "your-api-key-here"]:
                        errors.append(f"Provider '{provider_name}' has placeholder API key")
                    
                    # Check if it looks like a real key but isn't encrypted
                    if (api_key.startswith(("sk-", "api-", "key-")) and 
                        not api_key.startswith("encrypted:")):
    ...
    def fix_security_issues(self, config: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
        """
        ...
        for setting_path, required_value in self.critical_security_settings.items():
            ...
            if current_value != required_value:
                ...
                changes.append(f"Fixed critical setting: {setting_path} = {required_value}")
        ...
        for setting_path, recommended_value in self.recommended_security_settings.items():
            ...
            if current_value is None:  # Only set if not explicitly configured
                ...
                changes.append(f"Applied recommended setting: {setting_path} = {recommended_value}")
    ...
    def generate_security_report(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "overall_status": "SECURE" if is_valid and len(warnings) == 0 else 
                             "INSECURE" if not is_valid else "NEEDS_ATTENTION",
        ...
        max_issues = len(self.critical_security_settings) + len(self.recommended_security_settings)
...
def validate_config_security(config: Dict[str, Any], strict_mode: bool = True) -> Tuple[bool, List[str], List[str]]:
    """
...
def fix_config_security_issues(config: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """
