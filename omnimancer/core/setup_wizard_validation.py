...
class SetupWizardValidation:
    ...
    def __init__(self, console: Console, provider_info: Dict[str, Dict[str, Any]], 
                 provider_factory, cancellation_handler, signal_handler):
        """
    ...
    async def test_configuration(self, provider_name: str, provider_config: ProviderConfig) -> bool:
        """
        ...
        with Progress(
            ...
            if not self._validate_api_key_format(provider_name, provider_config.api_key):
                progress.update(task, description="❌ Invalid API key format!")
                await asyncio.sleep(0.5)
                self.console.print("[red]❌ API key format validation failed![/red]")
                self.ui.show_troubleshooting_guidance(provider_name, "Invalid API key format")
            ...
            try:
                ...
                if is_valid:
                    ...
                    if provider_name == "bedrock" and hasattr(provider, 'validate_model_access'):
                        model_test = await provider.validate_model_access()
                        
                        if model_test['success']:
                            progress.update(task, description="✅ Model accessible!")
                            await asyncio.sleep(0.5)
                            self.console.print("[green]✅ All configuration tests passed![/green]")
                            return True
                        else:
                            ...
                            self.console.print(f"[yellow]⚠️  Model Access Warning: {model_test['message']}[/yellow]")
                            if 'suggestion' in model_test:
                                self.console.print(f"[dim]💡 Suggestion: {model_test['suggestion']}[/dim]")
                            
                            self.console.print("\n[yellow]Note: Configuration will be saved, but you may need to:[/yellow]")
                            self.console.print("  [yellow]• Request model access in AWS Console[/yellow]")
                            self.console.print("  [yellow]• Check account permissions[/yellow]")
                            self.console.print("  [yellow]• Try a different model[/yellow]")
                            
                            return Confirm.ask("Continue with this configuration?", default=True)
                    else:
                        # Standard model availability check for other providers
                        model_available = await self._check_model_availability(provider, provider_config.model)
                        ...
                        if model_available:
                            ...
                            progress.update(task, description="⚠️  Model may not be available!")
                            await asyncio.sleep(0.5)
                            self.console.print("[yellow]⚠️  Model availability could not be confirmed[/yellow]")
                    ...
                    self.ui.show_troubleshooting_guidance(provider_name, "Authentication failed")
                    return Confirm.ask("Continue with this configuration anyway?")
    ...
    def _validate_api_key_format(self, provider_name: str, api_key: Optional[str]) -> bool:
        """
    ...
    async def validate_existing_configuration(self, provider_name: str, config_manager) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not provider_config:
                result["errors"].append(f"No configuration found for provider '{provider_name}'")
                return result
            
            # Validate API key format
            if not self._validate_api_key_format(provider_name, provider_config.api_key):
                result["errors"].append("Invalid API key format")
                result["suggestions"].append(f"Check API key format for {self.provider_info[provider_name]['name']}")
            ...
            try:
                ...
                if not credentials_valid:
                    ...
                    if not model_available:
                        result["warnings"].append(f"Model '{provider_config.model}' may not be available")
                        result["suggestions"].append("Consider updating to a newer model")
            
            except Exception as e:
                result["errors"].append(f"Provider initialization failed: {str(e)}")
                result["suggestions"].append("Check provider configuration and network connectivity")
