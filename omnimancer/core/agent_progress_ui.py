...
from .agent_mode_manager import AgentModeManager, AgentOperation, AgentOperationStatus, AgentMode
...
class AgentProgressUI:
    ...
    def __init__(self, agent_manager: AgentModeManager, console: Optional[Console] = None):
        """
...
Batch Approval: {'Enabled' if settings['enable_batch_approval'] else 'Disabled'}"""
    ...
    def show_approval_queue_panel(self) -> Panel:
        ...
        """
        approval_ops = [
            op for op in self.agent_manager.operation_queue 
        ...
        if len(approval_ops) > 5:
            approval_text.append(Text(f"... and {len(approval_ops) - 5} more", style="dim"))
    ...
    def update_operation_progress(self, operation_id: str, progress: float, status_text: str = ""):
        """
    ...
    def _on_operation_change(self, operation: AgentOperation):
        ...
        elif operation.status in [AgentOperationStatus.COMPLETED, AgentOperationStatus.FAILED, AgentOperationStatus.CANCELLED]:
            # Update final progress
            if operation.id in self.operation_progress:
                op_progress = self.operation_progress[operation.id]
                op_progress.progress = 100.0 if operation.status == AgentOperationStatus.COMPLETED else 0.0
                op_progress.status_text = operation.status.value.replace('_', ' ').title()
