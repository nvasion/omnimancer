...
class SetupWizardProviderSetup:
    ...
    def select_provider(self) -> Optional[str]:
        ...
        while True:
            try:
                ...
                if Confirm.ask(f"Configure {self.provider_info[selected_provider]['name']}?"):
    ...
    async def configure_provider(self, provider_name: str) -> Optional[ProviderConfig]:
        ...
        api_key = None
        if provider_name not in ["ollama", "claude-code"]:  # Ollama and Claude Code don't need API key
    ...
    def _get_api_key(self, provider_name: str, info: Dict[str, Any]) -> Optional[str]:
        ...
        while True:
            ...
            if info["api_key_prefix"] and not api_key.startswith(info["api_key_prefix"]):
                self.console.print(f"[yellow]⚠️  API key should start with '{info['api_key_prefix']}'[/yellow]")
    ...
    def _select_model(self, provider_name: str, info: Dict[str, Any]) -> Optional[str]:
        ...
        if choice == "2":
            ...
            if provider_name == "bedrock":
                ...
                self.console.print("  • Model ID: [dim]anthropic.claude-opus-4-20250514-v1:0[/dim]")
                self.console.print("  • Inference Profile: [dim]us.anthropic.claude-opus-4-20250514-v1:0[/dim]")
                self.console.print("  • Foundation ARN: [dim]arn:aws:bedrock:us-west-2::foundation-model/meta.llama4-maverick-17b-instruct-v1:0[/dim]")
                self.console.print("  • Inference Profile ARN: [dim]arn:aws:bedrock:us-west-2::inference-profile/us.meta.llama4-maverick-17b-instruct-v1:0[/dim]")
                ...
                self.console.print("  • [dim]my-custom-model[/dim] (your deployment name)")
            ...
            if custom_model:
                ...
                self.console.print("[yellow]No model entered, falling back to common models...[/yellow]")
        ...
        if provider_name == "ollama":
            ...
            while True:
                region_choice = Prompt.ask(
                    f"Select AWS region [1-{len(regions)}]", 
                    default="1"
                )
                try:
    ...
    def save_configuration(self, provider_name: str, provider_config: ProviderConfig, config_manager) -> None:
        """
        ...
        try:
            ...
            if is_first_provider:
                ...
                self.console.print(f"[green]✅ {self.provider_info[provider_name]['name']} set as default provider![/green]")
            else:
                # Ask if user wants to set this as the default provider
                if Confirm.ask(f"Set {self.provider_info[provider_name]['name']} as your default provider?", default=False):
                    config_manager.set_default_provider(provider_name)
                    self.console.print(f"[green]✅ {self.provider_info[provider_name]['name']} set as default provider![/green]")
                else:
                    self.console.print(f"[green]✅ {self.provider_info[provider_name]['name']} added to your providers![/green]")
