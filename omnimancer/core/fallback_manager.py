...
from ..utils.errors import ProviderError, ProviderUnavailableError, NetworkError
...
class EnhancedProviderFallback:
    ...
    def set_fallback_providers(self, providers: List[str], rankings: Optional[Dict[str, ProviderRank]] = None):
        """
    ...
    async def execute_with_fallback(self, operation_func: Callable, 
                                  *args, preserve_context: bool = True, **kwargs) -> Any:
        """
        ...
        original_provider_name = original_provider.get_provider_name() if original_provider else None
    ...
    async def _execute_with_retries(self, operation_func: Callable, provider_name: str, 
                                  *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retry_attempts):
            try:
                ...
                if attempt < self.max_retry_attempts - 1:
                    ...
                    logger.debug(f"Retrying {provider_name} in {delay:.2f}s (attempt {attempt + 1})")
        ...
        if last_error:
            ...
            raise ProviderError(f"All retry attempts failed for provider {provider_name}")
    ...
    async def _get_ranked_providers(self) -> List[str]:
        ...
        for provider in self.fallback_providers:
            if provider not in available_providers and provider not in self.excluded_providers:
    ...
    def _record_failure(self, provider_name: str, reason: FallbackReason, error_msg: str):
    ...
    def _exclude_provider_temporarily(self, provider_name: str):
        ...
        self.exclusion_expiry[provider_name] = time.time() + self.circuit_breaker_recovery_time
        
        logger.warning(f"Temporarily excluded provider {provider_name} due to consecutive failures")
    ...
    def _is_provider_excluded(self, provider_name: str) -> bool:
        ...
        if provider_name in self.exclusion_expiry:
            if time.time() > self.exclusion_expiry[provider_name]:
                ...
                logger.info(f"Re-enabled provider {provider_name} after recovery period")
                return False
        ...
        try:
            if hasattr(self.core_engine, 'chat_manager'):
                ...
                self.preserved_context = {
                    'messages': context.messages.copy() if hasattr(context, 'messages') else [],
    ...
    def get_fallback_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        recent_history = self.fallback_history[-limit:] if limit else self.fallback_history
    ...
    def configure_circuit_breaker(self, threshold: int = 5, recovery_time: int = 600):
        ...
        logger.info(f"Circuit breaker configured: threshold={threshold}, recovery_time={recovery_time}s")