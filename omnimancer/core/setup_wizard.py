...
class SetupWizard(SetupWizardCore):
    ...
    def __init__(self, config_manager: ConfigManager, provider_registry: ProviderRegistry):
        ...
        self.guide_generator = SetupWizardGuideGenerator(self.console, self.provider_info)
    ...
    async def start_wizard(self) -> bool:
        ...
        try:
            ...
            provider_config = await self.provider_setup.configure_provider(provider_name)
            if not provider_config:
                return False
            
            # Test the configuration
            if not await self.validation.test_configuration(provider_name, provider_config):
                return False
    ...
    async def validate_existing_configuration(self, provider_name: str) -> Dict[str, Any]:
        """
        ...
        return await validation.validate_existing_configuration(provider_name, self.config_manager)
    
    def create_setup_guide(self, provider_name: str, output_path: Optional[str] = None) -> str:
        """
    ...
    def _save_configuration(self, provider_name: str, provider_config: ProviderConfig) -> None:
        """
        ...
        """
        return self.provider_setup.save_configuration(provider_name, provider_config, self.config_manager)
    
    async def _test_configuration(self, provider_name: str, provider_config: ProviderConfig) -> bool:
        """
    ...
    async def _validate_provider_credentials(self, provider_name: str, config: Dict[str, Any]) -> bool:
    ...
    def _generate_model_table(self, provider_name: str) -> str:
        ...
        return self.ui._generate_model_table(provider_name) if hasattr(self.ui, '_generate_model_table') else ""
