...
from ..utils.errors import MCPError, MCPToolError, MCPConnectionError
...
class EnhancedMCPIntegrator:
    ...
    def __init__(self, mcp_manager: Optional[MCPManager] = None):
        ...
        self.execution_queue: Dict[ExecutionPriority, List[Dict[str, Any]]] = defaultdict(list)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"MCP integrator initialized with {len(self.discovered_tools)} tools")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize MCP integrator: {e}")
            return False
    
    async def discover_tools(self, force_refresh: bool = False) -> Dict[str, ToolDefinition]:
        """
        ...
        try:
            ...
            logger.info(f"Discovered {len(self.discovered_tools)} tools across "
    ...
    def _analyze_tool_capabilities(self, tool: ToolDefinition) -> Set[ToolCapability]:
        ...
        if any(keyword in text_lower for keyword in [
            'file', 'read', 'write', 'directory', 'folder', 'path', 'save', 'load'
        ]):
            capabilities.add(ToolCapability.FILE_OPERATIONS)
        
        # Web requests
        if any(keyword in text_lower for keyword in [
            'http', 'web', 'request', 'api', 'url', 'fetch', 'download', 'upload'
        ]):
            capabilities.add(ToolCapability.WEB_REQUESTS)
        
        # Data processing
        if any(keyword in text_lower for keyword in [
            'data', 'process', 'parse', 'transform', 'convert', 'analyze', 'filter'
        ]):
            capabilities.add(ToolCapability.DATA_PROCESSING)
        
        # System info
        if any(keyword in text_lower for keyword in [
            'system', 'info', 'status', 'monitor', 'health', 'performance'
        ]):
            capabilities.add(ToolCapability.SYSTEM_INFO)
        
        # Code execution
        if any(keyword in text_lower for keyword in [
            'execute', 'run', 'code', 'script', 'command', 'shell', 'bash'
        ]):
        ...
        # API integration
        if any(keyword in text_lower for keyword in [
            'api', 'integration', 'service', 'endpoint', 'rest', 'graphql'
        ]):
        ...
        # Communication
        if any(keyword in text_lower for keyword in [
            'email', 'message', 'notification', 'send', 'communicate'
        ]):
            capabilities.add(ToolCapability.COMMUNICATION)
        
        # Multimedia
        if any(keyword in text_lower for keyword in [
            'image', 'video', 'audio', 'media', 'picture', 'photo'
        ]):
    ...
    async def execute_tool_with_context(
        ...
        for attempt in range(context.retry_attempts):
            ...
            try:
                ...
                self._record_execution_metrics(tool_name, server_name, True, execution_time)
        ...
        self._record_execution_metrics(tool_name, server_name, False, total_execution_time, last_error)
    ...
    def _get_cached_result(self, tool_name: str, arguments: Dict[str, Any]) -> Optional[ToolExecutionResult]:
    ...
    def _cache_result(self, tool_name: str, arguments: Dict[str, Any], result: ToolExecutionResult):
    ...
    def find_best_tool_for_task(self, task_description: str, capability: Optional[ToolCapability] = None) -> Optional[str]:
        """
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            'mcp_manager_initialized': bool(self.mcp_manager and self.mcp_manager.initialized),
        ...
        if problematic_tools:
            ...
            if len(problematic_tools) > len(self.discovered_tools) * 0.3:  # More than 30% problematic
                health_status['integrator_healthy'] = False
