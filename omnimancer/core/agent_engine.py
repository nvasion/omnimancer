...
class ProgramExecutor(BaseManager):
    ...
    def __init__(self, approval_workflow: Optional[ApprovalWorkflow] = None):
        super().__init__()
        from .agent.program_executor import EnhancedProgramExecutor, ExecutionConfig, ExecutionMode
        ...
        self.allowed_commands = {
            ...
            'pwd', 'which', 'whereis', 'uname',
            'curl', 'wget', 'ping'
        }
        self.forbidden_commands = {
            'rm', 'rmdir', 'sudo', 'su', 'passwd', 'chpasswd',
            'systemctl', 'service', 'mount', 'umount', 'fdisk',
            'dd', 'mkfs', 'fsck', 'crontab', 'at', 'batch',
            'nc', 'netcat', 'ncat', 'socat', 'telnet', 'ssh',
            'scp', 'rsync', 'wget', 'curl'  # Note: curl/wget in both for complex validation
        }
    ...
    async def execute_operation(self, operation: Operation) -> OperationResult:
        ...
        config = ExecutionConfig(
            timeout_seconds=operation.data.get('timeout', self.default_config.timeout_seconds),
            max_memory_mb=operation.data.get('max_memory_mb', self.default_config.max_memory_mb),
            working_directory=working_dir,
            execution_mode=ExecutionMode(operation.data.get('execution_mode', 'development')),
    ...
    async def stream_command_output(self, operation: Operation):
        ...
        async for stream_type, content in self.enhanced_executor.stream_command_output(command, args, config):
    ...
    async def _execute_command(self, command: str, args: List[str], working_dir: Optional[str] = None) -> OperationResult:
        """
        ...
        try:
            ...
            result = await self.enhanced_executor.execute_command(command, args or [], config)
            ...
            return OperationResult(
                ...
                error=result.error_message if not result.success and result.error_message else (result.stderr if not result.success else None)
            )
...
class WebClient(BaseManager):
    ...
    async def _make_request(self, url: str, method: str, headers: Dict[str, str], data: Any) -> OperationResult:
...
class MCPIntegrator(BaseManager):
    ...
    async def _call_tool_enhanced(self, tool_name: str, arguments: Dict[str, Any], 
                                context: ToolExecutionContext) -> OperationResult:
    ...
    async def discover_tools(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        return {name: getattr(tool, 'description', '') for name, tool in discovered.items()}
    ...
    async def _call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> OperationResult:
        """
...
class ApprovalManager:
    ...
    async def request_approval(self, operation: Operation) -> bool:
        ...
        if not self.approval_callback:
            ...
            logger.warning(f"No approval callback set, defaulting to deny for {operation.type}")
            return False
...
class ProviderFallback:
    ...
    def __init__(self, core_engine: CoreEngine):
        ...
        self.enhanced_fallback = EnhancedProviderFallback(core_engine, core_engine.health_monitor)
    ...
    async def execute_with_fallback(self, operation_func, *args, **kwargs):
        ...
        return await self.enhanced_fallback.execute_with_fallback(operation_func, *args, **kwargs)
...
class AgentEngine(CoreEngine):
    ...
    def configure_approval_settings(self, 
    ...
    def set_approval_callbacks(self, approval_callback=None, batch_approval_callback=None) -> None:
    ...
    async def execute_with_approval(self, operation: Operation) -> OperationResult:
        ...
        try:
            ...
            logger.error(f"Operation execution failed: {e}", extra={'context': error_context})
    ...
    async def execute_with_enhanced_approval(self, operation: Operation) -> OperationResult:
        """
        ...
        try:
            ...
            if operation.requires_approval:
                approved = await self.enhanced_approval.request_single_approval(operation)
                if not approved:
                    return OperationResult(
                        success=False,
                        error="Operation not approved by user"
            ...
            logger.error(f"Enhanced approval operation execution failed: {e}", extra={'context': error_context})
    ...
    async def execute_batch_with_approval(self, operations: List[Operation]) -> List[OperationResult]:
        """
        ...
        try:
            ...
            if approval_required_ops:
                batch_request = await self.enhanced_approval.request_batch_approval(approval_required_ops)
            ...
            error_context = {
                ...
                'operations_with_approval': len([op for op in operations if op.requires_approval]),
                'operation_types': [op.type.value if hasattr(op.type, 'value') else str(op.type) for op in operations],
                'error_type': type(e).__name__
            }
            logger.error(f"Batch operation execution failed: {e}", extra={'context': error_context})
    ...
    def _get_manager_for_operation(self, operation: Operation) -> Optional[BaseManager]:
        ...
        if operation.type in [
            OperationType.FILE_READ, OperationType.FILE_WRITE, OperationType.FILE_DELETE,
            OperationType.DIRECTORY_CREATE, OperationType.DIRECTORY_DELETE
        ]:
    ...
    async def rollback_operation(self, operation_index: int) -> bool:
        ...
        if not result.rollback_data:
            logger.warning(f"No rollback data available for operation {operation_index}")
            return False
        ...
        try:
            ...
            if operation.type == OperationType.FILE_WRITE:
                ...
                rollback_op = Operation(
                    ...
                    data={'path': path, 'content': backup_content, 'create_backup': False},
    ...
    def configure_fallback_providers(self, providers: List[str], rankings: Optional[Dict[str, ProviderRank]] = None):
        """
    ...
    def configure_circuit_breaker(self, threshold: int = 5, recovery_time: int = 600):
        ...
        """
        self.fallback.enhanced_fallback.configure_circuit_breaker(threshold, recovery_time)
        logger.info(f"Configured circuit breaker: threshold={threshold}, recovery_time={recovery_time}s")
    ...
    def configure_approval_interface(self, 
    ...
    def get_pending_approval_requests(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'approval_workflow_completed': len(self.approval_workflow.completed_requests)
        }
    ...
    async def write_file_with_review(self, 
                                    path: Union[str, Path],
    ...
    async def write_file_with_default_review(self, 
                                            path: Union[str, Path],
                                            content: Union[str, bytes],
                                            encoding: str = 'utf-8') -> Dict[str, Any]:
        """
        ...
        if not callback:
            ...
            logger.warning("No default review callback set, falling back to regular write")
    ...
    def _setup_autonomous_file_workflow(self):
        ...
        async def autonomous_file_review_callback(review_data: Dict[str, Any]) -> Dict[str, Any]:
            """
        ...
        async def autonomous_write_file(path, content, encoding='utf-8', **kwargs):
            ...
            logger.debug(f"✅ Write completed for {path}: {result.get('success', False)}")
            return result
