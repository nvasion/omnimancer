...
class ProviderRegistry:
    ...
    def update_provider_models(self, provider_name: str, models: List[EnhancedModelInfo]) -> None:
        """
        ...
        """
        logger.info(f"Updating model catalog for provider: {provider_name} ({len(models)} models)")
        self.model_catalog[provider_name] = models
        self.last_update = datetime.now()
    
    def get_models_by_capability(self, capability: str) -> Dict[str, List[EnhancedModelInfo]]:
        """
    ...
    def get_models_by_pricing(self, max_cost_per_million: float) -> Dict[str, List[EnhancedModelInfo]]:
        """
    ...
    def get_models_by_performance(self, min_swe_score: float) -> Dict[str, List[EnhancedModelInfo]]:
        """
    ...
    def get_model_by_name(self, provider_name: str, model_name: str) -> Optional[EnhancedModelInfo]:
        """
    ...
    def get_catalog_stats(self) -> Dict[str, Any]:
        ...
        tool_models = sum(len(models) for models in self.get_tool_capable_models().values())
        multimodal_models = sum(len(models) for models in self.get_multimodal_models().values())
        ...
        high_perf = sum(len(models) for models in self.get_models_by_performance(60.0).values())
        mid_perf = sum(len(models) for models in self.get_models_by_performance(40.0).values()) - high_perf
    ...
    def validate_catalog(self) -> List[str]:
        ...
        for provider_name, models in self.model_catalog.items():
            if provider_name not in self.providers:
                errors.append(f"Models exist for unregistered provider: {provider_name}")
                continue
            ...
            for model in models:
                ...
                if len(duplicates) > 1:
                    errors.append(f"Duplicate model {model.name} in provider {provider_name}")
