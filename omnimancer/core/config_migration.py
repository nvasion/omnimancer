...
class ConfigMigration:
        ...
        try:
            ...
            try:
                ...
                if validation_errors:
                    messages.append("Warning: Migrated configuration has validation errors:")
                    messages.extend([f"  - {error}" for error in validation_errors])
            except Exception as e:
                messages.append(f"Warning: Could not validate migrated configuration: {e}")
    ...
    def _migrate_from_v1(self, old_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        new_config = {
            ...
            'storage_path': old_config.get('storage_path', str(Path.home() / '.omnimancer')),
            'providers': {},
            'chat_settings': {
        ...
        for provider_name, provider_config in old_providers.items():
            new_provider_config = self._migrate_provider_config_v1(provider_name, provider_config)
    ...
    def _migrate_provider_config_v1(self, provider_name: str, old_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        new_config = {
            ...
            'priority': old_config.get('priority', provider_defaults.get('priority', 0)),
...
class ConfigValidator:
    ...
    def _validate_general(self) -> List[str]:
        ...
        if not self.config.default_provider:
            ...
            errors.append(f"Default provider '{self.config.default_provider}' not configured")
    ...
    def _validate_provider(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_profile(self, profile_name: str, profile: 'ConfigProfile') -> List[str]:
        ...
        if not profile.default_provider:
            ...
            errors.append(f"Default provider '{profile.default_provider}' not configured")
    ...
    def _validate_performance(self) -> List[str]:
        ...
        if self.config.concurrent_requests_limit > 20:
            errors.append("High concurrent request limit may cause rate limiting issues")
