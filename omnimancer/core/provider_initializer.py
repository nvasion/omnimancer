...
class ProviderInitializer:
    ...
    def get_provider_class(cls, provider_name: str) -> Type[BaseProvider]:
        ...
        with cls._class_lock:
            ...
            try:
                ...
                logger.error(f"Failed to import provider module for {provider_name}: {e}")
                raise
            except Exception as e:
                logger.error(f"Error loading provider class for {provider_name}: {e}")
                raise ValueError(f"Failed to load provider class for {provider_name}: {e}")
    ...
    @classmethod
    def get_provider_instance(cls, provider_name: str, config: ProviderConfig, config_manager: 'ConfigManager' = None) -> BaseProvider:
        """
        ...
        with cls._instance_lock:
            ...
            # Create new instance
            instance = cls._create_provider_instance(provider_name, config, config_manager)
    ...
    @classmethod
    def _create_provider_instance(cls, provider_name: str, config: ProviderConfig, config_manager: 'ConfigManager' = None) -> BaseProvider:
        """
        ...
        kwargs = {k: v for k, v in config.model_dump(exclude={"api_key", "model"}).items() if v is not None}
    ...
    @classmethod
    def get_model_info(cls, provider_name: str, enhanced: bool = True) -> List[EnhancedModelInfo]:
        """
    ...
    @classmethod
    def _fetch_model_info(cls, provider_name: str, enhanced: bool) -> List[EnhancedModelInfo]:
        """
    ...
    def preload_provider_classes(cls, provider_names: List[str]) -> None:
        ...
        for provider_name in provider_names:
            try:
                ...
                logger.warning(f"Failed to preload provider class for {provider_name}: {e}")
    
    @classmethod
    async def initialize_providers(cls, provider_configs: Dict[str, ProviderConfig], config_manager: 'ConfigManager' = None) -> Dict[str, BaseProvider]:
        """
        ...
        for provider_name, config in provider_configs.items():
            try:
                ...
                provider = ProviderFactory.create_provider(provider_name, config, config_manager)
