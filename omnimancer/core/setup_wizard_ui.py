...
class SetupWizardUI:
    ...
    def show_welcome(self) -> None:
        ...
        welcome_text.append("This wizard will help you add AI providers to your configuration.\n")
        welcome_text.append("You can run this anytime to add more providers without affecting existing ones.\n")
    ...
    def show_troubleshooting_guidance(self, provider_name: str, error: str) -> None:
        ...
        if "api key" in error.lower() or "authentication" in error.lower() or "format" in error.lower():
            guidance.append("🔑 API Key Issues:\n", style="bold red")
            guidance.append(f"  • Get your API key from: {info['api_key_url']}\n")
            if info['api_key_prefix']:
                guidance.append(f"  • Ensure the key starts with '{info['api_key_prefix']}'\n")
            guidance.append("  • Check that your account has sufficient credits/quota\n")
            guidance.append("  • Verify the API key hasn't expired\n\n")
        
        if "connection" in error.lower() or "network" in error.lower() or "timeout" in error.lower():
    ...
    def _add_provider_specific_guidance(self, guidance: Text, provider_name: str, error: str) -> None:
        """
        ...
        if provider_name == "claude":
            ...
            guidance.append("  • Ensure you're using the correct API version (2023-06-01)\n")
            guidance.append("  • Check message format (Claude uses specific message structure)\n")
            guidance.append("  • Verify your account tier supports the selected model\n")
            guidance.append("  • Try reducing max_tokens if getting context errors\n\n")
            
        elif provider_name == "openai":
            guidance.append("🤖 OpenAI-Specific:\n", style="bold")
            guidance.append("  • Check if you have access to GPT-4 models (requires separate approval)\n")
            ...
            guidance.append("  • Enable the Generative AI API in Google Cloud Console\n")
            guidance.append("  • Check safety settings if requests are being blocked\n")
            guidance.append("  • Verify your Google Cloud project has billing enabled\n")
            guidance.append("  • Try gemini-1.5-flash for faster/cheaper responses\n\n")
            
        elif provider_name == "perplexity":
            guidance.append("🔍 Perplexity-Specific:\n", style="bold")
            guidance.append("  • Ensure you have a Perplexity Pro subscription for API access\n")
            guidance.append("  • Check your monthly API usage limits\n")
            guidance.append("  • Verify search functionality is enabled for your account\n")
            guidance.append("  • Try 'sonar' model if 'sonar-pro' is not accessible\n\n")
            
        elif provider_name == "xai":
            guidance.append("🚀 xAI-Specific:\n", style="bold")
            guidance.append("  • Ensure you have access to the xAI API (currently in beta)\n")
            ...
            guidance.append("  • Ensure you have a Mistral AI account with API access\n")
