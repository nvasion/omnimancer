...
class HealthMonitor:
    ...
    async def check_provider_health(self, provider_name: str, config: ProviderConfig, 
        ...
        try:
            ...
            if hasattr(self, 'provider_factory') and self.provider_factory:
                provider = self.provider_factory.create_provider(provider_name, config=config)
            else:
                provider = ProviderInitializer.get_provider_instance(provider_name, config)
            ...
            try:
                ...
                if hasattr(provider, '_mock_name') or str(type(provider)).find('Mock') != -1:
            ...
            try:
                ...
                model_available = model_info.available if hasattr(model_info, 'available') else True
            except Exception:
            ...
            with self._lock:
                ...
                self._cache_timestamps[cache_key] = time.time() - (self._cache_ttl / 2)  # Shorter TTL for errors
            
            return error_status
    
    async def check_all_providers_health(self, configs: Dict[str, ProviderConfig], 
    ...
    def start_monitoring(self, configs: Dict[str, ProviderConfig], 
    ...
    def _monitoring_worker(self) -> None:
        ...
        while self._monitoring_enabled and not self._stop_monitoring.is_set():
            try:
                ...
                try:
                    loop.run_until_complete(self.check_all_providers_health(self._provider_configs))
    ...
    async def check_all_providers(self, config, force: bool = False) -> 'HealthResult':
        ...
        return HealthResult(
            ...
            healthy_providers=sum(1 for h in health_status.values() if h.get('status') == 'healthy')
        )
...
class HealthResult:
    ...
    def __init__(self, overall_healthy: bool, provider_health: Dict[str, Dict[str, Any]], 
                 total_providers: int, healthy_providers: int):
