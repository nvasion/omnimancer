...
class EnhancedModelInfo:
    ...
    def to_model_info(self) -> ModelInfo:
        ...
        if self.is_free:
            ...
            cost_per_token = (self.cost_per_million_input + self.cost_per_million_output) / 2 / 1_000_000
...
class ProviderConfig(BaseModel):
    ...
    api_key: Optional[str] = None  # Optional for providers like Ollama that don't need keys
    model: str
    max_tokens: Optional[int] = None
    temperature: Optional[float] = None
    ...
    response_format: Optional[Dict[str, str]] = None  # Response format (e.g., json_object)
    ...
    def validate_search_recency_filter(cls, v):
        if v is not None:
            ...
            if v not in valid_filters:
                raise ValueError(f'search_recency_filter must be one of: {", ".join(valid_filters)}')
        return v
    ...
    def validate_claude_code_mode(cls, v):
        if v is not None:
            ...
            if v not in valid_modes:
                raise ValueError(f'claude_code_mode must be one of: {", ".join(valid_modes)}')
        return v
    ...
    def get_provider_specific_config(self) -> Dict[str, Any]:
        ...
        for field_name, field_value in self.model_dump().items():
            if field_value is not None and field_name not in ['api_key', 'model']:
    ...
    def get_all_provider_templates(cls) -> Dict[str, Dict[str, Any]]:
        ...
        """
        provider_names = [
            'claude', 'openai', 'gemini', 'cohere', 'ollama',
            'perplexity', 'xai', 'mistral', 'azure', 'vertex',
            'bedrock', 'openrouter', 'claude-code'
        ]
        ...
        for field in sensitive_fields:
            if field in data and data[field]:
                if isinstance(data[field], str):
                    ...
                    data[field] = {k: self._mask_string(str(v)) if isinstance(v, str) else v 
...
class Config(BaseModel):
    ...
    def create_profile(self, name: str, description: Optional[str] = None) -> ConfigProfile:
    ...
    def merge_from_env(self, env_vars: Dict[str, str]) -> None:
        ...
        for env_var, value in env_vars.items():
            if env_var.startswith('omnimancer_') and '_API_KEY' in env_var:
                ...
                provider_name = env_var.replace('omnimancer_', '').replace('_API_KEY', '').lower()
                if provider_name in self.providers:
                    # Don't store the actual API key in config_sources for security
                    self.config_sources[f'providers.{provider_name}.api_key'] = f'env:{env_var}'
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if not self.default_provider:
            ...
            errors.append(f"Default provider '{self.default_provider}' not found in configured providers")
        ...
        for provider_name, provider_config in self.providers.items():
            provider_errors = self._validate_provider_config(provider_name, provider_config)
    ...
    def _validate_provider_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_claude_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_openai_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_gemini_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_cohere_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
    ...
    def _validate_ollama_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if self.mcp.enabled:
            ...
            for server_name, server_config in self.mcp.servers.items():
                if not server_config.command:
                    errors.append(f"MCP server '{server_name}' has no command specified")
        
        return errors
    
    def _validate_profile_config(self, profile_name: str, profile: ConfigProfile) -> List[str]:
        """Validate a configuration profile."""
        errors = []
        
        if profile.default_provider not in profile.providers:
            errors.append(f"Profile '{profile_name}' default provider '{profile.default_provider}' not found in profile providers")
        
        # Validate each provider in the profile
        for provider_name, provider_config in profile.providers.items():
            provider_errors = self._validate_provider_config(provider_name, provider_config)
            errors.extend([f"Profile '{profile_name}': {error}" for error in provider_errors])
    ...
    def _validate_claude_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Claude model '{config.model}' for provider '{provider_name}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_openai_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown OpenAI model '{config.model}' for provider '{provider_name}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_gemini_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if not config.api_key and not config.service_account_path:
            errors.append(f"Gemini provider '{provider_name}' requires either an API key or service account")
        
        # Validate model
        valid_models = ["gemini-1.5-pro", "gemini-1.5-flash", "gemini-1.0-pro"]
        if config.model not in valid_models:
            errors.append(f"Unknown Gemini model '{config.model}' for provider '{provider_name}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_cohere_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Cohere model '{config.model}' for provider '{provider_name}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_ollama_config(self, provider_name: str, config: ProviderConfig) -> List[str]:
        ...
        if config.base_url:
            if not config.base_url.startswith(('http://', 'https://')):
                errors.append(f"Ollama provider '{provider_name}' base_url must start with 'http://' or 'https://'")
    ...
    def _validate_mcp_config(self) -> List[str]:
        ...
        for server_name, server_config in self.mcp.servers.items():
            ...
            if server_config.timeout <= 0:
                errors.append(f"MCP server '{server_name}' has invalid timeout: {server_config.timeout}")
    ...
    def _validate_profile_config(self, profile_name: str, profile: ConfigProfile) -> List[str]:
        ...
        # Validate each provider in the profile
        for provider_name, provider_config in profile.providers.items():
            provider_errors = self._validate_provider_config(f"{profile_name}.{provider_name}", provider_config)
            errors.extend(provider_errors)
    ...
    def get_providers_by_priority(self) -> List[tuple[str, ProviderConfig]]:
        ...
        return sorted(enabled_providers.items(), key=lambda x: x[1].priority, reverse=True)
    
    def get_providers_with_tools(self) -> Dict[str, ProviderConfig]:
        """Get all providers that support tools."""
        return {
            name: config for name, config in self.providers.items() 
            if config.enabled and (config.supports_tools or config.supports_function_calling)
        }
    ...
    def update_provider_health_status(self, provider_name: str, is_healthy: bool) -> None:
...
class ConfigTemplateManager:
    ...
    def _create_coding_template(self) -> ConfigTemplate:
        ...
        return ConfigTemplate(
            ...
            recommended_models={'claude': 'claude-3-5-sonnet-20241022', 'openai': 'gpt-4o', 'claude_code': 'claude-code-sonnet'},
            settings={'temperature': 0.2, 'max_tokens': 4096},
            provider_configs={
                'claude': {'temperature': 0.2, 'max_tokens': 4096, 'model': 'claude-3-5-sonnet-20241022'},
                'openai': {'temperature': 0.2, 'max_tokens': 4096, 'model': 'gpt-4o'},
                'claude_code': {'temperature': 0.2, 'max_tokens': 4096, 'model': 'claude-code-sonnet'}
            },
            mcp_servers={
                'filesystem': {'command': 'uvx', 'args': ['mcp-server-filesystem'], 'enabled': False},
    ...
    def _create_research_template(self) -> ConfigTemplate:
        ...
        return ConfigTemplate(
            ...
            recommended_models={'perplexity': 'sonar-pro', 'claude': 'claude-3-5-sonnet-20241022', 'openai': 'gpt-4o'},
            settings={'temperature': 0.3, 'max_tokens': 8192},
            provider_configs={
                'perplexity': {'temperature': 0.3, 'max_tokens': 8192, 'model': 'sonar-pro'},
                'claude': {'temperature': 0.3, 'max_tokens': 8192, 'model': 'claude-3-5-sonnet-20241022'},
                'openai': {'temperature': 0.3, 'max_tokens': 8192, 'model': 'gpt-4o'}
            },
            mcp_servers={
                'web-search': {'command': 'uvx', 'args': ['mcp-server-web-search'], 'enabled': False},
                'knowledge': {'command': 'uvx', 'args': ['mcp-server-knowledge'], 'enabled': False}
            }
        )
    ...
    def _create_creative_template(self) -> ConfigTemplate:
        ...
        return ConfigTemplate(
            ...
            recommended_models={'openai': 'gpt-4o', 'claude': 'claude-3-5-sonnet-20241022', 'gemini': 'gemini-1.5-pro'},
            settings={'temperature': 0.8, 'max_tokens': 4096},
            provider_configs={
                'openai': {'temperature': 0.8, 'max_tokens': 4096, 'model': 'gpt-4o'},
                'claude': {'temperature': 0.8, 'max_tokens': 4096, 'model': 'claude-3-5-sonnet-20241022'},
                'gemini': {'temperature': 0.8, 'max_tokens': 4096, 'model': 'gemini-1.5-pro'}
            }
        )
    ...
    def _create_performance_template(self) -> ConfigTemplate:
        ...
        return ConfigTemplate(
            ...
            recommended_providers=['openai', 'claude', 'gemini'],
            recommended_models={'openai': 'gpt-4o-mini', 'claude': 'claude-3-haiku', 'gemini': 'gemini-1.5-flash'},
            settings={'temperature': 0.5, 'max_tokens': 2048},
            provider_configs={
                'openai': {'temperature': 0.5, 'max_tokens': 2048, 'model': 'gpt-4o-mini'},
                'claude': {'temperature': 0.5, 'max_tokens': 2048, 'model': 'claude-3-haiku'},
                'gemini': {'temperature': 0.5, 'max_tokens': 2048, 'model': 'gemini-1.5-flash'}
            }
        )
    ...
    def _create_general_template(self) -> ConfigTemplate:
        ...
        return ConfigTemplate(
            ...
            recommended_models={'openai': 'gpt-4o-mini', 'anthropic': 'claude-3-haiku'},
            settings={'temperature': 0.7, 'max_tokens': 2048},
            provider_configs={
                'openai': {'temperature': 0.7, 'max_tokens': 2048, 'model': 'gpt-4o-mini'},
                'anthropic': {'temperature': 0.7, 'max_tokens': 2048, 'model': 'claude-3-haiku'}
            }
        )
