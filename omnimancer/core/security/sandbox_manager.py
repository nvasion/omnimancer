...
class SandboxedProcess:
    ...
    def __init__(self, process: subprocess.Popen, temp_dir: str, limits: ResourceLimits):
...
class SandboxManager:
    ...
    def create_sandbox_environment(self, limits: Optional[ResourceLimits] = None) -> str:
    ...
    def execute_sandboxed_command(self, 
                                  command: List[str],
            ...
            try:
                ...
                try:
                    ...
                    if memory_mb > sandboxed_proc.limits.max_memory_mb:
                        print(f"Process {sandboxed_proc.process.pid} exceeded memory limit")
                        sandboxed_proc.terminate()
                        break
                    ...
                    if cpu_time > sandboxed_proc.limits.max_cpu_seconds:
                        print(f"Process {sandboxed_proc.process.pid} exceeded CPU time limit")
                        sandboxed_proc.terminate()
                        break
                    ...
                    if runtime > sandboxed_proc.limits.timeout_seconds:
                        print(f"Process {sandboxed_proc.process.pid} exceeded runtime limit")
                        sandboxed_proc.terminate()
                        break
