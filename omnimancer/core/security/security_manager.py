...
from .permission_controller import PermissionController, PermissionOperation, PermissionLevel
...
class SecurityManager:
    ...
    def __init__(self,
        ...
        self.sandbox = SandboxManager(default_resource_limits) if enable_sandbox else None
    ...
    async def validate_operation(self, operation: PermissionOperation) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.approval and self.security_policies.get('require_approval_for_high_risk', True):
                ...
                if risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
                    ...
                    if approval_request.status == ApprovalStatus.PENDING:
                        ...
                        result['reasons'].append(f'Approval required for {risk_level.value} risk operation')
                        return result
                    elif approval_request.status != ApprovalStatus.APPROVED:
                        result['reasons'].append(f'Operation not approved: {approval_request.status.value}')
                        return result
            
            # Step 3: Determine if sandboxing is required
            if self.sandbox and self.security_policies.get('sandbox_all_commands', True):
                if operation.command or operation.operation_type in ['command_execute', 'file_write', 'file_create']:
            ...
            if self.audit:
                self.audit.log_event(
                    ...
                    operation_id=operation_id,
                    session_id=self.session_id,
                    metadata={'operation_type': operation.operation_type, 'error': str(e)}
                )
            
            result['reasons'].append(f'Security validation error: {str(e)}')
            return result
    
    async def execute_secure_command(self,
        ...
        try:
            ...
            if not validation_result['allowed']:
                ...
                result['stderr'] = f"Command blocked: {', '.join(validation_result['reasons'])}"
                return result
            
            # Handle approval if required
            if validation_result.get('approval_required'):
                result['stderr'] = f"Approval required. Request ID: {validation_result['approval_request_id']}"
                return result
    ...
    async def secure_file_access(self,
                               file_path: str,
                               operation: str,
        ...
        try:
            ...
            if not validation_result['allowed']:
                ...
                result['error'] = f"File access blocked: {', '.join(validation_result['reasons'])}"
                return result
            
            # Handle approval if required
            if validation_result.get('approval_required'):
                result['error'] = f"Approval required. Request ID: {validation_result['approval_request_id']}"
                return result
            ...
            if operation == 'read':
                ...
                if content is None:
                    ...
                    if len(content.encode('utf-8')) > max_size_mb * 1024 * 1024:
                        result['error'] = f'Content exceeds maximum file size ({max_size_mb}MB)'
                    else:
            ...
            if self.audit:
                self.audit.log_event(
                    ...
                    operation_id=operation_id,
                    session_id=self.session_id,
                    metadata={'file_path': file_path, 'operation': operation, 'error': str(e)}
                )
