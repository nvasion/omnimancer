...
class AuditEvent:
    ...
    def _calculate_hash(self) -> str:
        ...
        if self.metadata:
            filtered_metadata = {k: v for k, v in self.metadata.items() if k != 'event_hash'}
...
class AuditLogger:
    ...
    def log_command_execution(self,
        ...
        event_type = AuditEventType.COMMAND_EXECUTED if success else AuditEventType.COMMAND_BLOCKED
    ...
    def log_file_access(self,
        ...
        event_type = AuditEventType.FILE_ACCESS if allowed else AuditEventType.FILE_ACCESS_DENIED
        level = AuditLevel.INFO if allowed else AuditLevel.WARNING
    ...
    def log_security_alert(self,
        ...
        self.log_event(AuditEventType.SECURITY_ALERT, severity, message, metadata=metadata)
    ...
    def get_recent_events(self, 
        ...
        try:
            ...
            for line in reversed(lines[-count*2:]):  # Read more lines to account for filtering
                try:
