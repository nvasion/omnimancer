...
class PermissionOperation:
    ...
    def __init__(self, operation_type: str, path: Optional[str] = None, 
...
class PermissionController:
    ...
    def validate_path_access(self, path: str, operation: str = "read") -> bool:
        ...
        try:
            ...
            if operation in ['write', 'delete', 'execute']:
                ...
                if any(normalized_path.startswith(prefix) for prefix in system_prefixes):
                    return False
                ...
                is_in_safe_temp = any(normalized_path.startswith(prefix) for prefix in safe_temp_prefixes)
        ...
        if not pattern.startswith('/') and not pattern.startswith('~'):
            ...
            for i, part in enumerate(path_parts):
                if '*' not in pattern:
                    ...
                    if remaining_path.startswith(pattern + '/') or remaining_path == pattern:
                        return True
                else:
    ...
    async def check_operation_permission(self, 
                                       operation_type: str,
    ...
    async def grant_operation_permission(self,
                                       operation_type: str,
                                       operation_signature: str,
