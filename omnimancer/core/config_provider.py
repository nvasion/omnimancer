...
from .models import ConfigTemplateManager, ConfigTemplate
from .config_validator import ConfigValidator
# from .config_migration_helpers import ConfigMigrationHelper  # Removed - over-engineered
from .models import Config, ProviderConfig, MCPConfig
...
class ConfigurationProvider:
    ...
    def set_configuration_context(self, context: ConfigurationContext) -> None:
        ...
        self._save_mode_preferences()
        logger.info(f"Configuration context updated: use_case={context.use_case}, level={context.user_level}")
    ...
    async def comprehensive_validation(self, config_path: Optional[Path] = None) -> ValidationReport:
        ...
        try:
            ...
            is_compatible, comp_errors, comp_warnings = self._validate_compatibility(config_path)
            ...
            provider_status = await self._check_provider_health(config) if config else {}
            
            # Check for performance and security issues
            performance_issues = self._check_performance_issues(config, format_info) if config else []
            security_issues = self._check_security_issues(config) if config else []
            
            # Generate suggestions and upgrade recommendations
            suggestions = self._generate_suggestions(config, format_info, all_errors, all_warnings)
            upgrade_recommendations = self._generate_upgrade_recommendations(format_info)
            
            # Calculate health score
            health_score = self._calculate_health_score(
                critical_errors, all_warnings, provider_status, performance_issues, security_issues
            )
            
            # Check for migration and backup
            migration_required = len(all_errors) > 3 or format_info.get("version", self.current_version) < self.current_version
    ...
    def _detect_configuration_format(self, config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    def _analyze_config_format(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif "user_preferences" in detected_features or "template_source" in detected_features:
            format_type = "simple"
        
        # Check for issues
        if "providers" in config_data and len(config_data["providers"]) > 5:
            issues.append(f"Large number of providers ({len(config_data['providers'])}) might benefit from simplification")
    ...
    def _validate_compatibility(self, config_path: Optional[Path] = None) -> Tuple[bool, List[str], List[str]]:
        ...
        try:
            ...
            if format_info["compatibility_level"] == "incompatible":
                errors.append("Configuration format is incompatible and requires manual migration")
                errors.extend(format_info["issues"])
                return False, errors, warnings
            
            elif format_info["compatibility_level"] == "partial":
                warnings.append("Configuration has compatibility issues that may need attention")
            ...
            try:
                ...
                if validation_errors:
                    for error in validation_errors:
                        if any(critical in error.lower() for critical in ["missing", "required", "invalid"]):
                            errors.append(error)
                        else:
            ...
            if format_info["upgrade_available"]:
                warnings.append(f"Configuration upgrade available: v{format_info['version']} → v{self.current_version}")
    ...
    async def _load_config_safely(self, config_path: Optional[Path] = None) -> Optional[Config]:
    ...
    async def _check_provider_health(self, config: Config) -> Dict[str, Dict[str, Any]]:
        ...
        for provider_name, provider_config in config.providers.items():
            health_check = await self._check_single_provider_health(provider_name, provider_config)
            provider_status[provider_name] = asdict(health_check)
        
        return provider_status
    
    async def _check_single_provider_health(self, provider_name: str, provider_config: ProviderConfig) -> ProviderHealthCheck:
        ...
        if model_valid and provider_config.model:
            model_issues = self._validate_model_name(provider_name, provider_config.model)
            if model_issues:
        ...
        provider_specific_issues = self._check_provider_specific_issues(provider_name, provider_config)
    ...
    def _validate_model_name(self, provider_name: str, model_name: str) -> List[str]:
        ...
        known_models = {
            "claude": ["claude-3-sonnet-20240229", "claude-3-haiku-20240307", "claude-3-opus-20240229", "claude-3-5-sonnet-20241022"],
        ...
        if provider_name in known_models:
            ...
            if not any(model in model_name for model in valid_models):
                issues.append(f"Model '{model_name}' may not be valid for {provider_name}")
        
        return issues
    
    def _check_provider_specific_issues(self, provider_name: str, provider_config: ProviderConfig) -> List[str]:
        """Check provider-specific configuration issues."""
        issues = []
        
        if provider_name.lower() == "azure":
            if not hasattr(provider_config, 'azure_endpoint') or not provider_config.azure_endpoint:
                issues.append("Azure endpoint not specified")
            if not hasattr(provider_config, 'azure_deployment') or not provider_config.azure_deployment:
                issues.append("Azure deployment not specified")
        elif provider_name.lower() == "bedrock":
            if not hasattr(provider_config, 'aws_region') or not provider_config.aws_region:
    ...
    def _check_performance_issues(self, config: Config, format_info: Dict[str, Any]) -> List[str]:
        """Check for performance-related configuration issues."""
        issues = []
        
        if len(config.providers) > 8:
            issues.append(f"Large number of providers ({len(config.providers)}) may impact performance")
        
        timeout_issues = sum(1 for provider_config in config.providers.values() 
        ...
        if hasattr(config, 'mcp') and config.mcp and hasattr(config.mcp, 'servers'):
            if len(config.mcp.servers) > 10:
                issues.append(f"Large number of MCP servers ({len(config.mcp.servers)}) may impact startup time")
        
        if hasattr(config, 'configuration_mode') and config.configuration_mode == "simple":
    ...
    def _generate_suggestions(self, config: Optional[Config], format_info: Dict[str, Any], 
        ...
        if any("no providers" in error.lower() for error in errors):
            suggestions.append("Run 'omnimancer config setup' to configure AI providers")
        ...
        if format_info.get("upgrade_available", False):
            suggestions.append(f"Upgrade configuration from v{format_info.get('version')} to v{self.current_version}")
        
        if format_info.get("format_type") == "legacy" and len(format_info.get("detected_features", [])) > 5:
            suggestions.append("Consider using simplified configuration mode for easier management")
        ...
        if config and len(config.providers) == 1:
            ...
            suggestions.append("Consider simplifying provider configuration for better performance")
    ...
    def _generate_upgrade_recommendations(self, format_info: Dict[str, Any]) -> List[str]:
        ...
        if format_info.get("upgrade_available", False):
            ...
            if compatibility_level == "full":
                recommendations.append("Automatic upgrade available - run 'omnimancer config migrate'")
            elif compatibility_level == "partial":
                recommendations.append("Assisted upgrade recommended - review configuration issues first")
            else:
                recommendations.append("Manual upgrade required - backup configuration before changes")
        
        if "mcp" in format_info.get("detected_features", []) and format_info.get("version", "2.0") < "2.0":
            recommendations.append("MCP configuration can be simplified in newer format")
        
        if len(format_info.get("detected_features", [])) > 6:
            recommendations.append("Configuration complexity can be reduced with template-based setup")
        
        return recommendations
    
    def _calculate_health_score(self, critical_errors: List[str], warnings: List[str], 
                               provider_status: Dict[str, Dict[str, Any]], performance_issues: List[str],
                               security_issues: List[str]) -> float:
    ...
    def generate_compatibility_report(self) -> str:
        ...
        try:
            ...
            report_lines.append(f"- Version: {validation_report.format_info.get('version', 'unknown')}")
            report_lines.append(f"- Format Type: {validation_report.format_info.get('format_type', 'unknown').title()}")
            report_lines.append(f"- Compatible: {'Yes' if validation_report.is_compatible else 'No'}")
            report_lines.append(f"- Valid: {'Yes' if validation_report.is_valid else 'No'}")
            report_lines.append(f"- Health Score: {validation_report.health_score:.1f}/100")
            report_lines.append("")
            ...
            # Recommendations
            if validation_report.suggestions or validation_report.upgrade_recommendations:
            ...
            if validation_report.provider_status:
                ...
                for provider, status in validation_report.provider_status.items():
                    ...
                    report_lines.append(f"- **{provider}**: {status_icon} {status['status'].title()}")
            ...
            if validation_report.migration_required:
                ...
                report_lines.append("Consider migrating to the latest configuration format for better features.")
                if validation_report.backup_available:
                    report_lines.append("Configuration backups are available for safe migration.")
                else:
                    report_lines.append("**Warning**: No configuration backups found. Create backup before migration.")
            ...
            try:
                ...
                for name, provider_config in config.providers.items():
                    if name == "claude" and provider_config.model == "claude-v1":
                        ...
                        fixed_issues += 1
                    elif name == "openai" and provider_config.model in ["text-davinci-003", "code-davinci-002"]:
                ...
                if final_report.is_valid and final_report.is_compatible:
                    ...
                    messages.extend(final_report.suggestions[:3])  # Show top 3 suggestions
    ...
    def _create_simple_sections(self) -> List[SimpleConfigSection]:
        ...
        quick_setup_options = [
            SimpleConfigOption(
                ...
                option_type="choice",
                choices=[
                    {"value": "coding", "label": "Coding & Development", "description": "Writing code, debugging, technical documentation"},
                    {"value": "research", "label": "Research & Analysis", "description": "Research papers, data analysis, fact-checking"},
                    {"value": "creative", "label": "Creative Writing", "description": "Stories, articles, creative content"},
                    {"value": "general", "label": "General Use", "description": "Mixed tasks, general assistance"},
                    {"value": "performance", "label": "Fast & Economical", "description": "Quick responses, minimal costs"}
                ],
            ...
            SimpleConfigOption(
                ...
                option_type="choice",
                choices=[
                    {"value": "beginner", "label": "Beginner", "description": "New to AI tools, prefer simple options"},
                    {"value": "intermediate", "label": "Some Experience", "description": "Used AI tools before, comfortable with basic settings"},
                    {"value": "advanced", "label": "Experienced", "description": "Familiar with AI, want more control"},
                    {"value": "expert", "label": "Expert", "description": "Power user, need full customization"}
                ],
        ...
        sections.append(SimpleConfigSection(
            name="Quick Setup",
    ...
    def get_simple_mode_providers(self) -> List[SimplifiedProviderConfig]:
        ...
        for provider_name, info in provider_info.items():
            ...
            simplified_providers.append(SimplifiedProviderConfig(
                name=provider_name,
    ...
    def get_quick_setup_templates(self) -> List[Dict[str, Any]]:
        ...
        for name, template in self.template_manager.get_all_templates().items():
            simplified = {
                ...
                "complexity": "Simple" if name in ["general", "performance"] else "Moderate",
    ...
    def _get_recommended_for(self, template: ConfigTemplate) -> List[str]:
        ...
        recommendations = {
            "coding": ["Software developers", "Students learning programming", "Technical writers"],
        ...
        for provider in all_providers:
            if use_case in provider.use_case_fit or "general" in provider.use_case_fit:
                ...
                relevant_providers.append({
                    ...
                    "relevance": relevance,
                    "setup_difficulty": self._get_provider_setup_difficulty(provider.name),
    ...
    def get_configuration_preview(self, answers: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_cost == 0:
            ...
            preview["estimated_monthly_cost"] = f"~${total_cost}-{total_cost + 20}/month"
    ...
    def get_recommended_configuration(self, context: Optional[ConfigurationContext] = None) -> ConfigurationRecommendation:
    ...
    def apply_template_configuration(self, template_name: str, api_keys: Dict[str, str] = None) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for provider_name in template.recommended_providers[:3]:
                provider_config = self.template_manager.create_provider_config_from_template(template, provider_name)
                ...
                if provider_config:
                    ...
                    messages.append(f"Configured {provider_name} with {provider_config.model}")
    ...
    def validate_simple_answers(self, answers: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    def generate_simple_configuration(self, answers: Dict[str, Any]) -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        try:
            ...
            messages.append(f"Recommended template: {recommendation.template_name} (confidence: {recommendation.confidence:.0%})")
        ...
        try:
            ...
            if preferences_file.exists():
                ...
                self.context = ConfigurationContext(
                    ...
                    preferred_mode=ConfigurationMode(context_data.get("preferred_mode", "simple")),
...
def create_configuration_provider(config_manager: Optional[ConfigManager] = None) -> ConfigurationProvider:
    """
