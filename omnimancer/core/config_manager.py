...
from .models import Config, ProviderConfig, MCPConfig, MCPServerConfig, ConfigProfile
...
class ConfigManager:
    ...
    def load_config(self) -> Config:
        ...
        if migration.needs_migration():
            ...
            if not success:
                raise ConfigurationError(f"Configuration migration failed: {'; '.join(messages)}")
    ...
    def set_provider_config(self, provider_name: str, provider_config: ProviderConfig) -> None:
        """
    ...
    def setup_initial_config(self, provider_configs: Dict[str, Dict[str, str]], default_provider: str) -> None:
        """
        ...
        if default_provider not in provider_configs:
            raise ConfigurationError(f"Default provider '{default_provider}' not in provider configs")
        
        # Create provider configurations
        providers = {}
        for provider_name, config_data in provider_configs.items():
            if 'api_key' not in config_data or 'model' not in config_data:
                raise ConfigurationError(f"Provider '{provider_name}' missing api_key or model")
    ...
    def set_mcp_server_config(self, server_name: str, server_config: MCPServerConfig) -> None:
        """
    ...
    def validate_mcp_config(self) -> List[str]:
        ...
        if mcp_config.auto_approve_timeout <= 0:
            errors.append(f"Invalid MCP auto_approve_timeout: {mcp_config.auto_approve_timeout}")
        if mcp_config.max_concurrent_servers <= 0:
            errors.append(f"Invalid MCP max_concurrent_servers: {mcp_config.max_concurrent_servers}")
    ...
    def validate_provider_config(self, provider_name: str, provider_config: ProviderConfig) -> List[str]:
        """
    ...
    def _validate_claude_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Claude model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_openai_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown OpenAI model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_gemini_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Gemini model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_cohere_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.model not in valid_models:
            errors.append(f"Unknown Cohere model '{config.model}'. Valid models: {', '.join(valid_models)}")
    ...
    def _validate_ollama_config(self, config: ProviderConfig) -> List[str]:
        ...
        if config.timeout and config.timeout < 10:
            errors.append("Ollama timeout should be at least 10 seconds for local inference")
    ...
    def create_provider_config(self, provider_name: str, api_key: str = None, **kwargs) -> ProviderConfig:
        """
    ...
    def load_config_from_sources(self, env_vars: Optional[Dict[str, str]] = None, cli_args: Optional[Dict[str, Any]] = None) -> Config:
        """
    ...
    def _merge_cli_args(self, config: Config, cli_args: Dict[str, Any]) -> None:
        ...
        cli_mapping = {
            ...
            'temperature': lambda: self._set_provider_temperature(config, cli_args.get('temperature')),
            'max_tokens': lambda: self._set_provider_max_tokens(config, cli_args.get('max_tokens')),
            'debug': 'debug_mode',
            'log_level': 'log_level',
        }
    ...
    def _set_provider_model(self, config: Config, model: str) -> None:
        ...
        if model and config.default_provider in config.providers:
            ...
            config.config_sources[f'providers.{config.default_provider}.model'] = 'cli:model'
    ...
    def _set_provider_temperature(self, config: Config, temperature: float) -> None:
        ...
        if temperature is not None and config.default_provider in config.providers:
            ...
            config.config_sources[f'providers.{config.default_provider}.temperature'] = 'cli:temperature'
    ...
    def _set_provider_max_tokens(self, config: Config, max_tokens: int) -> None:
        ...
        if max_tokens is not None and config.default_provider in config.providers:
            ...
            config.config_sources[f'providers.{config.default_provider}.max_tokens'] = 'cli:max_tokens'
    ...
    def create_profile(self, name: str, description: Optional[str] = None, copy_from_current: bool = True) -> ConfigProfile:
        """
            ...
            if cli_arg in cli_args and cli_args[cli_arg] is not None:
                ...
                if '{provider}' in config_path:
                    ...
                    if provider not in config.providers:
                        config.providers[provider] = self.create_provider_config(provider)
    ...
    def _set_nested_value_in_config(self, config: Config, path: str, value: Any) -> None:
        """
    ...
    def list_profiles(self) -> Dict[str, str]:
        ...
        """
        config = self.get_config()
        return {name: profile.description or "" for name, profile in config.profiles.items()}
    ...
    def update_profile_provider_settings(self, profile_name: str, provider_name: str, **kwargs) -> None:
        """
        ...
        if provider_name not in profile.providers:
            profile.providers[provider_name] = self.create_provider_config(provider_name)
        ...
        try:
            ...
            if 'providers' in config_data:
                for provider_name, provider_data in config_data['providers'].items():
                    ...
                    defaults = self.get_provider_defaults(provider_name)
                    for key in ['supports_tools', 'supports_multimodal', 'supports_streaming']:
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        for provider_name, provider_config in config.providers.items():
            provider_capabilities[provider_name] = {
                ...
                'supports_multimodal': getattr(provider_config, 'supports_multimodal', False),
                'supports_streaming': getattr(provider_config, 'supports_streaming', True),
                'provider_type': getattr(provider_config, 'provider_type', provider_name)
            }
    ...
    def create_profile(self, name: str, description: Optional[str] = None, copy_from_current: bool = True) -> None:
        """
        Create a new configuration profile.
        ...
        if 'providers' in raw_config:
            for provider_name, provider_config in raw_config['providers'].items():
                ...
                if 'supports_tools' not in provider_config:
                    defaults = self.get_provider_defaults(provider_name)
                    provider_config['supports_tools'] = defaults.get('supports_tools', False)
                    provider_config['supports_multimodal'] = defaults.get('supports_multimodal', False)
                    provider_config['supports_streaming'] = defaults.get('supports_streaming', True)
                    migrated = True
    ...
    def restore_config(self, backup_path: str) -> None:
        ...
        try:
            ...
            if self.config_path.exists():
                ...
                current_backup_path = self.config_path.parent / f"config_pre_restore_{timestamp}_{millis}.json"
                import shutil
        ...
        try:
            ...
            for category, errors in validation_results.items():
                if isinstance(errors, list):
                    ...
                    category_total = sum(len(provider_errors) for provider_errors in errors.values())
            ...
            for provider_name in config.providers.keys():
                provider_health[provider_name] = {
                    ...
                    'health_check_enabled': config.providers[provider_name].health_check_enabled
                }
