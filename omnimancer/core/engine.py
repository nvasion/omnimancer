...
from ..core.models import ChatResponse, ModelInfo, EnhancedModelInfo
from ..utils.errors import ConfigurationError, ProviderError
from ..ui.progress_indicator import get_progress_indicator, OperationType
...
class CoreEngine:
    ...
    async def switch_model(self, provider_name: str, model_name: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if model_name:
                ...
                if model_name not in all_model_names:
                    raise ConfigurationError(f"Model '{model_name}' not available for provider '{provider_name}'")
            ...
            logger.info(f"Switched to provider: {provider_name}, model: {provider.model}")
            return True
    ...
    async def send_message(self, message: str) -> ChatResponse:
        ...
        try:
            ...
            if progress and progress.enabled:
                progress.start_operation("engine_context", OperationType.ANALYZE, "Getting chat context")
            ...
            if progress and progress.enabled:
                progress.start_operation("engine_provider", OperationType.NETWORK, f"Sending to {self.current_provider.get_provider_name()}")
            response = await self.current_provider.send_message(message, context)
            if progress and progress.enabled:
                progress.complete_operation("engine_provider", "completed" if response.is_success else "failed")
            ...
            if response.is_success:
                if progress and progress.enabled:
                    progress.start_operation("engine_history", OperationType.WRITE, "Updating chat history")
                self.chat_manager.add_assistant_message(response.content, self.current_provider.model)
                if progress and progress.enabled:
    ...
    def get_available_models(self) -> List[ModelInfo]:
        ...
        for provider in self.providers.values():
            try:
                ...
                logger.warning(f"Failed to get models from provider {provider.get_provider_name()}: {e}")
            ...
            try:
                ...
                for model in models:
                    model_dict = {
                        ...
                        'supports_multimodal': getattr(model, 'supports_multimodal', False),
            ...
            return {
                ...
                'providers': {
                    name: {
                        ...
                        'api_key': f"{provider_config.api_key[:8]}***" if provider_config.api_key else "Not set"
                    }
                    for name, provider_config in config.providers.items()
                },
                'current_provider': self.current_provider.get_provider_name() if self.current_provider else None,
                'current_model': self.current_provider.model if self.current_provider else None
            }
        except Exception as e:
    ...
    def get_current_model_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'supports_multimodal': getattr(model_info, 'supports_multimodal', False),
    ...
    async def check_provider_health(self, provider_name: Optional[str] = None, force: bool = False) -> Dict[str, Any]:
        """
    ...
    async def _mcp_status(self) -> str:
        ...
        status_info.append(f"Initialized: {'Yes' if self.mcp_manager.initialized else 'No'}")
        status_info.append(f"Connected Servers: {self.mcp_manager.connected_server_count}")
    ...
    async def _mcp_health(self) -> str:
        ...
        try:
            ...
            for server_name, status in server_status.items():
                ...
                health_info.append(f"   Enabled: {'Yes' if status['enabled'] else 'No'}")
                health_info.append(f"   Connected: {'Yes' if status['connected'] else 'No'}")
