...
class TerminalCapabilities:
    ...
    def detect(cls) -> 'TerminalCapabilities':
        ...
        supports_color = (
            ...
            'color' in os.environ.get('TERM', '').lower() or
        ...
        try:
            '✓'.encode(os.environ.get('LANG', 'ascii').split('.')[1] if '.' in os.environ.get('LANG', '') else 'utf-8')
        except (UnicodeEncodeError, LookupError):
            supports_unicode = False
        
        # Detect emoji support (simplified)
        supports_emoji = supports_unicode and os.name != 'nt'  # Windows terminal emoji support is limited
...
class RichTextRenderer:
    ...
    def render_code_block(self, 
                         ...
                         highlight_lines: Optional[List[int]] = None) -> Syntax:
        """
    ...
    def render_table(self,
        ...
        table = Table(
            ...
            width=None if self.capabilities.width > 120 else self.capabilities.width - 2
        )
        ...
        for row in rows:
            ...
            for i, value in enumerate(row):
                if isinstance(value, bool):
                    styled_value = Text("✓" if value else "✗", style="success" if value else "error")
                elif isinstance(value, (int, float)) and i > 0:  # Numeric columns (not first)
    ...
    def render_risk_indicator(self, 
                             risk_level: Union[RiskLevel, int],
                             show_label: bool = True,
                             show_bar: bool = True) -> Union[Text, Columns]:
        """
        ...
        if show_label:
            ...
            risk_text.append(
                risk_enum.value.upper(),
                style=self.get_risk_color(risk_enum)
            )
    ...
    def render_progress_bar(self,
                           ...
                           completed: Optional[int] = None) -> Progress:
        """
    ...
    def create_responsive_layout(self, 
                                 ...
                                 vertical_threshold: int = 100) -> Union[Columns, Layout]:
        """
    ...
    def render_shortcuts_help(self, 
                             ...
                             columns: int = 2) -> Union[Table, Columns]:
        """
        ...
        if self.capabilities.width < 60:
            ...
            # Split into columns
            items_per_col = len(shortcut_texts) // columns + (1 if len(shortcut_texts) % columns else 0)
            columns_list = []
...
def render_risk_badge(risk_level: Union[int, str], renderer: Optional[RichTextRenderer] = None) -> Text:
...
def render_operation_badge(operation_type: str, renderer: Optional[RichTextRenderer] = None) -> Text:
    """Quick function to render an operation type badge."""
    renderer = renderer or create_renderer()
    
    text = Text()
    text.append("⚡ ", style=renderer.get_operation_color(operation_type))
    text.append(operation_type.replace('_', ' ').title(), style=renderer.get_operation_color(operation_type))
