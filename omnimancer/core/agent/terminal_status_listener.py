...
from .status_core import (
    StatusStreamListener, StatusStreamEvent, StreamPriority,
    AgentEvent, EventType, OperationStatus, AgentStatus
)
            ...
            if event.event_type == EventType.AGENT_STATE_CHANGED:
                ...
                if event.agent_id and 'new_status' in event.data:
                    try:
                        ...
                        self.stats['agents_monitored'] = len(set(self.agent_statuses.keys()))
                    except ValueError:
                        logger.warning(f"Invalid agent status: {event.data['new_status']}")
                ...
                if event.operation_id and event.operation_id in self.active_operations:
                    self.active_operations[event.operation_id].update({
                        ...
                        'description': event.data.get('description', self.active_operations[event.operation_id]['description'])
                    })
            
            elif event.event_type in [EventType.OPERATION_COMPLETED, EventType.OPERATION_FAILED, EventType.OPERATION_CANCELLED]:
                # Complete operation
                if event.operation_id and event.operation_id in self.active_operations:
                    self.active_operations[event.operation_id].update({
                        'status': 'completed' if event.event_type == EventType.OPERATION_COMPLETED else 'failed',
                        'end_time': event.timestamp,
                        'progress': 100.0 if event.event_type == EventType.OPERATION_COMPLETED else self.active_operations[event.operation_id]['progress']
                    })
    ...
    def _generate_operations_section(self) -> str:
        ...
        if not active_ops:
            ...
            for op in active_ops[:5]:  # Show max 5 operations
                ...
                description = op['description'][:50] + "..." if len(op['description']) > 50 else op['description']
...
async def create_terminal_status_display(status_stream: 'AsyncStatusStream') -> TerminalStatusListener:
    """
