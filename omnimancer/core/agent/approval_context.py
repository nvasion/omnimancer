...
from .rich_renderer import RiskLevel, OperationType
...
class OperationDetails:
    ...
    def __post_init__(self):
        ...
        self.files_affected = [Path(f) if not isinstance(f, Path) else f 
                              for f in self.files_affected]
    
    def add_file_change(self, path: Union[str, Path], operation: FileOperationType, 
...
class ApprovalContext:
    ...
    # Operation details
    operation_details: OperationDetails = field(default_factory=lambda: OperationDetails(operation_type="unknown"))
    ...
    def set_decision(self, decision: ApprovalDecision) -> None:
        ...
        self.status = OperationStatus.APPROVED if decision.approved else OperationStatus.DENIED
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'operation_details': {
                ...
                'files_affected': [str(f) for f in self.operation_details.files_affected],
...
def create_command_execution_context(
    ...
    if any(sys_path in ' '.join([command] + arguments) for sys_path in ['/etc', '/usr/bin', '/sys']):
    ...
    if any(exec_cmd in command for exec_cmd in ['eval', 'exec', 'python', 'bash', 'sh']):
