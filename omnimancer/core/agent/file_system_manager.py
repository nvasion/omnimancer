...
from typing import Dict, List, Optional, Union, Any, Tuple, AsyncGenerator, Callable
...
class FileSystemManager:
    ...
    def _create_operation(self, 
                         ...
                         description: str, 
                         data: Dict[str, Any],
                         reversible: bool = False) -> Operation:
    ...
    async def check_file_exists(self, 
                               path: Union[str, Path],
                               follow_symlinks: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            resolved_path = original_path.resolve() if follow_symlinks else original_path
            ...
            try:
                security_result = await self.security.secure_file_access(str(resolved_path), "read")
                if not security_result['success']:
                    ...
                    if 'does not exist' in error_msg.lower() or 'not found' in error_msg.lower():
                        ...
                        logger.debug(f"SecurityManager doesn't recognize {resolved_path}, continuing with existence check")
                    else:
                ...
                logger.warning(f"Security check failed for {resolved_path}: {security_error}")
                # Continue with existence check despite security error
                logger.debug(f"Continuing existence check despite security error: {security_error}")
            ...
            try:
                ...
                return {
                    'exists': True,
                    'path': str(original_path),  # Return original path to preserve symlink paths
                    'is_file': is_file,
                ...
                logger.warning(f"Could not get metadata for {resolved_path}: {os_error}")
                return {
                    'exists': True,
                    'path': str(original_path),  # Return original path to preserve symlink paths
                    'is_file': False,
    ...
    async def write_file_with_confirmation(self,
                                          path: Union[str, Path],
                                          ...
                                          user_review_callback: Optional[Callable] = None) -> Dict[str, Any]:
        """
        ...
        """
        original_path = Path(path)
        operation_id = f"write_confirm_{hashlib.md5(str(original_path).encode()).hexdigest()[:8]}"
        ...
        try:
                ...
                try:
                    ...
                    if user_action == 'backup':
                        ...
                        return {
                            ...
                            'operation_id': operation_id,
                            'path': str(original_path),
                            'reason': confirmation_result.get('reason', 'User cancelled'),
                            'user_decision': confirmation_result,
                            'file_exists': True
                        }
            ...
            if file_info['exists'] and file_info.get('is_symlink', False):
                ...
                logger.debug(f"Removing symlink at {original_path} before writing new file")
            ...
            write_result.update({
                ...
                'confirmation_requested': file_info['exists'] and confirmation_callback is not None,
                'user_confirmed': file_info['exists'] and confirmation_callback is not None
            })
    ...
    async def write_file(self,
        ...
        if content is None:
            raise FileOperationError(f"Content cannot be None for write operation to {path}")
        ...
        operation_description = f"{'Modify' if file_exists else 'Create'} file: {path.name}"
        ...
        if content is None:
            ...
            content_str = decoded_content[:1000] + "..." if len(decoded_content) > 1000 else decoded_content
            content_length = len(content)
        ...
        operation = self._create_operation(
            ...
            reversible=file_exists and backup  # Reversible if we have backup capability
        )
        
        # Request approval if needed
        approved = await self._request_approval_if_needed(operation)
        if not approved:
            raise FileOperationError(f"File write operation denied by approval workflow: {path}")
        ...
        try:
            ...
            try:
                if operation_id in self.active_operations and 'temp_path' in self.active_operations[operation_id]:
    ...
    async def _execute_write_operation(self, path: Union[str, Path], content: Union[str, bytes], 
                                     encoding: str = 'utf-8', backup: bool = True, 
                                     atomic: bool = True, operation_id: str = None,
        ...
        try:
            # Security validation
            security_result = await self.security.secure_file_access(str(path), "write", content if isinstance(content, str) else None)
            if not security_result['success']:
                raise FileOperationError(f"Security check failed: {security_result['error']}")
            ...
            if backup and await aiofiles.os.path.exists(path):
                try:
                    ...
                    if operation_id:
                        self.active_operations[operation_id]['backup_path'] = backup_path
                except Exception as e:
            ...
            if backup and operation_id and 'backup_path' in self.active_operations[operation_id]:
    ...
    async def _perform_direct_write(self, path: Union[str, Path], content: Union[str, bytes], 
                                   encoding: str = 'utf-8', backup: bool = True, 
    ...
    async def restore_backup(self, backup_path: Union[str, Path], target_path: Union[str, Path]) -> bool:
    ...
    async def create_directory(self, path: Union[str, Path], parents: bool = True) -> bool:
        ...
        approved = await self._request_approval_if_needed(operation)
        if not approved:
            raise FileOperationError(f"Directory creation denied by approval workflow: {path}")
        
        # Security validation
        perm_operation = PermissionOperation(operation_type="file_create", path=str(path))
        validation = await self.security.validate_operation(perm_operation)
        
        if not validation['allowed']:
            raise FileOperationError(f"Directory creation blocked: {', '.join(validation['reasons'])}")
    ...
    async def delete_file(self, path: Union[str, Path], backup: bool = True) -> Dict[str, Any]:
        ...
        approved = await self._request_approval_if_needed(operation)
        if not approved:
            raise FileOperationError(f"File delete operation denied by approval workflow: {path}")
    ...
    async def copy_file(self, src: Union[str, Path], dst: Union[str, Path]) -> bool:
        ...
        if not read_result['success']:
            raise FileOperationError(f"Source read access denied: {read_result['error']}")
        
        # Validate destination write access
        write_result = await self.security.secure_file_access(str(dst), "write")
        if not write_result['success']:
            raise FileOperationError(f"Destination write access denied: {write_result['error']}")
    ...
    async def move_file(self, src: Union[str, Path], dst: Union[str, Path], backup: bool = True) -> Dict[str, Any]:
        ...
        approved = await self._request_approval_if_needed(operation)
        if not approved:
            raise FileOperationError(f"File move operation denied by approval workflow: {src} → {dst}")
    ...
    async def get_file_info(self, path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            if path.is_file() and stat_result.st_size < 10 * 1024 * 1024:  # Only for files < 10MB
    ...
    async def list_directory(self, path: Union[str, Path], recursive: bool = False) -> List[Dict[str, Any]]:
    ...
    async def _atomic_write(self, path: Path, content: Union[str, bytes], encoding: str) -> Dict[str, Any]:
    ...
    async def _direct_write(self, path: Path, content: Union[str, bytes], encoding: str) -> Dict[str, Any]:
    ...
    async def _read_large_file(self, path: Path, binary: bool, encoding: str) -> Union[str, bytes]:
    ...
    async def git_status(self, path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    ...
    @asynccontextmanager
    async def streaming_read(self, path: Union[str, Path], chunk_size: Optional[int] = None):
    ...
    def clear_completed_operations(self) -> int:
        ...
        completed = [op_id for op_id, op in self.active_operations.items() 
    ...
    async def read_before_write(self, 
                               path: Union[str, Path],
    ...
    async def _read_before_write_with_recovery(self,
                                              path: Union[str, Path],
                                              new_content: Union[str, bytes],
                                              encoding: str = 'utf-8',
                                              user_review_callback: Optional[Callable] = None,
        ...
        """
        path = Path(path).resolve()
        operation_type = 'create' if not await aiofiles.os.path.exists(path) else 'modify'
        ...
        try:
            ...
            if file_exists:
                try:
                    ...
                    error_result = self.error_handler.handle_error(read_error, retry_count, max_retries)
                    
                    if error_result.get('should_retry'):
                        return await self._read_before_write_with_recovery(
                            path, new_content, encoding, user_review_callback, max_retries, retry_count + 1
                        )
                    elif error_result.get('fallback_action') == 'regular_write':
                        logger.warning(f"Falling back to regular write for {path} due to read error")
                        return await self._perform_direct_write(path=path, content=new_content, encoding=encoding)
            ...
            review_data = {
                ...
                'new_content': new_content if isinstance(new_content, str) else new_content.decode(encoding, errors='ignore'),
                'encoding': encoding,
            ...
            try:
                if current_content and not current_content.startswith('<Error reading file:'):
                ...
                error_result = self.error_handler.handle_error(diff_error, retry_count, max_retries)
            ...
            if user_review_callback:
                try:
                    ...
                    error_result = self.error_handler.handle_error(callback_error, retry_count, max_retries)
                    ...
                    if error_result.get('should_retry'):
                        ...
                        logger.warning(f"Falling back to regular write for {path} due to callback error")
                        return await self._perform_direct_write(path=path, content=new_content, encoding=encoding)
                    else:
                        return {
                            'success': False,
            ...
            if user_decision.get('cancelled'):
                ...
                error_result = self.error_handler.handle_error(rejection_error, retry_count, max_retries)
            ...
            if user_decision.get('modified_content') is not None:
                try:
                    ...
                    content_error = ContentValidationError(str(path), str(validation_error))
                    error_result = self.error_handler.handle_error(content_error, retry_count, max_retries)
            ...
            try:
                ...
                write_result['content_modified_by_user'] = user_decision.get('modified_content') is not None
                ...
                error_result = self.error_handler.handle_error(write_error, retry_count, max_retries)
            ...
            logger.error(f"Unexpected error in read_before_write for {path}: {e}", exc_info=True)
            return {
                'success': False,
        ...
        if isinstance(content, str):
            ...
            for pattern in suspicious_patterns:
                if pattern in content_lower:
                    logger.warning(f"Potentially suspicious content pattern detected: {pattern}")
    ...
    def _generate_content_diff(self, 
                              current_content: str, 
                              new_content: str, 
                              file_path: str) -> str:
    ...
    async def preview_file_modification(self, 
                                       path: Union[str, Path],
                                       new_content: Union[str, bytes],
        ...
        preview = {
            ...
            'new_size': len(new_content) if isinstance(new_content, str) else len(new_content.decode(encoding, errors='ignore')),
            'encoding': encoding
        }
        
        # Generate diff if both contents exist
        if current_content:
            new_content_str = new_content if isinstance(new_content, str) else new_content.decode(encoding, errors='ignore')
    ...
    async def is_git_repository(self, path: Optional[Union[str, Path]] = None) -> bool:
        ...
        check_path = Path(path).resolve() if path else self.get_current_working_directory()
    ...
    async def get_git_repository_root(self, path: Optional[Union[str, Path]] = None) -> Optional[Path]:
    ...
    async def get_directory_context(self, path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
        """Get comprehensive directory context including working directory and repository status."""
        
        target_path = Path(path).resolve() if path else self.get_current_working_directory()
        ...
        if context['is_git_repository']:
            ...
            if repo_root:
                ...
                try:
                    context['relative_to_repo_root'] = str(target_path.relative_to(repo_root))
                except ValueError:
    ...
    async def create_file(self, path: Union[str, Path], content: Union[str, bytes] = "", encoding: str = 'utf-8', **kwargs) -> Dict[str, Any]:
        """Create a new file with the given content (test compatibility wrapper)."""
        return await self.write_file(path=path, content=content, encoding=encoding, **kwargs)
    
    async def modify_file(self, path: Union[str, Path], content: Union[str, bytes], encoding: str = 'utf-8', **kwargs) -> Dict[str, Any]:
