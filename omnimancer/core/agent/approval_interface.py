...
from .approval_manager import (
    EnhancedApprovalManager, ChangePreview, BatchApprovalRequest, 
    PreviewFormat, ChangeType
)
...
class ApprovalInterface:
    ...
    async def handle_single_approval(self, approval_data: Dict[str, Any]) -> bool:
        ...
        self._print_header(f"Approval Request - {operation.type.value.replace('_', ' ').title()}")
    ...
    async def handle_batch_approval(self, batch_request: BatchApprovalRequest) -> Dict[str, Any]:
        """
        ...
        """
        self._print_header(f"Batch Approval Request - {len(batch_request.operations)} Operations")
        ...
        if approval_mode == "all_at_once":
            ...
            return {'deny_all': True, 'reason': 'User cancelled batch approval'}
    
    async def _handle_bulk_approval(self, batch_request: BatchApprovalRequest) -> Dict[str, Any]:
        ...
        if choice == ApprovalChoice.APPROVE_ALL:
            self._print_success(f"✓ All {len(batch_request.operations)} operations approved")
            return {'approve_all': True}
        elif choice == ApprovalChoice.DENY_ALL:
            reason = input("Reason for denial (optional): ").strip()
            self._print_warning(f"✗ All {len(batch_request.operations)} operations denied")
            return {'deny_all': True, 'reason': reason or 'User denied all operations'}
        else:
            return {'deny_all': True, 'reason': 'User cancelled'}
    
    async def _handle_individual_approval(self, batch_request: BatchApprovalRequest) -> Dict[str, Any]:
    ...
    async def _handle_filtered_approval(self, batch_request: BatchApprovalRequest) -> Dict[str, Any]:
        ...
        if choice == "1":
            ...
            for i, preview in enumerate(batch_request.previews):
                if preview.risk_assessment and "low" in preview.risk_assessment.lower():
                    approved_indices.append(i)
            self._print_success(f"✓ Approved {len(approved_indices)} low-risk operations")
            ...
            for i, operation in enumerate(batch_request.operations):
                if operation.type in [OperationType.FILE_READ]:
                    approved_indices.append(i)
            self._print_success(f"✓ Approved {len(approved_indices)} read-only operations")
            ...
            type_choice = input("Select operation types to approve (comma-separated numbers): ").strip()
            try:
                selected_indices = [int(x.strip()) - 1 for x in type_choice.split(",")]
                selected_types = [operation_types[i] for i in selected_indices if 0 <= i < len(operation_types)]
                
                for i, operation in enumerate(batch_request.operations):
                    if operation.type in selected_types:
                        approved_indices.append(i)
                
                self._print_success(f"✓ Approved {len(approved_indices)} operations of selected types")
            ...
            indices_input = input(f"Operations 1-{len(batch_request.operations)}: ").strip()
            try:
                approved_indices = [int(x.strip()) - 1 for x in indices_input.split(",")]
                approved_indices = [i for i in approved_indices if 0 <= i < len(batch_request.operations)]
                self._print_success(f"✓ Approved {len(approved_indices)} selected operations")
            except ValueError:
    ...
    async def _get_user_choice(self, available_choices: List[ApprovalChoice]) -> ApprovalChoice:
        ...
        while True:
            ...
            if user_input in choice_map:
                ...
                if choice in available_choices:
                    ...
                    self._print_error(f"'{user_input}' is not available in this context")
            else:
                self._print_error(f"Invalid choice: '{user_input}'")
    
    async def _show_detailed_view(self, 
                                operation: Operation, 
    ...
    def _print_operation_info(self, operation: Operation, risk_level: Optional[RiskLevel] = None):
