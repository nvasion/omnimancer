...
from ..security.approval_workflow import ApprovalWorkflow, ApprovalRequest, ApprovalStatus, RiskLevel
...
class ChangePreview:
    ...
    def _format_html_preview(self) -> str:
        ...
        if self.risk_assessment:
            html_parts.append(f"<p><strong>Risk:</strong> <span class='risk'>{self.risk_assessment}</span></p>")
        
        html_parts.append(f"<p><strong>Reversible:</strong> {'Yes' if self.reversible else 'No'}</p>")
...
class BatchApprovalRequest:
    ...
    approved_operations: Set[int] = field(default_factory=set)  # Indices of approved operations
...
class EnhancedApprovalManager:
            ...
            if self.approval_callback:
                ...
                if user_approved:
                    ...
                    self.approval_workflow.deny_request(
                        approval_request.id,
                        "user",
                        "User denied through UI"
                    )
                    self._record_approval_history(operation, approval_request, preview, approved=False)
                    return False
    ...
    async def request_batch_approval(self, operations: List[Operation]) -> BatchApprovalRequest:
        """
    ...
    def _process_batch_approval_result(self, 
        ...
        if approval_result.get('approve_all'):
            ...
            if len(batch_request.approved_operations) == len(batch_request.operations):
                ...
                batch_request.status = ApprovalStatus.APPROVED  # Partial approval still counts as approved
    ...
    async def _generate_file_write_preview(self, operation: Operation) -> ChangePreview:
        ...
        preview = ChangePreview(
            change_type=ChangeType.FILE_CREATE if not current_content else ChangeType.FILE_MODIFY,
    ...
    async def _generate_file_delete_preview(self, operation: Operation) -> ChangePreview:
        ...
        try:
            ...
            logger.debug(f"Could not read content of {file_path} for deletion preview: {e}")
    ...
    async def _generate_directory_create_preview(self, operation: Operation) -> ChangePreview:
    ...
    async def _generate_directory_delete_preview(self, operation: Operation) -> ChangePreview:
    ...
    async def _generate_web_request_preview(self, operation: Operation) -> ChangePreview:
    ...
    def _assess_file_write_risk(self, file_path: str, content: str) -> str:
        ...
        if any(pattern in path_lower for pattern in ['.env', 'password', 'secret', 'key', 'credential']):
            risk_factors.append("sensitive file")
        
        if any(pattern in path_lower for pattern in ['config', 'settings', '.ssh', '/etc']):
    ...
    def _assess_file_delete_risk(self, file_path: str) -> str:
        ...
        if any(pattern in file_path.lower() for pattern in ['.env', 'config', 'settings', 'backup']):
    ...
    def _assess_command_risk(self, command: str) -> str:
        ...
        if any(dangerous in command_lower for dangerous in ['rm -rf', 'sudo', 'chmod 777']):
            return "Critical - Dangerous command"
        elif any(risky in command_lower for risky in ['rm', 'mv', 'cp', 'chmod', 'chown']):
    ...
    def _record_approval_history(self, 
                                operation: Operation, 
