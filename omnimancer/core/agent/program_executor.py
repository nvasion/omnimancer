...
class ExecutionConfig:
    ...
    stream_callback: Optional[Callable[[str, str], None]] = None  # (stream_type, content)
    ...
    def __init__(self):
        self.command_categories = {
            CommandCategory.SAFE_READ: {
                ...
                'diff', 'less', 'more', 'file', 'stat', 'du', 'tree', 'echo'
            },
            CommandCategory.DEVELOPMENT: {
                'git', 'python', 'python3', 'node', 'npm', 'yarn', 'pip', 'pip3',
                'make', 'cmake', 'javac', 'java', 'gcc', 'g++', 'rustc', 'cargo',
                'go', 'mvn', 'gradle', 'composer'
            },
            CommandCategory.SYSTEM_INFO: {
                'ps', 'top', 'htop', 'df', 'free', 'uptime', 'whoami', 'id',
                'uname', 'which', 'whereis', 'env', 'printenv', 'pwd', 'date', 'sleep'
            },
            CommandCategory.BUILD_TOOLS: {
                'docker', 'docker-compose', 'podman', 'buildah', 'kubernetes',
                'kubectl', 'helm', 'terraform', 'ansible'
            },
            CommandCategory.PACKAGE_MANAGERS: {
                'apt', 'apt-get', 'yum', 'dnf', 'pacman', 'brew', 'choco',
                'snap', 'flatpak', 'conda', 'mamba'
            },
            CommandCategory.DANGEROUS: {
                'rm', 'rmdir', 'mv', 'cp', 'chmod', 'chown', 'chgrp', 'sudo', 'su',
                'passwd', 'dd', 'fdisk', 'parted', 'mkfs', 'mount', 'umount',
                'systemctl', 'service', 'kill', 'killall', 'pkill'
            },
            CommandCategory.NETWORK: {
                'curl', 'wget', 'ssh', 'scp', 'rsync', 'nc', 'netcat', 'telnet',
                'ftp', 'sftp', 'ping', 'traceroute', 'nslookup', 'dig'
            }
        }
...
class StreamingExecutor:
    ...
    async def stream_process_output(self, process: asyncio.subprocess.Process) -> None:
        ...
        async def read_stream(stream: asyncio.StreamReader, stream_type: str, buffer: List[str]):
            try:
                while True:
...
class EnhancedProgramExecutor:
    ...
    async def execute_command(self, 
                            command: str, 
                            args: List[str] = None,
                            config: ExecutionConfig = None) -> CommandResult:
        """
        ...
        try:
            ...
            if not self.validator.is_command_allowed(command, config.execution_mode):
                raise SecurityError(f"Command '{command}' not allowed in {config.execution_mode.value} mode")
            ...
            if config.require_approval and self.approval_workflow:
                approved = await self._request_approval(command, args, risk_level, config)
                if not approved:
            ...
            if config.execution_mode == ExecutionMode.SANDBOX:
                result = await self._execute_sandboxed(command, sanitized_args, config, execution_id)
            else:
    ...
    async def _execute_direct(self, 
                            command: str, 
                            args: List[str], 
                            config: ExecutionConfig) -> CommandResult:
        ...
        try:
            ...
            if config.enable_streaming:
                ...
                stream_task = asyncio.create_task(streaming_executor.stream_process_output(process))
    ...
    async def _execute_sandboxed(self, 
                               ...
                               args: List[str], 
                               config: ExecutionConfig,
                               execution_id: str) -> CommandResult:
    ...
    async def stream_command_output(self, 
                                  command: str, 
                                  args: List[str] = None,
                                  config: ExecutionConfig = None) -> AsyncIterator[tuple[str, str]]:
        """
        ...
        execution_task = asyncio.create_task(self.execute_command(command, args, config))
        
        # Yield output as it becomes available
        try:
            while not execution_task.done():
                try:
                    stream_type, content = await asyncio.wait_for(output_queue.get(), timeout=0.1)
