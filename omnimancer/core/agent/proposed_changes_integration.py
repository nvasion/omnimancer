...
from .approval_manager import EnhancedApprovalManager, ChangePreview, ChangeType, PreviewFormat
...
class ProposedChangesIntegration:
    ...
    async def fetch_proposed_changes(self,
                                    operation_id: str,
                                    file_paths: Optional[List[str]] = None) -> ChangeSet:
        """
        ...
        try:
            ...
            if hasattr(self.approval_manager, 'get_pending_operations'):
                operations = await self.approval_manager.get_pending_operations(operation_id)
            else:
                operations = []
    ...
    async def display_proposed_changes(self,
                                      change_set: ChangeSet,
    ...
    async def display_inline_changes(self,
                                    file_path: str,
    ...
    async def display_side_by_side_changes(self,
                                          file_path: str,
                                          original_content: str,
                                          modified_content: str) -> None:
        """
    ...
    async def apply_proposed_changes(self,
                                    change_set_id: str,
        ...
        try:
            ...
            if selected_changes is not None:
                changes_to_apply = [change_set.changes[i] for i in selected_changes 
    ...
    def get_change_statistics(self, change_set: ChangeSet) -> Dict[str, Any]:
        ...
        for change in change_set.changes:
            # Count operations
            op_type = change.operation_type.value if change.operation_type else 'unknown'
    ...
    async def _create_proposed_change(self, operation: Dict[str, Any]) -> ProposedChange:
        ...
        if original_content and modified_content:
            line_changes = self._calculate_line_changes(original_content, modified_content)
    ...
    async def _display_single_change(self, change: ProposedChange, display_mode: ChangeDisplayMode):
    ...
    async def _get_change_approval(self, change_set: ChangeSet) -> Dict[str, Any]:
        ...
        if choice == "A":
            ...
            return {'approved': len(selected) > 0, 'selected_changes': selected}
        elif choice == "D":
    ...
    async def _select_specific_changes(self, change_set: ChangeSet) -> List[int]:
        ...
        selection = Prompt.ask(
            "Enter change numbers to approve (comma-separated)",
            default=""
        )
    ...
    async def _apply_single_change(self, change: ProposedChange) -> Dict[str, Any]:
        ...
        try:
            if change.operation_type == ChangeType.FILE_CREATE:
                ...
                return {'success': False, 'error': f'Unsupported operation: {change.operation_type}'}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def _create_risk_distribution_bar(self, risk_distribution: Dict[str, int]) -> str:
        ...
        for risk in ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']:
            ...
            if count > 0:
                ...
                bar_parts.append(f"[{color}]{risk}: {count} ({percentage:.0f}%)[/{color}]")
