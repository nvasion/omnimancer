...
from .status_core import AgentStatus, OperationType, OperationStatus, EventType, AgentEvent, AgentOperation
...
class AgentStatusTable:
    ...
    def create_table(self, agent_statuses: Dict[str, AgentStatus], 
                    agent_metadata: Dict[str, Dict[str, Any]] = None) -> Table:
        """
...
class OperationProgress:
    ...
    def create_progress_display(self, operations: List[AgentOperation]) -> Union[Progress, Text]:
        """
        ...
        return self.progress if self.progress.task_ids else Text("No active operations", style="dim")
...
class EventLogPanel:
    ...
    def create_event_log(self, events: List[AgentEvent], title: str = "Recent Events") -> Panel:
        """
...
class SystemStatsPanel:
    ...
    def create_stats_panel(self, stats: Dict[str, Any], title: str = "System Statistics") -> Panel:
        """
...
class OperationDetailsTree:
    ...
    def create_operation_tree(self, operations: List[AgentOperation], 
                             title: str = "Operations") -> Panel:
        """
        ...
        """
        if not operations:
            return Panel(Text("No operations to display", style="dim"), 
        ...
        for agent_id, agent_operations in by_agent.items():
            # Add agent branch
            agent_branch = tree.add(f"[cyan]{agent_id}[/cyan] ({len(agent_operations)} operations)")
            ...
            if len(agent_operations) > 5:
                agent_branch.add(f"[dim]... and {len(agent_operations) - 5} more operations[/dim]")
...
class ComprehensiveStatusDisplay:
    ...
    def create_comprehensive_display(self,
        ...
        layout["left"].split_column(
            Layout(self.agent_table.create_table(agent_statuses, agent_metadata), name="agents"),
            Layout(self.operation_progress.create_progress_display(active_operations), name="progress")
        )
        ...
        layout["bottom_right"].split_row(
            ...
            Layout(self.operation_tree.create_operation_tree(active_operations), name="tree")
        )
    ...
    def create_simple_display(self,
                            agent_statuses: Dict[str, AgentStatus],
                            recent_events: List[AgentEvent],
                            system_stats: Dict[str, Any]) -> Group:
        """
...
def create_agent_status_summary(agent_statuses: Dict[str, AgentStatus]) -> Text:
    ...
    enabled = sum(1 for status in agent_statuses.values() if status == AgentStatus.ENABLED)
