...
class SecuritySettings:
    ...
    allowed_commands: List[str] = field(default_factory=lambda: [
        'git', 'npm', 'pip', 'python', 'node', 'make', 'cargo', 'go'
    ])
    restricted_paths: List[str] = field(default_factory=lambda: [
        '.ssh', '.env', '.aws', '.config/gcloud', '/etc', '/System'
    ])
...
class UserPreferences:
    ...
    auto_approve_types: List[str] = field(default_factory=lambda: [
...
class AgentConfig:
    ...
    def _get_config_path(self, section: str, format: ConfigFormat = ConfigFormat.JSON) -> Path:
    ...
    def _save_config_file(self, path: Path, data: Dict[str, Any], format: ConfigFormat = ConfigFormat.JSON) -> None:
    ...
    def _save_all_configs(self) -> None:
        ...
        if 'security' in self.dirty_sections or not self._get_config_path('security').exists():
            security_data = asdict(self.security)
            self._save_config_file(self._get_config_path('security'), security_data)
        
        # Save agent settings
        if 'agent' in self.dirty_sections or not self._get_config_path('agent').exists():
            agent_data = asdict(self.agent)
            self._save_config_file(self._get_config_path('agent'), agent_data)
        
        # Save filesystem settings
        if 'filesystem' in self.dirty_sections or not self._get_config_path('filesystem').exists():
        ...
        if 'webclient' in self.dirty_sections or not self._get_config_path('webclient').exists():
            webclient_data = asdict(self.web_client)
            self._save_config_file(self._get_config_path('webclient'), webclient_data)
        
        # Save user preferences
        if 'preferences' in self.dirty_sections or not self._get_config_path('preferences').exists():
            preferences_data = asdict(self.user_preferences)
            self._save_config_file(self._get_config_path('preferences'), preferences_data)
        
        # Save provider configurations
        if 'providers' in self.dirty_sections or not self._get_config_path('providers').exists():
            providers_data = {}
    ...
    def export_config(self, export_path: Path, format: ConfigFormat = ConfigFormat.JSON, include_sensitive: bool = False) -> None:
        ...
        for provider_id, provider_config in self.providers.items():
            ...
            if not include_sensitive:
                ...
                provider_dict['custom_headers'] = {
                    k: '[REDACTED]' if any(sensitive in k.lower() for sensitive in ['key', 'token', 'authorization', 'secret']) else v
    ...
    def import_config(self, import_path: Path, merge: bool = True) -> None:
        ...
        if imported_version != self.CONFIG_VERSION:
            logging.warning(f"Configuration version mismatch: {imported_version} vs {self.CONFIG_VERSION}")
        ...
        return {
            ...
            'sections': {
                ...
                'providers': {
                    ...
                    'types': list(set(p.provider_type.value for p in self.providers.values()))
                }
            }
        }
    ...
    async def config_transaction(self):
        ...
        try:
            ...
            self.user_preferences = UserPreferences(**checkpoint_data['user_preferences'])
