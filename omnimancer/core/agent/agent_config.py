...
from .config import ProviderType, SecuritySettings, AgentSettings, FileSystemSettings, WebClientSettings
...
class BehaviorRules:
    ...
    require_confirmation_for: List[str] = field(default_factory=lambda: ["file_delete", "system_commands"])
...
class CustomAgentConfig:
    ...
    capabilities: Set[PersonaCapability] = field(default_factory=lambda: {
        PersonaCapability.GENERAL_PURPOSE
    })
    enabled_tools: Set[AgentTool] = field(default_factory=lambda: {
    ...
    # Metadata
    metadata: PersonaMetadata = field(default_factory=lambda: PersonaMetadata(
        is_builtin=False,
        is_custom=True
    ))
    ...
    def _validate_config(self):
        ...
        if PersonaCapability.FILE_OPERATIONS in self.capabilities:
            if AgentTool.FILE_OPERATIONS not in self.enabled_tools:
                raise ValueError("File operations capability requires file operations tool")
    ...
    def inherit_from_template(self, template_manager: ConfigTemplateManager) -> None:
        ...
        try:
            ...
            if not self.capabilities or self.capabilities == {PersonaCapability.GENERAL_PURPOSE}:
            ...
            if (self.model_settings.provider_type == ProviderType.ANTHROPIC and 
                self.model_settings.model_name == "claude-3-haiku-20240307"):
                ...
                if primary_provider in template.recommended_models:
                    self.model_settings.model_name = template.recommended_models[primary_provider]
                ...
                if primary_provider in template.provider_configs:
                    ...
                    self.model_settings.temperature = provider_config.get('temperature', 0.7)
                    self.model_settings.max_tokens = provider_config.get('max_tokens')
                    
        except Exception as e:
            logger.warning(f"Failed to inherit from template {self.base_template_id}: {e}")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data['model_settings']['provider_type'] = self.model_settings.provider_type.value
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CustomAgentConfig':
        ...
        if 'capabilities' in data:
            data['capabilities'] = {PersonaCapability(cap) for cap in data['capabilities']}
        ...
        if 'metadata' in data:
            ...
            for field_name in ['created_at', 'updated_at']:
                if field_name in metadata_data:
                    metadata_data[field_name] = datetime.fromisoformat(metadata_data[field_name])
    ...
    def get_display_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_used': self.metadata.last_used.strftime('%Y-%m-%d %H:%M') if self.metadata.last_used else 'Never',
...
class CustomAgentValidator:
    ...
    def _build_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "model_settings": {
                    ...
                    "properties": {
                        ...
                        "top_p": {"type": ["number", "null"], "minimum": 0.0, "maximum": 1.0}
                    },
                ...
                "context_parameters": {
                    "type": "object",
                    "properties": {
                        "context_window_size": {"type": "integer", "minimum": 1000},
            ...
            "required": ["id", "name", "model_settings", "capabilities", "enabled_tools"],
            "additionalProperties": True
        }
...
class AgentRepository:
    ...
    def __init__(self, storage_dir: Optional[Path] = None):
        ...
        """
        self.storage_dir = storage_dir or (Path.home() / '.omnimancer' / 'custom_agents')
    ...
    def list_by_category(self, category: PersonaCategory) -> List[CustomAgentConfig]:
        ...
        self._load_cache()
        return [config for config in self._cache.values() if config.category == category]
    ...
    def search(self, query: str, fields: Optional[List[str]] = None) -> List[CustomAgentConfig]:
    ...
    def import_config(self, import_path: Path, new_id: bool = True) -> Optional[CustomAgentConfig]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_updated': max([c.updated_at for c in self._cache.values()]).isoformat() if self._cache else None
        }
