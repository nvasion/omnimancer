...
class AgentSwitcher:
    ...
    def __init__(self, 
        ...
        self.state_storage_path = state_storage_path or Path.home() / ".omnimancer" / "session_states"
        ...
        with self._lock:
            try:
                ...
                if not target_persona:
                    raise SwitchValidationError(f"Persona not found: {target_persona_id}")
                ...
                if not success:
                    self._rollback(context)
                    return (False, f"Failed to activate persona: {target_persona_id}")
    ...
    def _validate_persona_available(self, context: SwitchContext) -> Tuple[bool, str]:
        ...
        if context.to_persona.status not in [PersonaStatus.AVAILABLE, PersonaStatus.ACTIVE]:
            return (False, f"Persona {context.to_persona.id} is not available (status: {context.to_persona.status})")
        return (True, "Persona available")
    
    def _validate_no_active_operations(self, context: SwitchContext) -> Tuple[bool, str]:
        """Validate no active operations are in progress."""
        if context.session_state.active_operations:
            return (False, f"Cannot switch: {len(context.session_state.active_operations)} operations in progress")
    ...
    def _save_current_state(self, context: SwitchContext) -> None:
        ...
        if context.from_persona:
            state_file = self.state_storage_path / f"{context.from_persona.id}_state.json"
            try:
    ...
    def _restore_state(self, context: SwitchContext) -> None:
        ...
        if state_file.exists():
            try:
                ...
                context.session_state.user_preferences.update(saved_state.user_preferences)
        ...
        if self.status_manager:
            try:
                ...
                try:
                    ...
                    if context.from_persona:
                        loop.create_task(self.status_manager.set_agent_status(
                    ...
                    loop.create_task(self.status_manager.set_agent_status(
                        ...
                        metadata={
                            ...
                            'from_persona': context.from_persona.id if context.from_persona else None
                        }
                    ))
                except RuntimeError:
    ...
    def can_switch(self, target_persona_id: str) -> Tuple[bool, str]:
        ...
        if self.current_state != SwitchState.IDLE:
            return (False, f"Switch in progress (state: {self.current_state.value})")
    ...
    def handle_active_operation_switch(self, 
                                      target_persona_id: str,
                                      operation_handler: Callable) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            with self._lock:
                ...
                if current_state.active_operations:
                    logger.warning(f"Attempting switch with {len(current_state.active_operations)} active operations")
            ...
            return self.switch_persona(target_persona_id, 
...
def get_agent_switcher(persona_manager: Optional[PersonaManager] = None) -> AgentSwitcher:
