...
from .file_content_display import UnifiedFileContentDisplay, FileDisplayConfig, DisplayMode
...
from .approval_dialog import ApprovalDialog, DialogOptions, DialogState
from ...cli.approval_prompt import CLIApprovalPrompt, ApprovalDecision, ApprovalDecisionType
...
class FileModificationWorkflow:
    ...
    async def execute_file_modification_workflow(self,
                                                operation_id: str,
                                                file_paths: Optional[List[str]] = None,
                                                context_metadata: Optional[Dict[str, Any]] = None) -> WorkflowContext:
        """
        ...
        try:
            ...
            await self._transition_state(workflow_context, WorkflowState.DISPLAYING_CHANGES)
            ...
            await self._transition_state(workflow_context, WorkflowState.AWAITING_APPROVAL)
            approval_result = await self._handle_change_approval(workflow_context, change_set)
            ...
            if self.config.auto_apply_approved:
                await self._transition_state(workflow_context, WorkflowState.APPLYING_CHANGES)
    ...
    async def execute_workflow(self, changeset: ChangeSet) -> WorkflowContext:
        ...
        try:
            ...
            await self._transition_state(workflow_context, WorkflowState.AWAITING_APPROVAL)
            approval_result = await self._handle_change_approval(workflow_context, changeset)
            ...
            await self._transition_state(workflow_context, WorkflowState.APPLYING_CHANGES)
            apply_result = await self._apply_approved_changes(workflow_context, changeset, approval_result)
    ...
    async def execute_single_file_workflow(self,
                                          file_path: str,
                                          operation_type: str,
        ...
        try:
            ...
            current_model = workflow_context.metadata.get('current_model', 'Omnimancer AI')
            if operation_type == "create":
                display_result = await self.unified_display.display_file_creation(
                    file_path, new_content or "", {
                        'interactive': True,
                ...
                display_result = await self.unified_display.display_file_modification(
                    file_path, current_content or "", new_content or "", {
                        'interactive': True,
                        'current_model': current_model
                    }
                )
            ...
            if display_result.get('approved', False):
                ...
                if operation_type == "create" and new_content is not None:
                    ...
                    write_method = getattr(self.file_system_manager, '_original_write_file', self.file_system_manager.write_file)
                    await write_method(
                        path=file_path, 
                        content=new_content,
    ...
    async def cancel_workflow(self, operation_id: str, reason: str = "User cancelled") -> bool:
        """
    ...
    async def _handle_change_approval(self,
                                     workflow_context: WorkflowContext,
        ...
        if use_batch_interface:
            ...
            for i, change in enumerate(change_set.changes):
                ...
                decision = ApprovalDecision(
                    decision=ApprovalDecisionType.APPROVED if result.get('approved') else ApprovalDecisionType.DENIED,
    ...
    async def _apply_approved_changes(self,
        ...
        if len(change_set.changes) > 1:
            with Progress(console=self.console) as progress:
                task = progress.add_task("Applying changes...", total=len(change_set.changes))
        ...
        # Display results
        if apply_result['success']:
            self.console.print(f"[green]✓ Successfully applied {len(apply_result.get('applied', []))} changes[/green]")
        else:
    ...
    async def _transition_state(self, workflow_context: WorkflowContext, new_state: WorkflowState):
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_changes_per_workflow': sum(
