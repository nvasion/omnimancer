...
class CostCalculator:
    ...
    def update_cost_data(self, provider_name: str, model_name: str, cost_info: TokenCost) -> None:
...
class TokenEstimator:
    ...
    def _initialize_tiktoken(self) -> None:
        ...
        try:
            import tiktoken
            self._tiktoken_encoding = tiktoken.get_encoding("cl100k_base")  # GPT-4 encoding
        except ImportError:
            logger.warning("tiktoken not available, falling back to character-based estimation")
            self._tiktoken_encoding = None
    
    def estimate_tokens(self, text: str, method: TokenCountMethod = TokenCountMethod.ESTIMATED) -> int:
    ...
    def estimate_tokens_by_provider(self, text: str, provider_name: str, model_name: str) -> int:
...
class TokenUsageTracker:
    ...
    def record_usage(self,
                    ...
                    response_efficiency: float = 0.0) -> DetailedTokenUsage:
        ...
        with self._lock:
            ...
            logger.debug(f"Recorded {usage.total_tokens} tokens for {provider_name}:{model_name} (${estimated_cost:.4f})")
            return usage
    
    def estimate_and_record_usage(self,
                                 provider_name: str,
                                 model_name: str,
                                 input_text: str,
                                 output_text: str,
                                 persona_name: Optional[str] = None,
                                 operation_id: Optional[str] = None) -> DetailedTokenUsage:
    ...
    def get_usage_summary(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Efficiency metrics
            efficiency_scores = [u.context_utilization for u in relevant_usage if u.context_utilization > 0]
            response_scores = [u.response_efficiency for u in relevant_usage if u.response_efficiency > 0]
            ...
            return {
                ...
                "average_context_utilization": statistics.mean(efficiency_scores) if efficiency_scores else 0,
                "average_response_efficiency": statistics.mean(response_scores) if response_scores else 0,
    ...
    def analyze_usage_patterns(self, days_back: int = 7) -> UsagePattern:
        ...
        with self._lock:
            ...
            peak_hour = max(hourly_usage.keys(), key=hourly_usage.get) if hourly_usage else datetime.now()
            ...
            # Persona breakdown
            persona_breakdown = defaultdict(lambda: {"tokens": 0, "cost": 0.0, "requests": 0})
            for usage in relevant_usage:
        ...
        with self._lock:
            for usage_dict in data.get("usage_history", []):
                ...
                if isinstance(usage_dict.get("timestamp"), str):
                    usage_dict["timestamp"] = datetime.fromisoformat(usage_dict["timestamp"])
                
                usage = DetailedTokenUsage(**usage_dict)
                self.usage_history.append(usage)
        
        logger.info(f"Loaded {len(data.get('usage_history', []))} usage records from {filepath}")
