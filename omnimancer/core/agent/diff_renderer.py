...
from .rich_renderer import RichTextRenderer, ColorScheme, create_renderer
...
class EnhancedDiffRenderer:
    ...
    def render_file_tree(self, file_changes: List[FileChange]) -> Tree:
        ...
        for directory, changes in sorted(file_groups.items()):
            ...
            for change in sorted(changes, key=lambda x: x.file_path):
                ...
                stats = f"(+{change.lines_added}/-{change.lines_removed})" if change.lines_added or change.lines_removed else ""
                
                dir_node.add(
                    f"{icon} {file_name} {stats}",
                    style=color
                )
    ...
    def render_unified_diff(self, file_change: FileChange) -> Panel:
        ...
        diff_content = self.renderer.apply_diff_highlighting(
            ...
            file_type=file_change.language
        )
    ...
    def render_side_by_side_diff(self, file_change: FileChange) -> Panel:
        ...
        title.append(f" (+{file_change.lines_added}/-{file_change.lines_removed})", style="dim")
    ...
    def render_diff_set(self, 
                       diff_text: str, 
                       diff_type: DiffType = DiffType.UNIFIED,
                       show_summary: bool = True,
                       show_file_tree: bool = True) -> List[RenderableType]:
        """
    ...
    def render_file_content_comparison(self, 
                                     old_content: str,
                                     new_content: str,
                                     file_path: str,
                                     language: Optional[str] = None) -> Panel:
        """
...
def create_diff_renderer(renderer: Optional[RichTextRenderer] = None) -> EnhancedDiffRenderer:
    """Create an enhanced diff renderer with default configuration."""
    return EnhancedDiffRenderer(renderer=renderer)


def render_git_diff(diff_output: str, 
                   diff_type: DiffType = DiffType.UNIFIED,
...
def compare_files(old_file: str, 
                 new_file: str,
                 file_path: str,
                 console: Optional[Console] = None) -> None:
    """
