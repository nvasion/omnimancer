...
from .persona import (
    AgentPersona, PersonaConfiguration, PersonaManager, PersonaStatus
)
from .persona_validator import PersonaValidator, ValidationResult, ValidationSeverity
...
class TemplateApplicator:
    ...
    def apply_template(self,
                      ...
                      options: Optional[ApplicationOptions] = None) -> Tuple[ApplicationResult, ApplicationContext]:
        """
        ...
        try:
            ...
            logger.info(f"Template application completed for persona {persona_id}: {context.result.value}")
    ...
    def _execute_application_pipeline(self,
                                    context: ApplicationContext,
                                    options: ApplicationOptions) -> ApplicationResult:
        ...
        if options.validate_before_apply:
            ...
            if validation_result.has_blocking_issues() and not options.force_application:
    ...
    def _create_backup(self, context: ApplicationContext, options: ApplicationOptions) -> None:
        """Create backup of current state."""
        # Backup original configuration
        if context.persona.configuration:
            context.original_configuration = copy.deepcopy(context.persona.configuration)
        ...
        if options.preserve_session_state and self.agent_switcher:
            context.session_backup = copy.deepcopy(self.agent_switcher.current_session_state)
        
        # Store additional rollback data
        context.rollback_data = {
            'persona_metadata': copy.deepcopy(context.persona.metadata.__dict__) if context.persona.metadata else {},
    ...
    def _verify_application(self, context: ApplicationContext) -> bool:
        ...
        try:
            ...
            if hasattr(context.persona, 'configuration') and context.persona.configuration:
                try:
                    # Try to get template to ensure it's accessible
                    template = context.persona.configuration.get_template(self.template_manager)
                    if not template:
    ...
    def _rollback_application(self, context: ApplicationContext, options: ApplicationOptions) -> None:
    ...
    def rollback_persona(self, persona_id: str) -> Tuple[bool, str]:
        ...
        if not recent_context:
            return False, f"No rollback data available for persona {persona_id}"
    ...
    def get_application_history(self, persona_id: Optional[str] = None) -> List[ApplicationContext]:
        """
        ...
        """
        if persona_id:
            return [ctx for ctx in self.application_history if ctx.persona.id == persona_id]
...
def get_template_applicator(persona_manager: Optional[PersonaManager] = None) -> TemplateApplicator:
