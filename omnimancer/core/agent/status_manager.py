...
from .status_core import (
    AgentStatus, OperationType, OperationStatus, EventType, StreamPriority,
    AgentOperation, AgentEvent, StatusStreamEvent, StreamMetrics,
    StatusDisplayConfig, EventListener, StatusStreamListener,
    create_minimal_config, create_debug_config
)
...
class StatusUpdateCollector:
    ...
    async def add_update(self, stream_event: StatusStreamEvent) -> bool:
        ...
        async with self._lock:
            ...
            now = time.time()
            should_flush = (
                len(self.updates) >= self.batch_size or
                (now - self.last_flush) >= self.flush_interval or
                stream_event.priority in [StreamPriority.HIGH, StreamPriority.CRITICAL]
            )
...
class UnifiedStatusManager:
    ...
    def __init__(self, config: Optional[StatusDisplayConfig] = None):
        ...
        self.stream_event_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.max_queue_size)
        self.priority_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.max_queue_size // 2)
    ...
    async def start_operation(self, operation: AgentOperation) -> str:
        ...
        async with self._lock:
            ...
            if operation.agent_id:
                self.operations_by_agent[operation.agent_id].append(operation.operation_id)
    ...
    async def update_operation_progress(self, operation_id: str, progress: float, 
    ...
    async def complete_operation(self, operation_id: str, 
        ...
        async with self._lock:
            ...
            await self._emit_event(AgentEvent(
                ...
                operation_id=operation_id,
                data={
                    'duration': operation.duration.total_seconds() if operation.duration else 0,
    ...
    async def fail_operation(self, operation_id: str, error: str,
                           error_metadata: Optional[Dict[str, Any]] = None) -> None:
        """
    ...
    async def cancel_operation(self, operation_id: str, reason: Optional[str] = None) -> None:
        """
        Cancel an operation.
    ...
    def get_active_operations(self, agent_id: Optional[str] = None) -> List[AgentOperation]:
    ...
    def get_operation_history(self, agent_id: Optional[str] = None, 
                            limit: Optional[int] = None) -> List[AgentOperation]:
    ...
    # Streaming System
    async def emit_stream_event(self, event: AgentEvent, priority: StreamPriority = StreamPriority.NORMAL) -> bool:
        """
        ...
        try:
            ...
            logger.warning(f"Status stream queue full, dropping event: {event.event_type}")
            return False
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'enabled_agents': sum(1 for status in self.agent_statuses.values() 
    ...
    def get_metrics(self) -> StreamMetrics:
        ...
        self.metrics.queue_size = self.stream_event_queue.qsize() + self.priority_queue.qsize()
        return self.metrics
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy": (
                self.running and 
                metrics.queue_size < self.config.max_queue_size * 0.9 and
                metrics.avg_processing_time < 0.1
            )
        }
    
    # Private Methods
    async def _emit_event(self, event: AgentEvent, stream_priority: Optional[StreamPriority] = None) -> None:
        """
        ...
        if stream_priority is not None:
            ...
            if event.event_type in [EventType.ERROR_OCCURRED, EventType.OPERATION_FAILED]:
                stream_priority = StreamPriority.HIGH
            elif event.event_type in [EventType.AGENT_STATE_CHANGED, EventType.APPROVAL_REQUESTED]:
    ...
    async def _process_events(self) -> None:
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                for listener in self.event_listeners[:]:  # Copy list to avoid modification during iteration
                    try:
    ...
    async def _distribute_to_stream_listeners(self, stream_event: StatusStreamEvent) -> None:
    ...
    def _move_to_history(self, operation: AgentOperation) -> None:
        ...
        if operation.agent_id and operation.operation_id in self.operations_by_agent[operation.agent_id]:
...
async def initialize_status_system(config: Optional[StatusDisplayConfig] = None) -> UnifiedStatusManager:
    """
