...
class ResponseCache:
    ...
    async def get(self, 
                  url: str, 
                  method: str = "GET",
                  headers: Optional[Dict[str, str]] = None) -> Optional[WebResponse]:
...
class WebClient:
    ...
    async def request(self,
                     ...
                     cache_ttl: Optional[int] = None) -> WebResponse:
        """Make HTTP request with security, rate limiting, and caching."""
        
        url = url.strip()
        headers = headers or {}
        method_str = method.value if isinstance(method, RequestMethod) else str(method).upper()
        ...
        response = await self._make_request_with_retry(
            url, method_str, headers, params, data, json_data, 
            cookies, allow_redirects, timeout or self.timeout
        )
        
        # Cache successful GET responses
        if (response.is_success and method == RequestMethod.GET and 
            self.cache and response.status != 204):
    ...
    async def scrape_content(self, 
        ...
        if extract_main_content:
            ...
            try:
                ...
                main_soup = soup.find('main') or soup.find('article') or soup.find('div', class_=re.compile(r'content|main|article'))
                if not main_soup:
                    main_soup = soup
        else:
            main_soup = soup
