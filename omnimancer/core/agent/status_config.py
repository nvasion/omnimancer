...
from .status_core import (
    StatusDisplayConfig, EventType, StatusDisplayLevel, 
    StatusUpdateFrequency, create_minimal_config, create_debug_config
)
            ...
            if hasattr(self.config, key):
                ...
                if key == 'display_level' and isinstance(value, str):
                    ...
                    try:
                        ...
                        continue
                elif key in ['filtered_event_types', 'important_event_types'] and isinstance(value, list):
                    try:
    ...
    def _config_to_dict(self) -> Dict[str, Any]:
        ...
        for key, value in self.config.__dict__.items():
            if isinstance(value, Enum):
                ...
                config[key] = [item.value if isinstance(item, Enum) else item for item in value]
...
def get_status_config_manager(config_file: Optional[Path] = None) -> StatusConfigManager:
    """
...
def load_config_from_env() -> StatusDisplayConfig:
    ...
    if os.getenv('omnimancer_STATUS_ENABLED'):
        config.enabled = os.getenv('omnimancer_STATUS_ENABLED', 'true').lower() == 'true'
    
    if os.getenv('omnimancer_STATUS_LEVEL'):
        try:
            config.display_level = StatusDisplayLevel(os.getenv('omnimancer_STATUS_LEVEL'))
        except ValueError:
            pass
    ...
    if os.getenv('omnimancer_STATUS_AUTO_START'):
        config.auto_start_display = os.getenv('omnimancer_STATUS_AUTO_START', 'false').lower() == 'true'
    ...
    if os.getenv('omnimancer_STATUS_COMPACT'):
        config.compact_mode = os.getenv('omnimancer_STATUS_COMPACT', 'false').lower() == 'true'
