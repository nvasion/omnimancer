...
class MetricValue:
    ...
    labels: Dict[str, str] = field(default_factory=dict)  # For grouping (persona, provider, etc.)
...
class PerformanceMetricsCollector:
    ...
    def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            self.record_metric("system.memory_percent", snapshot.memory_percent, "percent")
    ...
    def _collect_agent_metrics(self) -> None:
        ...
        try:
            ...
            if self.token_tracker:
                recent_summary = self.token_tracker.get_usage_summary(timedelta(minutes=1))
    ...
    def record_operation_timing(self,
                              operation_id: str,
    ...
    def _handle_agent_event(self, event: AgentEvent) -> None:
        ...
        try:
            if event.event_type == EventType.OPERATION_STARTED:
                if event.operation:
                    with self.lock:
                        self.operation_start_times[event.operation.operation_id] = datetime.now()
                        
            elif event.event_type in [EventType.OPERATION_COMPLETED, EventType.OPERATION_FAILED]:
                if event.operation:
                    with self.lock:
                        start_time = self.operation_start_times.pop(event.operation.operation_id, None)
    ...
    def get_dashboard_metrics(self) -> Dict[str, Any]:
        ...
        # Performance metrics
        response_time = self.get_metric_summary("operation.duration_ms", timedelta(minutes=10))
        success_rate = self.get_metric_summary("operation.success", timedelta(minutes=10), AggregationMethod.AVERAGE)
        ...
        # Token usage metrics
        tokens_per_min = self.get_metric_summary("agent.tokens_per_minute", timedelta(minutes=10))
        cost_per_min = self.get_metric_summary("agent.cost_per_minute", timedelta(minutes=10))
    ...
    def establish_baseline(self,
                          ...
                          labels: Optional[Dict[str, str]] = None) -> Optional[PerformanceBaseline]:
        ...
        with self.lock:
            values = [
                v for v in self.metric_buffers[metric_name].values
                if v.timestamp >= cutoff_time
            ]
    ...
    def detect_anomalies(self, metric_name: str, labels: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        ...
        if current_value < lower_bound:
            anomalies.append({
                ...
                "severity": "high" if current_value < (lower_bound * 0.5) else "medium"
            })
            ...
            anomalies.append({
                ...
                "severity": "high" if current_value > (upper_bound * 1.5) else "medium"
            })
...
def initialize_metrics_collection(auto_start: bool = True) -> PerformanceMetricsCollector:
