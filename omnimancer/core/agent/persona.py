...
class PersonaConfiguration:
    ...
    def get_primary_provider_config(self, template_manager: ConfigTemplateManager) -> Dict[str, Any]:
        ...
        if self.primary_provider not in template.provider_configs:
            raise ValueError(f"Provider {self.primary_provider} not found in template {self.template_id}")
    ...
    def to_provider_config(self, provider_id: str, template_manager: ConfigTemplateManager) -> ProviderConfig:
        ...
        provider_type = provider_type_mapping.get(self.primary_provider, ProviderType.ANTHROPIC)
...
class PersonaManager:
            ...
            if persona.activate():
                ...
                if self.status_manager:
                    try:
                        ...
                        logger.debug(f"No event loop available for status update of persona {persona_id}")
                        pass
            ...
            if persona.deactivate():
                ...
                if self.status_manager:
                    try:
                        ...
                        # No event loop running, skip async status update
                        logger.debug(f"No event loop available for status update of persona {persona.id}")
                        pass
    ...
    def _apply_persona_configuration(self, persona: AgentPersona) -> None:
        ...
        try:
            ...
            provider_config = persona.configuration.to_provider_config(persona.id, self.template_manager)
            
            # Add to agent configuration
            self.agent_config.add_provider_config(f"persona_{persona.id}", provider_config)
    ...
    def get_persona_recommendations(self, context: str) -> List[AgentPersona]:
        ...
        if any(word in context_lower for word in ['code', 'programming', 'development', 'bug', 'function']):
            coding_persona = self.get_persona('coding')
            if coding_persona:
                recommendations.append(coding_persona)
        
        # Research-related keywords
        if any(word in context_lower for word in ['research', 'search', 'find', 'information', 'study']):
            research_persona = self.get_persona('research')
            if research_persona:
                recommendations.append(research_persona)
        
        # Creative-related keywords
        if any(word in context_lower for word in ['write', 'story', 'creative', 'poem', 'novel']):
            creative_persona = self.get_persona('creative')
            if creative_persona:
                recommendations.append(creative_persona)
        
        # Performance-related keywords
        if any(word in context_lower for word in ['fast', 'quick', 'speed', 'efficient']):
