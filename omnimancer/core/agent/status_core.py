...
class AgentOperation:
    ...
    def is_active(self) -> bool:
        ...
        return self.status in [OperationStatus.PENDING, OperationStatus.RUNNING, OperationStatus.WAITING_APPROVAL]
    ...
    def update_progress(self, percentage: float, description: Optional[str] = None) -> None:
    ...
    def fail(self, error_details: str, error_metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    def update_processing_time(self, processing_time: float) -> None:
        ...
        if self.events_processed == 0:
            ...
            self.avg_processing_time = (
                (self.avg_processing_time * min(self.events_processed, 100) + processing_time) 
...
class StatusDisplayConfig:
    ...
    important_event_types: Set[EventType] = field(default_factory=lambda: {
    ...
    def __post_init__(self):
        ...
        if self.save_event_history and not self.event_history_file:
            self.event_history_file = Path.home() / ".omnimancer" / "agent_status_history.json"
