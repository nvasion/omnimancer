...
from .status_integrated_engine import StatusIntegratedAgentEngine, create_status_integrated_engine
from .terminal_status_listener import TerminalStatusListener
from .status_ui_components import ComprehensiveStatusDisplay, create_agent_status_summary
...
class CLIStatusIntegration:
    ...
    def __init__(self, engine: StatusIntegratedAgentEngine, console: Optional[Console] = None):
        """
    ...
    async def show_status_snapshot(self, detailed: bool = False) -> None:
        ...
        try:
            ...
            if 'error' in status_summary:
                self.console.print(f"[red]❌ Error getting status: {status_summary['error']}[/red]")
                return
            
            # Display basic info
            agent_status = status_summary.get('status', 'unknown')
    ...
    def _show_detailed_status(self, status_summary: Dict[str, Any]) -> None:
        ...
        if system_stats:
            ...
            self.console.print(f"  Events Emitted: {system_stats.get('events_emitted', 0)}")
            self.console.print(f"  Operations Started: {system_stats.get('operations_started', 0)}")
            self.console.print(f"  Operations Completed: {system_stats.get('operations_completed', 0)}")
            self.console.print(f"  Operations Failed: {system_stats.get('operations_failed', 0)}")
    
    async def execute_with_status_display(self, operation_func: Callable, *args, **kwargs) -> Any:
        """
    ...
    def is_live_display_running(self) -> bool:
        ...
        return self.status_listener is not None and hasattr(self.status_listener, 'live_display') and \
               self.status_listener.live_display is not None
    ...
    async def handle_command(self, args: List[str]) -> None:
        ...
        try:
            if subcommand == 'show':
                ...
                if success:
                    self.console.print("[green]Live status display started. Press Ctrl+C to stop.[/green]")
                    try:
...
"""
        ...
        self.console.print(Panel(help_text, title="📊 Status Command Help", border_style="blue"))


# Factory function for creating CLI integration
async def create_cli_status_integration(config_manager: ConfigManager,
                                      console: Optional[Console] = None) -> CLIStatusIntegration:
    """
...
def add_status_integration_to_engine(engine, console: Optional[Console] = None) -> CLIStatusIntegration:
    """
    ...
    if isinstance(engine, StatusIntegratedAgentEngine):
        ...
        logger.warning(f"Engine {type(engine).__name__} does not support status integration")
...
class DummyCLIStatusIntegration:
    ...
    async def start_live_status_display(self) -> bool:
        self.console.print("[yellow]Status display not available for this engine[/yellow]")
        return False
    ...
    async def show_status_snapshot(self, detailed: bool = False) -> None:
        self.console.print("[yellow]Status information not available for this engine[/yellow]")
