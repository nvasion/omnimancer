...
from .agent_config import (
    CustomAgentConfig, AgentRepository, CustomAgentStatus, AgentTool
)
from .config import (
    AgentConfig, SecuritySettings, AgentSettings, 
    FileSystemSettings, WebClientSettings, ProviderConfig
)
...
class CustomAgentIntegration:
    ...
    def convert_custom_to_standard_config(self, custom_config: CustomAgentConfig) -> AgentConfig:
        """
    ...
    def _build_security_settings(self, custom_config: CustomAgentConfig) -> SecuritySettings:
    ...
    def _build_provider_config(self, custom_config: CustomAgentConfig) -> ProviderConfig:
        """Build provider configuration from model settings."""
    ...
    def _build_filesystem_settings(self, custom_config: CustomAgentConfig) -> FileSystemSettings:
        ...
        if AgentTool.FILE_OPERATIONS not in custom_config.enabled_tools:
            ...
            if safety_level == "strict":
                ...
                fs_settings.backup_directory = f"/tmp/omnimancer_backups_{custom_config.id[:8]}"
    ...
    def _build_webclient_settings(self, custom_config: CustomAgentConfig) -> WebClientSettings:
    ...
    def apply_permission_overrides(self, custom_config: CustomAgentConfig) -> None:
        ...
        try:
            ...
            self.audit_logger.log_event(
                ...
                metadata={
                    'agent_id': custom_config.id,
                    'enabled_tools': [tool.value for tool in custom_config.enabled_tools],
                    'restrictions_count': len(custom_config.behavior_rules.custom_restrictions),
                    'auto_approve_count': len(custom_config.behavior_rules.auto_approve_patterns)
                }
            )
    ...
    def _apply_auto_approval_pattern(self, pattern: str, custom_config: CustomAgentConfig) -> None:
    ...
    def setup_approval_integration(self, custom_config: CustomAgentConfig, 
                                  approval_manager: EnhancedApprovalManager) -> None:
        """
        ...
        try:
            ...
            confirmation_requirements = custom_config.behavior_rules.require_confirmation_for
            
            # Set up approval callbacks with custom logic
            def custom_approval_filter(operation_type: str, operation_data: Dict[str, Any]) -> bool:
                ...
                if safety_level == "strict":
                    ...
                    risky_operations = [
                        "file_write", "file_delete", "command_execute", 
                        "web_request", "system_command"
                    ]
                    ...
                    moderate_operations = [
                        "file_delete", "command_execute", "system_command"
                    ]
    ...
    def activate_custom_agent(self, custom_config: CustomAgentConfig) -> AgentConfig:
        ...
        try:
            ...
            logger.info(f"Successfully activated custom agent: {custom_config.name} ({custom_config.id})")
    ...
    def validate_custom_agent_security(self, custom_config: CustomAgentConfig) -> List[str]:
        """
        ...
        try:
            ...
            if (AgentTool.PROGRAM_EXECUTOR in custom_config.enabled_tools and 
                custom_config.behavior_rules.safety_level == "permissive"):
                warnings.append(
                    "Program executor with permissive safety level may be risky"
                )
            
            # Check for missing confirmation requirements
            if (AgentTool.FILE_OPERATIONS in custom_config.enabled_tools and 
                "file_delete" not in custom_config.behavior_rules.require_confirmation_for):
                warnings.append(
            ...
            dangerous_phrases = [
                "ignore safety", "bypass security", "disable approval", 
                "execute without asking", "override permissions"
            ]
            
            for phrase in dangerous_phrases:
                if phrase in system_prompt:
                    warnings.append(f"System prompt contains potentially dangerous phrase: '{phrase}'")
...
def activate_custom_agent_by_id(agent_id: str, 
                               integration: Optional[CustomAgentIntegration] = None) -> Optional[AgentConfig]:
...
def validate_agent_security(agent_id: str,
                           integration: Optional[CustomAgentIntegration] = None) -> List[str]:
