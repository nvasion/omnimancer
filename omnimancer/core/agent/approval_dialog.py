...
from .rich_renderer import RichTextRenderer, RiskLevel, OperationType, create_renderer
from .diff_renderer import EnhancedDiffRenderer, DiffType, create_diff_renderer
from .approval_context import ApprovalContext, OperationDetails, ApprovalDecision
from .input_handler import InteractiveInputHandler, KeyAction, InputState, create_input_handler
...
class ApprovalDialog:
    ...
    def _render_file_details(self, operation: OperationDetails) -> Table:
        ...
        if len(operation.files_affected) > 5:
            table.add_row(f"... and {len(operation.files_affected) - 5} more files", "", "")
        ...
        try:
            ...
            if final_action == KeyAction.APPROVE:
                return ApprovalDecision(
                    ...
                    timestamp=datetime.now(),
                    additional_data={"input_action": "approve", "interactive": True}
                )
                ...
                return ApprovalDecision(
                    ...
                    timestamp=datetime.now(),
                    additional_data={"input_action": "deny", "interactive": True}
                )
                ...
                return ApprovalDecision(
                    ...
                    timestamp=datetime.now(),
                    additional_data={"input_action": "quit", "interactive": True}
                )
        ...
        try:
            ...
            if hasattr(self, 'live_display') and self.current_context:
                ...
                if input_state.help_visible:
                    ...
                    if input_state.current_section == "diff" and "diff" in self.sections:
                    ...
                    elif input_state.current_section == "details" and "details" in self.sections:
    ...
    def _render_diff_panel(self, context: ApprovalContext, expanded: bool = False) -> Panel:
    ...
    def _render_operation_details(self, operation: OperationDetails, expanded: bool = False) -> Panel:
        ...
        # Environment/Context
        if operation.environment_vars or operation.working_directory:
            sections.append(self._render_environment_details(operation, show_all=expanded))
    ...
    def _render_file_details(self, operation: OperationDetails, show_all: bool = False) -> Table:
    ...
    def _render_environment_details(self, operation: OperationDetails, show_all: bool = False) -> Table:
    ...
    def _render_metadata(self, operation: OperationDetails, show_all: bool = False) -> Table:
    ...
    def _render_controls_with_state(self, input_state: InputState) -> Panel:
        ...
        if self.options.enable_shortcuts:
            ...
            content = Text(f"Use 'y' to approve, 'n' to deny, 'q' to quit | Mode: {input_state.mode.value}")
