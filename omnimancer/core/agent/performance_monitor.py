...
from .status_core import (
    AgentStatus, OperationType, OperationStatus, EventType, AgentOperation, AgentEvent
)
from .status_manager import UnifiedStatusManager as AgentStatusManager, get_status_manager
...
class TokenUsageMetrics:
    ...
    def efficiency_ratio(self) -> float:
        ...
        return (self.output_tokens / self.total_tokens) if self.total_tokens > 0 else 0.0
...
class PerformanceMetricsCollector:
    ...
    def record_token_usage(self, tokens: TokenUsageMetrics) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Recorded token usage: {tokens.total_tokens} tokens for {tokens.provider_name}")
    
    def start_operation_timing(self, operation_id: str, operation_type: OperationType) -> OperationTiming:
    ...
    def complete_operation_timing(self, operation_id: str) -> Optional[OperationTiming]:
        ...
        with self._lock:
            if operation_id in self.active_timings:
                ...
                logger.debug(f"Completed timing for {operation_id}: {timing.duration_ms}ms")
                return timing
            return None
    ...
    def record_context_metrics(self, context: ContextMetrics) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Recorded context metrics: {context.efficiency_score:.1f} efficiency score")
    ...
    def get_token_usage_stats(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Group by provider
            provider_stats = defaultdict(lambda: {"tokens": 0, "cost": 0.0, "requests": 0})
            ...
            for usage in relevant_usage:
                operation_stats[usage.operation_type.value]["tokens"] += usage.total_tokens
            ...
            return {
                ...
                "time_period": f"{time_window.total_seconds():.0f} seconds" if time_window else "all time"
            }
    ...
    def get_performance_snapshot(self) -> PerformanceSnapshot:
        ...
        with self._lock:
            ...
            recent_timings = [
                timing for timing in self.timing_history
                if timing.is_complete and (datetime.now() - timing.start_time) <= recent_window
            ]
            ...
            if recent_tokens:
                ...
                token_metrics.output_tokens = sum(t.output_tokens for t in recent_tokens)
                token_metrics.cost_estimate = sum(t.cost_estimate for t in recent_tokens)
            
            # Calculate average response time
            avg_response_time = 0.0
            if recent_timings:
                avg_response_time = statistics.mean(t.duration_ms for t in recent_timings)
            
            # Calculate success rate
            total_ops = len(recent_timings)
            successful_ops = len([t for t in recent_timings if t.duration_ms > 0])  # Completed ops
...
class PerformanceMonitor:
    ...
    def record_api_call_metrics(self, 
                               provider_name: str,
                               model_name: str,
                               input_tokens: int,
                               output_tokens: int,
                               cached_tokens: int = 0,
                               cost_estimate: float = 0.0,
                               operation_type: OperationType = OperationType.API_CALL) -> None:
    ...
    def start_operation_tracking(self, operation_id: str, operation_type: OperationType) -> str:
        """Start tracking an operation."""
        timing = self.metrics_collector.start_operation_timing(operation_id, operation_type)
        return timing.operation_id
    
    def complete_operation_tracking(self, operation_id: str) -> Optional[OperationTiming]:
    ...
    def get_performance_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_alerts": [asdict(alert) for alert in self.alerts[-10:]],  # Last 10 alerts
            "suggestions": [asdict(suggestion) for suggestion in self.suggestions[-5:]],  # Last 5 suggestions
    ...
    def _handle_agent_event(self, event: AgentEvent) -> None:
        ...
        try:
            if event.event_type == EventType.OPERATION_STARTED:
                ...
                if event.operation:
                    self.start_operation_tracking(event.operation.operation_id, event.operation.operation_type)
    ...
    def _check_token_usage_alerts(self, metrics: TokenUsageMetrics) -> None:
        ...
        with self._lock:
            ...
            recent_usage = self.metrics_collector.get_token_usage_stats(timedelta(minutes=1))
            ...
            if tokens_per_minute > thresholds["excessive_usage_per_minute"]:
                ...
                self._create_alert(
                    ...
                    tokens_per_minute,
                    provider_name=metrics.provider_name,
                    suggested_actions=["Monitor usage patterns", "Consider optimizing context"]
                )
        ...
        with self._lock:
            ...
            if timing.duration_ms > thresholds["very_slow_response_ms"]:
                self._create_alert(
                    ...
                    timing.duration_ms,
                    suggested_actions=["Check network connectivity", "Consider switching providers"]
                )
    ...
    def _create_alert(self,
                     ...
                     provider_name: Optional[str] = None,
                     persona_name: Optional[str] = None,
                     suggested_actions: Optional[List[str]] = None) -> PerformanceAlert:
