...
from .status_core import (
    AgentStatus, AgentOperation, OperationType as StatusOperationType, 
    OperationStatus, EventType, StreamPriority
)
...
class StatusIntegratedAgentEngine(AgentEngine):
    ...
    def __init__(self, config_manager: ConfigManager, base_path: Optional[Path] = None,
        ...
        self.active_status_operations: Dict[str, str] = {}  # agent_operation_id -> status_operation_id
        
        # Agent identification
        self.agent_id = "main_agent"  # Could be made configurable
        
        logger.info("StatusIntegratedAgentEngine initialized with status tracking enabled")
        ...
        try:
            ...
            await self.status_manager.set_agent_status(
                ...
                AgentStatus.ENABLED,
                {
                    ...
                    'base_path': str(self.file_system.base_path) if hasattr(self, 'file_system') else None
                }
            )
    ...
    async def shutdown_status_system(self) -> None:
        ...
        try:
            ...
            for agent_op_id, status_op_id in list(self.active_status_operations.items()):
        ...
        try:
            ...
            if self.enable_status_display:
                await self.status_manager.set_agent_status(
                    self.agent_id,
                    AgentStatus.RUNNING,
                    {'current_operation': operation.type.value if hasattr(operation.type, 'value') else str(operation.type)}
                )
            ...
            if self.enable_status_display:
                await self.status_manager.set_agent_status(
                    self.agent_id,
                    AgentStatus.ENABLED,
                    {'last_operation_result': 'success' if result.success else 'failed'}
                )
    ...
    async def _create_status_operation(self, operation: Operation) -> AgentOperation:
        ...
        status_operation = AgentOperation(
            ...
            metadata={
                ...
                'original_operation_type': operation.type.value if hasattr(operation.type, 'value') else str(operation.type),
    ...
    async def update_operation_progress(self, operation_id: str, progress: float, 
        ...
        """
        if not self.enable_status_display or operation_id not in self.active_status_operations:
            return
...
async def create_status_integrated_engine(config_manager: ConfigManager,
                                        base_path: Optional[Path] = None,
                                        auto_initialize: bool = True) -> StatusIntegratedAgentEngine:
    """
...
class StatusTrackingContext:
    ...
    async def __aenter__(self):
        if self.engine.enable_status_display:
            ...
            from .types import Operation as TempOperation, OperationType as TempOperationType
            ...
            status_operation = await self.engine._create_status_operation(temp_operation)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self.engine.enable_status_display and self.operation_id:
            status_operation_id = self.engine.active_status_operations.get(self.operation_id)
    ...
    async def update_progress(self, progress: float, description: Optional[str] = None):
        ...
        if self.operation_id:
            await self.engine.update_operation_progress(self.operation_id, progress, description)