...
from ...ui.progress_indicator import ProgressIndicator, OperationType as ProgressOperationType, OperationInfo
...
class WorkflowOrchestrator:
        ...
        self.register_workflow(
            ...
            [
                ...
                WorkflowStep(
                    ...
                    dependencies=["detect_tech_stack", "check_configuration", "analyze_structure"]
                )
            ]
        )
            ...
            try:
                ...
                if step.requires_approval and self.approval_manager:
                    if not await self._request_approval(step):
                        ...
                        self.context.history.append(step)
                        self.progress_indicator.complete_operation(operation_id, "cancelled")
                        break
    ...
    async def _display_workflow_plan(self, name: str, steps: List[WorkflowStep]) -> None:
    ...
    def _check_dependencies(self, step: WorkflowStep, history: List[WorkflowStep]) -> bool:
        """Check if all dependencies for a step are satisfied."""
        if not step.dependencies:
            return True
        
        completed_steps = {s.name for s in history if s.status == WorkflowStatus.COMPLETED}
    ...
    async def _display_workflow_summary(self) -> None:
        ...
        completed = sum(1 for s in self.context.history if s.status == WorkflowStatus.COMPLETED)
        failed = sum(1 for s in self.context.history if s.status == WorkflowStatus.FAILED)
    ...
    def _get_progress_operation_type(self, workflow_step_type: WorkflowStepType) -> ProgressOperationType:
    ...
    async def _list_directory_action(self, context: WorkflowContext, params: Dict[str, Any]) -> List[str]:
    ...
    async def _detect_tech_stack_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _check_configuration_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_structure_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_summary_action(self, context: WorkflowContext, params: Dict[str, Any]) -> str:
...
"""
    ...
    async def _read_file_action(self, context: WorkflowContext, params: Dict[str, Any]) -> str:
        """Read file content."""
    ...
    async def _prepare_changes_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _show_diff_action(self, context: WorkflowContext, params: Dict[str, Any]) -> None:
    ...
    async def _write_file_action(self, context: WorkflowContext, params: Dict[str, Any]) -> bool:
        """Write file content."""
        file_path = params.get("file_path")
    ...
    async def _validate_changes_action(self, context: WorkflowContext, params: Dict[str, Any]) -> bool:
    ...
    async def _understand_request_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _plan_approach_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_action(self, context: WorkflowContext, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _verify_results_action(self, context: WorkflowContext, params: Dict[str, Any]) -> bool:
    ...
    async def _provide_summary_action(self, context: WorkflowContext, params: Dict[str, Any]) -> str:
        ...
        if success:
            if "ai_response" in result:
                ...
                context.console.print(f"[bold green]📋 Summary:[/bold green]\n{summary}")
        else:
