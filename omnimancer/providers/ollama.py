...
class OllamaProvider(BaseProvider):
    ...
    async def send_message(self, message: str, context: ChatContext) -> ChatResponse:
        ...
        try:
            ...
            raise NetworkError(f"Network error connecting to Ollama: {e}", provider="ollama")
    ...
    async def _get_server_models(self) -> List[Dict]:
        ...
        try:
            ...
            raise NetworkError(f"Error fetching models from Ollama: {e}", provider="ollama")
            ...
            try:
                ...
                if "error" in data:
                    ...
                    if "not found" in error_msg.lower():
                        ...
                        raise ProviderError(f"Ollama error: {error_msg}", provider="ollama")
                ...
                if not content:
                    ...
                    if data.get("done", True) is False:
                        ...
                        raise ProviderError("Empty response from Ollama", provider="ollama")
                ...
                raise ProviderError(f"Invalid response format from Ollama: {e}", provider="ollama")
