...
from ..core.models import ModelInfo, EnhancedModelInfo, ProviderConfig
...
class ProviderFactory:
    ...
    @classmethod
    def _get_cached_provider_instance(cls, provider_name: str, config: ProviderConfig) -> BaseProvider:
    ...
    @classmethod
    def create_provider(cls, name: str, config: ProviderConfig, config_manager: 'ConfigManager' = None) -> BaseProvider:
        """
    ...
    def get_all_models(cls) -> Dict[str, List[ModelInfo]]:
        ...
        for provider_name, provider_class in cls._providers.items():
            try:
                ...
                if models and isinstance(models[0], EnhancedModelInfo):
                    all_models[provider_name] = [model.to_model_info() for model in models]
                else:
    ...
    def get_all_enhanced_models(cls) -> Dict[str, List[EnhancedModelInfo]]:
        ...
        for provider_name, provider_class in cls._providers.items():
            try:
                ...
                if models and isinstance(models[0], ModelInfo):
                    all_models[provider_name] = [EnhancedModelInfo.from_model_info(model) for model in models]
                else:
                    all_models[provider_name] = models
            except Exception:
                # If we can't create the provider, skip it
        ...
        for provider_name, provider_class in cls._providers.items():
            try:
                ...
                result[provider_name] = {
                    ...
                    'tool_capable_models': [m for m in models if getattr(m, 'supports_tools', False)],
                    'multimodal_models': [m for m in models if getattr(m, 'supports_multimodal', False)]
                }
            except Exception:
    ...
    def get_models_for_provider(cls, provider_name: str) -> List[ModelInfo]:
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to get models for provider {provider_name}: {e}")
    ...
    def get_provider_capabilities(cls, provider_name: str) -> Dict[str, bool]:
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to get capabilities for provider {provider_name}: {e}")
    
    @classmethod
    async def check_provider_health(cls, provider_name: str, config: ProviderConfig) -> Dict[str, any]:
        """
    ...
    @classmethod
    async def get_all_provider_health(cls, configs: Dict[str, ProviderConfig]) -> Dict[str, Dict[str, any]]:
        """
    ...
    def get_models_by_capability(cls, capability: str) -> Dict[str, List[ModelInfo]]:
        ...
        for provider_name, provider_class in cls._providers.items():
            try:
                ...
                if capability == 'tools':
                    filtered_models = [m for m in all_models if getattr(m, 'supports_tools', False)]
                elif capability == 'multimodal':
                    filtered_models = [m for m in all_models if getattr(m, 'supports_multimodal', False)]
                ...
                if filtered_models and isinstance(filtered_models[0], EnhancedModelInfo):
                    capability_models[provider_name] = [model.to_model_info() for model in filtered_models]
                else:
    ...
    @classmethod
    def get_enhanced_models_by_capability(cls, capability: str) -> Dict[str, List[EnhancedModelInfo]]:
        """
        ...
        for provider_name, provider_class in cls._providers.items():
            try:
                ...
                if all_models and isinstance(all_models[0], ModelInfo):
                    all_models = [EnhancedModelInfo.from_model_info(model) for model in all_models]
                ...
                if capability == 'tools':
                    ...
                    filtered_models = [m for m in all_models if m.available and not m.deprecated]
                else:
                    filtered_models = []
    ...
    @classmethod
    def get_models_by_performance(cls, min_swe_score: float) -> Dict[str, List[EnhancedModelInfo]]:
        """
    ...
    @classmethod
    def get_models_by_pricing(cls, max_cost_per_million: float) -> Dict[str, List[EnhancedModelInfo]]:
        """
            ...
            try:
                ...
                for model in all_models:
                    if model.is_free:
                        ...
                        avg_cost = (model.cost_per_million_input + model.cost_per_million_output) / 2
    ...
    def get_provider_model_summary(cls, provider_name: str) -> Dict[str, any]:
        ...
        try:
            ...
            available_models = len([m for m in models if m.available and not m.deprecated])
            ...
            if models:
                ...
                if paid_models:
                    cheapest_model = min(paid_models, key=lambda x: (x.cost_per_million_input + x.cost_per_million_output) / 2)
            ...
            return {
                ...
                } if cheapest_model else None,
                'model_names': [m.name for m in models if m.available and not m.deprecated]
            }
            
        except Exception as e:
            return {'error': f'Failed to get summary for provider {provider_name}: {str(e)}'}
    ...
    @classmethod
    async def batch_health_check(cls, configs: Dict[str, ProviderConfig], timeout: float = 30.0) -> Dict[str, Dict[str, any]]:
        """
        ...
        async def check_single_provider(provider_name: str, config: ProviderConfig) -> tuple[str, Dict[str, any]]:
            try:
