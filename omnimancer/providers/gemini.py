...
from ..core.models import ChatContext, ChatResponse, ModelInfo, ToolDefinition, ToolCall, ToolResult
...
class GeminiProvider(BaseProvider):
    ...
    async def validate_credentials(self) -> bool:
        ...
        if not self.api_key.startswith("AIza"):
            raise AuthenticationError("Invalid Gemini API key format - should start with 'AIza'")
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if "API_KEY" in error_msg.upper() or "INVALID" in error_msg.upper():
                        raise AuthenticationError(f"Invalid Gemini API key: {error_msg}")
                    else:
                        raise ProviderError(f"Gemini API validation error: {error_msg}")
                except (ValueError, KeyError):
                    raise ProviderError("Invalid response from Gemini API during validation")
            elif response.status_code == 403:
                raise AuthenticationError("Gemini API access forbidden - check API key permissions")
            elif response.status_code == 404:
                raise ModelNotFoundError(f"Gemini model '{self.model}' not found or not accessible")
            else:
                try:
                    error_data = response.json()
        ...
        except (AuthenticationError, NetworkError, ProviderError, ModelNotFoundError):
    ...
    def _handle_response(self, response: httpx.Response) -> ChatResponse:
        ...
        if response.status_code == 200:
            try:
                ...
                if candidates and len(candidates) > 0:
                    ...
                    if parts and len(parts) > 0:
                        ...
                        raise ProviderError("Empty parts in Gemini API response", provider="gemini")
                else:
                    raise ProviderError("Empty candidates in Gemini API response", provider="gemini")
            except (ValueError, KeyError) as e:
                raise ProviderError(f"Invalid JSON response from Gemini API: {e}", provider="gemini")
            ...
            try:
                ...
                if "API_KEY" in error_msg.upper() or "INVALID_API_KEY" in error_msg.upper():
            ...
            async with httpx.AsyncClient() as client:
                ...
                for model_data in data.get("models", []):
                    ...
                    if "gemini" in model_name.lower():
                        ...
                        supports_multimodal = "vision" in model_name or "1.5" in model_name
                        ...
                        models.append(ModelInfo(
                            ...
                            latest_version="latest" in model_data.get("version", "")
                        ))
    ...
    def _convert_tools_to_gemini_format(self, tools: List[ToolDefinition]) -> List[Dict]:
        """
