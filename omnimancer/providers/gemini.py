            ...
            try:
                ...
                if candidates and len(candidates) > 0:
                    ...
                    if parts and len(parts) > 0:
                        ...
                        raise ProviderError(
                            "Empty parts in Gemini API response", provider="gemini"
                        )
                else:
                    raise ProviderError(
                        "Empty candidates in Gemini API response", provider="gemini"
                    )
            except (ValueError, KeyError) as e:
                raise ProviderError(
                    f"Invalid JSON response from Gemini API: {e}", provider="gemini"
                )
