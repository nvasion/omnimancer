        ...
        if response.status_code == 200:
            try:
                ...
                if text:
                    ...
                    raise ProviderError("Empty response from Cohere API", provider="cohere")
            except (ValueError, KeyError) as e:
                raise ProviderError(f"Invalid JSON response from Cohere API: {e}", provider="cohere")
            ...
            try:
                ...
                if "quota" in error_msg.lower() or "usage" in error_msg.lower():
                    raise QuotaExceededError(
                        "Cohere API usage quota exceeded",
                        provider="cohere"
                    )
            except (ValueError, KeyError):
                pass
