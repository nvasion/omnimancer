...
class AzureProvider(BaseProvider):
    ...
    def __init__(self, api_key: str, model: str = "", **kwargs):
        ...
        if not self.azure_endpoint.endswith('.openai.azure.com'):
            if not self.azure_endpoint.endswith('.openai.azure.com/'):
                self.azure_endpoint = f"{self.azure_endpoint.rstrip('/')}.openai.azure.com"
    ...
    def _handle_response(self, response: httpx.Response) -> ChatResponse:
        ...
        if response.status_code == 200:
            ...
            raise AuthenticationError("Invalid Azure OpenAI API key or insufficient permissions")
        elif response.status_code == 429:
            raise RateLimitError("Azure OpenAI API rate limit exceeded")
        elif response.status_code == 404:
            raise ModelNotFoundError(f"Azure deployment '{self.azure_deployment}' not found")
        else:
            try:
