...
class BedrockProvider(BaseProvider):
    ...
    def __init__(self, api_key: str, model: str = "", **kwargs):
        ...
        if not model:
            raise ValueError("Model ID is required for AWS Bedrock provider. "
    ...
    def _convert_to_arn(self, model_id: str, model_type: str = "foundation-model") -> str:
    ...
    def _suggest_arn_conversion(self, model_id: str, error_message: str) -> dict:
        ...
        if not self._is_arn(model_id):
            ...
            inference_arn = self._convert_to_arn(f"us.{model_id}", "inference-profile") if not model_id.startswith("us.") else self._convert_to_arn(model_id, "inference-profile")
    ...
    async def send_message_with_tools(
        ...
        try:
            ...
            request_body = self._prepare_bedrock_request_with_tools(message, context, available_tools)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    elif 'not supported' in error_msg or 'inference profile' in error_msg:
                        if not self._is_arn(self.model):
                            # Suggest ARN format for inference profile access
                            inference_arn = self._convert_to_arn(f"us.{self.model}", "inference-profile") if not self.model.startswith("us.") else self._convert_to_arn(self.model, "inference-profile")
                            return {
                                'success': False,
