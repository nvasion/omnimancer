...
def handle_command(args, **kwargs):
    ...
    if action == 'list':
        ...
        if console:
            table = Table(title="Custom Commands", show_header=True, header_style="bold magenta")
            table.add_column("Command", style="cyan", no_wrap=True)
            table.add_column("Description", style="green")
            table.add_column("Type", style="yellow")
            
            for name, cmd in commands.items():
                cmd_type = "Python" if cmd.handler else "Script" if cmd.script_path else "JSON"
        ...
        if console:
            ...
            if cmd.arguments:
                ...
                for arg in cmd.arguments:
                    ...
                    if arg.get('choices'):
                        info_text.append(f"\n    Choices: {', '.join(arg['choices'])}", style="yellow")
                    info_text.append("\n")
            ...
            panel = Panel(info_text, title=f"[bold blue]Command Info[/bold blue]", border_style="blue")
            console.print(panel)
            return None
        else:
            ...
            if cmd.arguments:
                ...
                for arg in cmd.arguments:
                    lines.append(f"  - {arg.get('name', 'unnamed')}: {arg.get('description', '')}")
            return "\n".join(lines)
