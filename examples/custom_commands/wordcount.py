...
async def handle_command(args, **kwargs):
    ...
    try:
        history = engine.conversation_manager.get_current_messages() if hasattr(engine, 'conversation_manager') else []
    except:
        history = []
    ...
    if console and count_type == 'all':
        ...
        table.add_row("Messages", str(user_messages), str(assistant_messages), str(total_messages))
        table.add_row("Words", str(user_words), str(assistant_words), str(total_words))
        table.add_row("Avg Words/Msg", 
