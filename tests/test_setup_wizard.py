...
from omnimancer.core.models import ProviderConfig, EnhancedModelInfo
...
class TestAPIKeyHandling:
    ...
    def test_get_api_key_valid(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard.Prompt.ask', return_value="sk-test123"):
    ...
    def test_get_api_key_empty_then_continue(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard.Prompt.ask', return_value=""), \
             patch('omnimancer.core.setup_wizard.Confirm.ask', return_value=True):
    ...
    def test_validate_api_key_format_openai_valid(self, setup_wizard):
        ...
        assert setup_wizard._validate_api_key_format("openai", "sk-test123456789012345678901234567890") is True
    ...
    def test_validate_api_key_format_claude_valid(self, setup_wizard):
        ...
        assert setup_wizard._validate_api_key_format("claude", "sk-ant-test123456789012345678901234567890") is True
...
class TestProviderConfiguration:
    ...
    async def test_configure_provider_openai_success(self, setup_wizard, mock_console):
        ...
        with patch.object(setup_wizard, 'console', mock_console), \
             patch.object(setup_wizard.provider_setup, '_get_api_key', return_value="sk-test123"), \
             patch.object(setup_wizard.provider_setup, '_select_model', return_value="gpt-4"), \
             patch.object(setup_wizard.provider_setup, '_get_additional_settings', return_value={}):
    ...
    @pytest.mark.asyncio
    async def test_configure_provider_ollama_no_api_key(self, setup_wizard, mock_console):
        """Test Ollama provider configuration (no API key required)."""
        with patch.object(setup_wizard, 'console', mock_console), \
             patch.object(setup_wizard.provider_setup, '_select_model', return_value="llama3.2"), \
             patch.object(setup_wizard.provider_setup, '_get_additional_settings', return_value={}):
...
class TestConfigurationTesting:
    ...
    async def test_test_configuration_success(self, setup_wizard, mock_console):
        ...
        provider_config = ProviderConfig(api_key="sk-test123456789012345678901234567890", model="gpt-4")
        
        with patch.object(setup_wizard, 'console', mock_console), \
             patch.object(setup_wizard.provider_factory, 'create_provider') as mock_create, \
...
class TestConfigurationSaving:
    ...
    def test_save_configuration_success(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard.Confirm.ask', return_value=False):
            ...
            with patch.object(setup_wizard.config_manager, 'get_config') as mock_get_config:
                # Mock an empty config (first provider)
                from omnimancer.core.models import Config, ChatSettings
                mock_config = Config(
                    providers={}, 
                ...
                with patch.object(setup_wizard.config_manager, 'set_provider_config') as mock_set_provider:
                    with patch.object(setup_wizard.config_manager, 'set_default_provider') as mock_set_default:
                        setup_wizard._save_configuration("openai", provider_config)
                        
                        # Verify config manager methods were called
                        mock_set_provider.assert_called_once_with("openai", provider_config)
...
class TestUtilityMethods:
    ...
    def test_get_additional_settings_with_values(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard.Confirm.ask', return_value=True), \
             patch('omnimancer.core.setup_wizard.Prompt.ask', side_effect=["0.8", "2048"]):
...
class TestFullWizardWorkflow:
    ...
    @pytest.mark.skip(reason="Over-engineered test - complex mocking issues after config simplification")
    @pytest.mark.asyncio
    async def test_start_wizard_complete_success(self, setup_wizard, mock_console):
        ...
        mock_provider_setup.save_configuration = Mock()  # Add missing save_configuration mock
        mock_validation.test_configuration = AsyncMock(return_value=True)
        
        with patch.object(setup_wizard.config_manager, 'is_first_run', return_value=True), \
             patch('omnimancer.core.setup_wizard_ui.SetupWizardUI', return_value=mock_ui), \
             patch('omnimancer.core.setup_wizard_provider_setup.SetupWizardProviderSetup', return_value=mock_provider_setup), \
             patch('omnimancer.core.setup_wizard_validation.SetupWizardValidation', return_value=mock_validation):
            ...
            mock_validation.test_configuration.assert_called_once_with("openai", provider_config)
    ...
    async def test_start_wizard_no_provider_selected(self, setup_wizard, mock_console):
        ...
        with patch.object(setup_wizard, 'console', mock_console), \
             ...
             patch.object(setup_wizard.config_manager, 'is_first_run', return_value=True), \
...
class TestProviderSpecificBehavior:
    ...
    def test_api_key_validation_all_providers(self, setup_wizard):
        ...
        test_cases = [
            ...
            ("xai", "invalid", False),
            ("mistral", "mistral-test123456789012345678901", True),  # Key with correct prefix for mistral
            ("mistral", "short", False),
...
class TestEdgeCases:
    ...
    def test_provider_info_completeness(self, setup_wizard):
        ...
        required_keys = ["name", "description", "strengths", "models", "api_key_url", "setup_notes"]
        
        for provider_name, info in setup_wizard.provider_info.items():
            for key in required_keys:
                assert key in info, f"Provider {provider_name} missing {key}"
            
            assert len(info["strengths"]) > 0, f"Provider {provider_name} has no strengths"
            assert len(info["models"]) > 0, f"Provider {provider_name} has no models"
    
    @pytest.mark.asyncio
    async def test_configure_provider_unknown_provider(self, setup_wizard, mock_console):
    ...
    def test_get_api_key_with_prefix_warning(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard_provider_setup.Prompt.ask', return_value="invalid-key"), \
             patch('omnimancer.core.setup_wizard_provider_setup.Confirm.ask', return_value=True), \
...
class TestAsyncBehavior:
    ...
    async def test_async_methods_handle_exceptions_gracefully(self, setup_wizard):
        ...
        with patch('omnimancer.core.setup_wizard_validation.Confirm.ask', return_value=False):
            # Use the actual validation method but patch its dependencies to cause an exception
            with patch.object(setup_wizard.validation.provider_factory, 'create_provider', 
...
class TestSetupWizardSmokeTests:
    ...
    def test_configuration_methods_exist(self):
        ...
        methods = [
            'show_provider_help', 'create_setup_guide', 'generate_all_setup_guides'
        ]
