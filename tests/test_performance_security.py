...
class TestProviderInitializer:
    ...
    def test_provider_instance_caching(self):
        ...
        with patch.object(ProviderInitializer, 'get_provider_class', return_value=mock_provider_class):
            # Create config
            config = ProviderConfig(api_key="test_key", model="test_model")
            
            # First call should create instance
            instance1 = ProviderInitializer.get_provider_instance('test_provider', config)
            assert instance1 == mock_instance
            mock_provider_class.assert_called_once()
            
            # Second call should use cached instance
            mock_provider_class.reset_mock()
            instance2 = ProviderInitializer.get_provider_instance('test_provider', config)
            ...
            mock_provider_class.reset_mock()
            instance3 = ProviderInitializer.get_provider_instance('test_provider', config2)
...
class TestConfigValidator:
    ...
    def test_validation_caching(self):
        ...
        try:
            ...
            errors3 = validator.validate_config(config)
...
class TestHealthMonitor:
    ...
    async def test_health_check_caching(self):
        ...
        with patch('omnimancer.core.provider_initializer.ProviderInitializer.get_provider_instance', 
            ...
            mock_provider.validate_credentials.reset_mock()
            status3 = await monitor.check_provider_health('test_provider', config, force=True)
...
class TestConfigRepair:
    ...
    def test_analyze_config(self):
        ...
        model_issue = next((i for i in issues if "has no model specified" in i['message']), None)
        assert model_issue is not None
        assert model_issue['fixable'] is True
        
        timeout_issue = next((i for i in issues if "timeout should be at least" in i['message']), None)
        assert timeout_issue is not None
        assert timeout_issue['fixable'] is True
        
        mcp_issue = next((i for i in issues if "Invalid MCP auto_approve_timeout" in i['message']), None)
    ...
    def test_fix_issues(self):
        ...
        assert fixed_config.providers["ollama"].timeout >= 10  # Timeout should be increased
