...
class TestAzureProviderInitialization:
    ...
    def test_initialization_with_minimal_config(self, azure_provider_minimal):
        ...
        assert azure_provider_minimal.azure_endpoint == "https://minimal-resource.openai.azure.com"
        assert azure_provider_minimal.azure_deployment == "gpt-35-turbo"  # Defaults to model name
        assert azure_provider_minimal.api_version == "2024-02-15-preview"  # Default version
...
class TestAzureProviderMessageSending:
    ...
    @pytest.mark.asyncio
    async def test_send_message_success(self, azure_provider, sample_chat_context, mock_successful_response):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            response = await azure_provider.send_message("What can you help me with?", sample_chat_context)
            
            assert response.content == "I can help you with a wide range of topics including programming, writing, analysis, and more."
            assert response.model_used == "gpt-4"
            assert response.tokens_used == 35
            assert response.timestamp is not None
    
    @pytest.mark.asyncio
    async def test_send_message_url_construction(self, azure_provider, sample_chat_context):
    ...
    async def test_send_message_timeout(self, azure_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(NetworkError, match="Request to Azure OpenAI API timed out"):
                await azure_provider.send_message("Test message", sample_chat_context)
    
    @pytest.mark.asyncio
    async def test_send_message_network_error(self, azure_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_authentication_error(self, azure_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(AuthenticationError, match="Invalid Azure OpenAI API key"):
                await azure_provider.send_message("Test message", sample_chat_context)
    
    @pytest.mark.asyncio
    async def test_send_message_rate_limit_error(self, azure_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(RateLimitError, match="Azure OpenAI API rate limit exceeded"):
                await azure_provider.send_message("Test message", sample_chat_context)
    
    @pytest.mark.asyncio
    async def test_send_message_deployment_not_found(self, azure_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ModelNotFoundError, match="Azure deployment .* not found"):
                await azure_provider.send_message("Test message", sample_chat_context)
    
    @pytest.mark.asyncio
    async def test_send_message_empty_response(self, azure_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Empty response from Azure OpenAI API"):
                await azure_provider.send_message("Test message", sample_chat_context)
...
class TestAzureProviderMessagePreparation:
    ...
    def test_prepare_messages_with_context(self, azure_provider, sample_chat_context):
        ...
        assert messages[1]["content"] == "I'm here to help you with various tasks. What would you like to know?"
...
class TestAzureProviderResponseHandling:
    ...
    def test_handle_response_with_error_json(self, azure_provider):
        ...
        with pytest.raises(ProviderError, match="Azure OpenAI API error: Invalid request format"):
