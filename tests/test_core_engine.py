...
from omnimancer.core.models import ChatResponse, ModelInfo, EnhancedModelInfo, ChatContext, ProviderConfig
...
def mock_provider():
    ...
    mock.get_available_models.return_value = [
        ModelInfo(name="gpt-4", provider="openai", description="GPT-4", max_tokens=8192, cost_per_token=0.03, available=True),
        ModelInfo(name="gpt-3.5-turbo", provider="openai", description="GPT-3.5 Turbo", max_tokens=4096, cost_per_token=0.002, available=True)
    ]
    mock.get_model_info.return_value = ModelInfo(name="gpt-4", provider="openai", description="GPT-4", max_tokens=8192, cost_per_token=0.03, available=True)
    mock.send_message = AsyncMock(return_value=ChatResponse(
...
def mock_providers(mock_provider):
    ...
    claude_provider.get_available_models.return_value = [
        ModelInfo(name="claude-3-sonnet", provider="claude", description="Claude 3 Sonnet", max_tokens=200000, cost_per_token=0.015, available=True),
        ModelInfo(name="claude-3-haiku", provider="claude", description="Claude 3 Haiku", max_tokens=200000, cost_per_token=0.0025, available=True)
    ]
    claude_provider.get_model_info.return_value = ModelInfo(name="claude-3-sonnet", provider="claude", description="Claude 3 Sonnet", max_tokens=200000, cost_per_token=0.015, available=True)
    claude_provider.send_message = AsyncMock(return_value=ChatResponse(
...
def core_engine(mock_config_manager):
    ...
    with patch('omnimancer.core.engine.HealthMonitor') as mock_health_monitor_class, \
...
class TestProviderInitialization:
    ...
    async def test_initialize_providers_success(self, core_engine, mock_providers):
        ...
        with patch.object(core_engine.provider_initializer, 'initialize_providers', 
                         return_value=mock_providers) as mock_init:
            
            await core_engine.initialize_providers()
            
            assert core_engine.providers == mock_providers
            assert core_engine.current_provider == mock_providers["openai"]  # default provider
            assert core_engine._initialized is True
            mock_init.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_initialize_providers_no_default_uses_first(self, core_engine, mock_providers):
        ...
        with patch.object(core_engine.provider_initializer, 'initialize_providers', 
            ...
            assert core_engine.current_provider.get_provider_name() in ["openai", "claude"]
    ...
    async def test_initialize_providers_failure(self, core_engine):
        ...
        with patch.object(core_engine.provider_initializer, 'initialize_providers', 
            ...
            with pytest.raises(ConfigurationError, match="Provider initialization failed"):
                await core_engine.initialize_providers()
...
class TestModelSwitching:
    ...
    @pytest.mark.asyncio
    async def test_switch_model_provider_not_available(self, core_engine, mock_providers):
    ...
    async def test_switch_model_exception(self, core_engine, mock_providers):
        ...
        core_engine.providers = mock_providers
        mock_providers["openai"].get_available_models.side_effect = Exception("Model fetch failed")
...
class TestMessageSending:
    ...
    async def test_send_message_success(self, core_engine, mock_provider):
        ...
        with patch.object(core_engine.chat_manager, 'get_current_context') as mock_context, \
             patch.object(core_engine.chat_manager, 'add_user_message') as mock_add_user, \
             patch.object(core_engine.chat_manager, 'add_assistant_message') as mock_add_assistant:
    ...
    async def test_send_message_provider_failure(self, core_engine, mock_provider):
        ...
        with patch.object(core_engine.chat_manager, 'get_current_context', return_value=Mock()):
            response = await core_engine.send_message("Hello")
...
class TestModelInformation:
    ...
    def test_get_available_models_provider_error(self, core_engine, mock_providers):
        ...
        mock_providers["openai"].get_available_models.side_effect = Exception("Provider error")
        core_engine.providers = mock_providers
...
class TestConfigurationInfo:
    ...
    def test_get_conversation_summary_error(self, core_engine):
        ...
        with patch.object(core_engine.chat_manager, 'get_current_context', 
...
class TestProviderValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_current_provider_exception(self, core_engine, mock_provider):
...
class TestHealthChecking:
    ...
    async def test_check_provider_health_specific(self, core_engine):
        ...
        with patch.object(core_engine.health_monitor, 'check_provider_health', 
                         new_callable=AsyncMock, return_value=health_status) as mock_check:
    ...
    async def test_check_provider_health_all(self, core_engine):
        ...
        with patch.object(core_engine.health_monitor, 'check_all_providers_health', 
                         new_callable=AsyncMock, return_value=all_health) as mock_check:
...
class TestConversationManagement:
    ...
    def test_save_conversation(self, core_engine):
        ...
        with patch.object(core_engine.chat_manager, 'get_current_context') as mock_context, \
             patch.object(core_engine.conversation_manager, 'save_conversation', 
                         return_value="conversation_123.json") as mock_save:
    ...
    def test_list_conversations(self, core_engine):
        ...
        with patch.object(core_engine.conversation_manager, 'list_conversations', 
                         return_value=conversations) as mock_list:
    ...
    def test_load_conversation_success(self, core_engine):
        ...
        with patch.object(core_engine.conversation_manager, 'load_conversation', 
                         return_value=mock_context) as mock_load:
