...
class TestSecurityValidation:
    ...
    def test_api_key_masking_in_logs(self):
        ...
        assert "sk-ant-api03-1234567890abcdef1234567890abcdef1234567890abcdef" not in config_str
    ...
    def test_sensitive_data_not_in_error_messages(self):
        ...
        test_scenarios = [
            ("Invalid API key format", "sk-1234567890abcdef1234567890abcdef1234567890abcdef"),
        ...
        for scenario_name, sensitive_value in test_scenarios:
            ...
            assert sensitive_value not in error_message, f"Sensitive data exposed in {scenario_name}"
    ...
    def test_provider_credential_validation_security(self):
        ...
        for provider_name, provider_config in config.providers.items():
            ...
            if provider_config.api_key:
                ...
                masked_key = f"{provider_config.api_key[:8]}***" if len(provider_config.api_key) > 8 else "***"
                
                # Test that validation messages use masked keys
                validation_message = f"Validating credentials for {provider_name}: {masked_key}"
                assert provider_config.api_key not in validation_message
                
                print(f"✅ {provider_name}: Credential validation properly masks API key")
    ...
    def test_configuration_serialization_security(self):
        ...
        assert config_dict["providers"]["openai"]["api_key"] == "sk-1234567890abcdef1234567890abcdef1234567890abcdef"
                ...
                for provider_name, provider_config in masked_data["providers"].items():
                    if isinstance(provider_config, dict):
                        ...
                        if "api_key" in provider_config and provider_config["api_key"]:
                            ...
                            provider_config["api_key"] = f"{key[:8]}***" if len(key) > 8 else "***"
                        
                        # Mask AWS credentials
                        if "aws_access_key_id" in provider_config and provider_config["aws_access_key_id"]:
                            key = provider_config["aws_access_key_id"]
                            provider_config["aws_access_key_id"] = f"{key[:4]}***" if len(key) > 4 else "***"
                        
                        if "aws_secret_access_key" in provider_config and provider_config["aws_secret_access_key"]:
    ...
    def test_provider_specific_security_measures(self):
        ...
        assert openai_config.api_key.startswith("sk-"), "OpenAI API key should start with 'sk-'"
        assert len(openai_config.api_key) >= 40, "OpenAI API key should be at least 40 characters"
        
        # Test Claude provider security
        claude_config = config.providers["claude"]
        assert claude_config.api_key.startswith("sk-ant-"), "Claude API key should start with 'sk-ant-'"
        
        # Test Azure provider security
        azure_config = config.providers["azure"]
        assert azure_config.azure_endpoint.startswith("https://"), "Azure endpoint should use HTTPS"
        ...
        assert bedrock_config.aws_access_key_id.startswith("AKIA"), "AWS access key should start with 'AKIA'"
        assert len(bedrock_config.aws_secret_access_key) >= 20, "AWS secret key should be at least 20 characters"
        
        # Test Vertex AI provider security
        vertex_config = config.providers["vertex"]
        assert vertex_config.vertex_credentials_path.endswith(".json"), "Vertex credentials should be JSON file"
    ...
    def test_environment_variable_security(self):
        ...
        with patch.dict(os.environ, test_env_vars):
            ...
            for env_var, expected_value in test_env_vars.items():
                ...
                assert actual_value == expected_value, f"Environment variable {env_var} not set correctly"
            
            print("✅ Environment variable security: Variables can be accessed securely")
    ...
    def test_secure_storage_recommendations(self):
        ...
        assert validate_openai_key("sk-1234567890abcdef1234567890abcdef1234567890abcdef")
        assert not validate_openai_key("invalid-key")
        
        assert validate_claude_key("sk-ant-api03-1234567890abcdef1234567890abcdef1234567890abcdef")
    ...
    def test_configuration_backup_security(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"✅ Original file permissions: {stat.filemode(original_stat.st_mode)}")
            ...
            assert backup_data == original_data, "Backup should contain same data as original"
    ...
    def test_network_security_measures(self):
        ...
        for provider_name, endpoint in secure_endpoints.items():
            if endpoint:
                assert endpoint.startswith("https://"), f"{provider_name} should use HTTPS: {endpoint}"
    ...
    def test_error_handling_security(self):
        ...
        def sanitize_error_message(error_message: str) -> str:
            ...
            error_message = re.sub(r'sk-ant-[a-zA-Z0-9-]{40,}', 'sk-ant-***', error_message)
...
if __name__ == "__main__":
    ...
    def run_security_tests():
        ...
        try:
            ...
            print("✅ Task 11.3 - Final validation and security review completed successfully")
            print("\n🔒 Security Summary:")
