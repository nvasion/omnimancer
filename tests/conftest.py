...
from omnimancer.core.models import (
    ChatContext, ChatMessage, ChatResponse, Config, ModelInfo, 
    ProviderConfig, ChatSettings, MessageRole
)
...
@pytest.fixture
def mock_engine(mock_config_manager, mock_chat_manager, mock_conversation_manager,
                mock_openai_provider, mock_claude_provider):
    ...
    # Chat functionality
    engine.send_message = AsyncMock(return_value=ChatResponse(
    ...
    engine.get_available_models.return_value = [
        ModelInfo(name="gpt-4", provider="openai", description="GPT-4", 
                 max_tokens=8192, cost_per_token=0.00003, available=True),
        ModelInfo(name="claude-3-sonnet", provider="claude", description="Claude 3", 
                 max_tokens=4096, cost_per_token=0.00002, available=True)
    ]
    engine.get_current_model_info.return_value = ModelInfo(
        name="gpt-4", provider="openai", description="GPT-4",
        max_tokens=8192, cost_per_token=0.00003, available=True
    )
    ...
    engine.get_conversation_info = MagicMock(return_value={
        "filename": "test.json", "message_count": 5, "current_model": "gpt-4"
    })
...
class MockProviderFactory:
    ...
    def create_working_provider(
        ...
        provider.send_message = AsyncMock(return_value=create_chat_response(
        ...
        provider.get_model_info.return_value = create_model_info(name=model, provider=name)
        provider.validate_credentials = AsyncMock(return_value=True)
        return provider
    ...
    def create_rate_limited_provider(
        name: str = "rate_limited",
        model: str = "rate-limited-model"
    ) -> MagicMock:
    ...
    for provider in providers:
        if provider == "openai":
            models.extend([
                ...
                create_model_info("gpt-3.5-turbo", provider, "GPT-3.5 Turbo", 4096, 0.000002)
            ])
        elif provider == "claude":
            models.extend([
                create_model_info("claude-3-sonnet", provider, "Claude 3 Sonnet", 4096, 0.00002),
                create_model_info("claude-3-haiku", provider, "Claude 3 Haiku", 4096, 0.000005)
            ])
