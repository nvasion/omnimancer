...
from omnimancer.core.models import Config, ProviderConfig, MCPConfig, MCPServerConfig, ChatSettings
...
class TestConfigValidator:
    ...
    def test_validate_config_valid(self):
        ...
        with patch('omnimancer.core.config_validator.Path') as mock_path:
            mock_path.return_value.expanduser.return_value.parent.exists.return_value = True
    ...
    def test_validate_config_no_default_provider(self):
        ...
        with patch.object(self.validator, 'validate_provider_config', return_value=[]):
            with patch.object(self.validator, 'validate_mcp_config', return_value=[]):
                ...
                assert len(errors) > 0
                assert any("No default provider configured" in error for error in errors)
    ...
    def test_validate_config_default_provider_not_configured(self):
        ...
        assert len(errors) > 0
        assert any("Default provider 'claude' is not configured" in error for error in errors)
...
class TestConfigTemplateManager:
    ...
    def test_init(self):
        ...
        expected_templates = ['coding', 'research', 'creative', 'general', 'performance']
    ...
    def test_get_template_existing(self):
        ...
        assert 'openai' in template.recommended_providers or 'claude' in template.recommended_providers
    ...
    def test_create_coding_template(self):
        ...
        assert template.name == 'coding'
        assert template.use_case == 'coding'
        assert 'software development' in template.description.lower() or 'coding' in template.description.lower()
        
        # Should have coding-relevant providers
        assert any(provider in ['openai', 'claude', 'claude_code'] 
    ...
    def test_template_mcp_servers_disabled_by_default(self):
        ...
        if coding_template.mcp_servers:
            for server_name, server_config in coding_template.mcp_servers.items():
                assert server_config.get('enabled', True) is False, f"Server {server_name} should be disabled by default"
            ...
            if template.mcp_servers:
                for server_name, server_config in template.mcp_servers.items():
                    ...
                    if description:  # Only check non-empty descriptions
                        assert 'uvx' in description.lower(), f"Server {server_name} description should mention uvx requirement"
...
class TestConfigMigration:
    ...
    def test_needs_migration(self):
        ...
        new_config = {"config_version": "2.0", "providers": {"openai": {"model": "gpt-4"}}}
        with open(self.tmp_config_path, 'w') as f:
...
class TestConfigIntegration:
    ...
    def test_config_validation_with_issues(self):
        ...
        assert len(errors) > 0
        assert any("Default provider 'nonexistent' is not configured" in error for error in errors)
        assert any("has no model specified" in error for error in errors)
