...
class TestWorkflowExecutionIntegration:
    ...
    def mock_engine_with_responses(self):
        ...
        async def mock_send_message(message):
            ...
            if "I executed the operations" in message or "What should I do next" in message:
...
Task completed."""
    ...
    @pytest.mark.asyncio
    async def test_complete_workspace_analysis_workflow(self, temp_workspace, cli_with_mocked_engine):
        ...
        async def mock_parse_and_execute_operations(content):
            ...
            for match in re.finditer(file_read_pattern, content):
                ...
                if os.path.exists(filename):
                    ...
                    content = content.replace(match.group(0), f"❌ File not found: {filename}")
            ...
            for match in re.finditer(command_pattern, content, re.DOTALL):
                ...
                content = content.replace(match.group(0), f"✅ Command executed successfully: `{command}`\nOutput: {result}")
        ...
        with patch.object(cli, '_parse_and_execute_operations', side_effect=mock_parse_and_execute_operations):
            # Create a command to analyze workspace
            command = Command.create_chat_message("analyze this workspace and create a summary file")
            ...
            assert any('ls' in cmd for cmd in executed_commands), "ls command was not executed"
            
            # Verify files were created
            assert len(created_files) > 0, "No files were created"
            assert 'WORKSPACE_SUMMARY.md' in created_files, "Summary file was not created"
            
            # Verify the summary file exists and has content
            assert os.path.exists('WORKSPACE_SUMMARY.md'), "Summary file was not actually created"
            
            with open('WORKSPACE_SUMMARY.md', 'r') as f:
                summary_content = f.read()
            
            assert len(summary_content) > 0, "Summary file is empty"
            assert 'Workspace Analysis Summary' in summary_content, "Summary file doesn't contain expected header"
            
            # Verify engine was called multiple times (for continuation)
            assert cli.engine.send_message.call_count >= 2, "Workflow didn't continue as expected"
    
    @pytest.mark.asyncio
    async def test_workflow_with_command_execution(self, temp_workspace, cli_with_mocked_engine):
...
[COMMAND_EXEC] find . -name "*.py" -type f [/COMMAND_EXEC]"""
        ...
        with patch.object(cli, '_parse_and_execute_operations', side_effect=mock_command_execution):
            command = Command.create_chat_message("check git status and list all files")
            
            await cli._handle_chat_message(command)
            
            # Verify multiple commands were executed
            assert len(executed_commands) >= 2, f"Expected multiple commands, got: {executed_commands}"
            assert any('git status' in cmd for cmd in executed_commands), "git status was not executed"
            assert any('ls' in cmd for cmd in executed_commands), "ls command was not executed"
    
    @pytest.mark.asyncio
    async def test_workflow_error_recovery(self, temp_workspace, cli_with_mocked_engine):
...
[FILE_WRITE:readonly_file.txt]Some content[/FILE_WRITE]"""
                ...
                response.content = "Operations completed with some failures as expected."
        ...
        async def mock_error_prone_execution(content):
            ...
            for match in re.finditer(command_pattern, content, re.DOTALL):
                ...
                if 'invalid_command' in command:
                    error_count += 1
                    content = content.replace(match.group(0), f"❌ Command failed: {command}")
                
            return content
        
        with patch.object(cli, '_parse_and_execute_operations', side_effect=mock_error_prone_execution):
            command = Command.create_chat_message("try operations that might cause errors")
            
            await cli._handle_chat_message(command)
            
            # Verify that errors were handled gracefully
            assert error_count > 0, "No errors were simulated"
            assert cli.engine.send_message.call_count >= 1, "Workflow didn't continue after errors"