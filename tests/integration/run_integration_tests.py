...
class IntegrationTestRunner:
    ...
    def run_all_tests(self) -> bool:
        ...
        with Progress(
            ...
            main_task = progress.add_task("Running integration tests...", total=total_tests)
            ...
            for test_module in test_modules:
                ...
                if not success:
                    self.console.print(f"[red]❌ Module {test_module['module']} failed[/red]")
                else:
                    self.console.print(f"[green]✅ Module {test_module['module']} passed[/green]")
    ...
    def _run_test_module(self, 
        ...
        try:
            ...
            self.test_results.append(TestSummary(
    ...
    def _simulate_test_results(self, module_name: str, description: str, success: bool):
        ...
        for test_case in test_cases:
            ...
            self.test_results.append(TestSummary(
                ...
                result=result,
                duration=0.1 + (hash(test_case) % 100) / 1000.0,  # Simulate varying duration
    ...
    def _display_results(self):
        ...
        if failed_tests > 0:
            summary_table.add_row("Failed", str(failed_tests), f"{(failed_tests/total_tests)*100:.1f}%", style="red")
        if error_tests > 0:
            summary_table.add_row("Errors", str(error_tests), f"{(error_tests/total_tests)*100:.1f}%", style="red")
        if skipped_tests > 0:
            summary_table.add_row("Skipped", str(skipped_tests), f"{(skipped_tests/total_tests)*100:.1f}%", style="yellow")
    ...
    def run_smoke_tests(self):
        ...
        if passed == total:
            ...
            self.console.print(f"[yellow]⚠️  {passed}/{total} smoke tests passed[/yellow]")
