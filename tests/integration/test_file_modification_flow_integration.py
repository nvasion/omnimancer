...
from omnimancer.core.agent.approval_manager import EnhancedApprovalManager, ChangeType
...
class TestFileModificationFlowIntegration:
    ...
    def setup_method(self):
        ...
        self.file_system_manager._original_write_file = self.file_system_manager.write_file
    ...
    def create_mock_change_set(self, changes: List[Dict[str, Any]]) -> ChangeSet:
        ...
        return ChangeSet(
            ...
            total_risk_score=sum(1.0 if change.risk_level == RiskLevel.HIGH else 0.5 if change.risk_level == RiskLevel.MEDIUM else 0.1 for change in proposed_changes)
        )
    ...
    async def test_single_file_creation_approved(self):
        ...
        # Mock user approval
        with patch.object(self.workflow.unified_display, 'display_file_creation', new_callable=AsyncMock) as mock_display:
            mock_display.return_value = {'approved': True}
            
            # Mock file system operation
            with patch.object(self.file_system_manager, '_original_write_file', new_callable=AsyncMock) as mock_write:
                ...
                mock_write.assert_called_once_with(path=file_path, content=content, read_before_write=False)
                
                # Verify state transitions occurred
                assert len(self.state_changes) > 0
                assert any(change['to'] == WorkflowState.COMPLETED for change in self.state_changes)
    ...
    async def test_single_file_modification_denied(self):
        ...
        # Mock user denial
        with patch.object(self.workflow.unified_display, 'display_file_modification', new_callable=AsyncMock) as mock_display:
    ...
    async def test_batch_workflow_mixed_approvals(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            mock_fetch.return_value = change_set
            
            # Mock selective approval (approve first 2, deny last)
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                ...
                # Mock application
                async def mock_apply_changes(workflow_context, change_set, approval_result):
                ...
                with patch.object(self.workflow, '_apply_approved_changes', new_callable=AsyncMock, side_effect=mock_apply_changes) as mock_apply:
                    
                    # Execute workflow
                    result = await self.workflow.execute_file_modification_workflow("batch-test")
    ...
    async def test_workflow_with_partial_failures(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                ...
                async def mock_apply_partial_failure(workflow_context, change_set, approval_result):
                    apply_result = {
                        'success': False,
                        'applied': ['/test/success_file.py'],
                        'failed': [('/test/failure_file.py', 'Permission denied')]
                    }
                    workflow_context.applied_changes.extend(apply_result.get('applied', []))
                    workflow_context.failed_changes.extend(apply_result.get('failed', []))
                    return apply_result
                
                with patch.object(self.workflow, '_apply_approved_changes', new_callable=AsyncMock, side_effect=mock_apply_partial_failure) as mock_apply:
                    
                    result = await self.workflow.execute_file_modification_workflow("partial-failure-test")
                    
                    # Should still be marked as approved but not fully applied
                    assert result.final_result == WorkflowResult.APPROVED_NOT_APPLIED
    ...
    async def test_workflow_timeout_handling(self):
        ...
        changes = [{'file_path': '/test/timeout_file.py', 'operation': 'FILE_CREATE'}]
        change_set = self.create_mock_change_set(changes)
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                mock_approval.side_effect = asyncio.TimeoutError("User approval timeout")
                
                result = await self.workflow.execute_file_modification_workflow("timeout-test")
    ...
    async def test_workflow_cancellation(self):
        ...
        # Start a workflow
        changes = [{'file_path': '/test/cancel_file.py', 'operation': 'FILE_CREATE'}]
        change_set = self.create_mock_change_set(changes)
        
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch, \
             patch('builtins.input', return_value='D'), \
             patch.object(self.workflow, '_handle_change_approval') as mock_approval:
            mock_fetch.return_value = change_set
            ...
            # Cancel the workflow
            success = await self.workflow.cancel_workflow("cancel-test", "User cancelled")
    ...
    async def test_workflow_status_tracking(self):
        ...
        changes = [{'file_path': '/test/status_file.py', 'operation': 'FILE_CREATE'}]
        change_set = self.create_mock_change_set(changes)
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                ...
                with patch.object(self.workflow, '_apply_approved_changes') as mock_apply:
                    mock_apply.return_value = {'success': True, 'applied': ['/test/status_file.py'], 'failed': []}
                    
                    # Start workflow
                    workflow_task = asyncio.create_task(
                        self.workflow.execute_file_modification_workflow("status-test")
                    )
                    ...
                    final_status = await self.workflow.get_workflow_status("status-test")
    ...
    async def test_high_risk_changes_workflow(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                mock_approval.return_value = {'approved': True, 'explicit_approval': True}
                
                with patch.object(self.workflow, '_apply_approved_changes') as mock_apply:
                    mock_apply.return_value = {'success': True, 'applied': ['/etc/critical_config.conf'], 'failed': []}
                    
                    result = await self.workflow.execute_file_modification_workflow("high-risk-test")
    ...
    async def test_empty_changeset_workflow(self):
        ...
        empty_changeset = ChangeSet(id="empty-test", description="No changes", changes=[])
        
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            mock_fetch.return_value = empty_changeset
            
            result = await self.workflow.execute_file_modification_workflow("empty-test")
    ...
    async def test_workflow_statistics_tracking(self):
        ...
        for op_id, expected_result, applied_count, failed_count in test_workflows:
            # Mock a simple workflow for each test case
            changes = [{'file_path': f'/test/{op_id}_file.py', 'operation': 'FILE_CREATE'}]
            change_set = self.create_mock_change_set(changes)
                ...
                with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                    ...
                    if expected_result != WorkflowResult.DENIED:
                        apply_response = {
                            ...
                            'applied': [f'/test/{op_id}_file.py'] * applied_count,
                            'failed': [(f'/test/failed_{i}.py', 'Error') for i in range(failed_count)]
                        }
                        
                        with patch.object(self.workflow, '_apply_approved_changes') as mock_apply:
                            mock_apply.return_value = apply_response
                            
                            result = await self.workflow.execute_file_modification_workflow(op_id)
                    else:
                        result = await self.workflow.execute_file_modification_workflow(op_id)
    ...
    async def test_concurrent_workflows(self):
        ...
        for i in range(3):
            changes = [{'file_path': f'/test/concurrent_{i}.py', 'operation': 'FILE_CREATE'}]
        ...
        async def mock_apply_changes(*args, **kwargs):
            ...
            workflow_context = args[0] if args else kwargs.get('workflow_context')
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes', side_effect=mock_fetch_changes) as mock_fetch, \
             patch.object(self.workflow, '_handle_change_approval') as mock_approval, \
             patch.object(self.workflow, '_apply_approved_changes', side_effect=mock_apply_changes) as mock_apply:
            
            mock_approval.return_value = {'approved': True, 'all_changes': True}
            
            # Start multiple workflows concurrently
            workflow_tasks = [
                asyncio.create_task(self.workflow.execute_file_modification_workflow(f"concurrent-{i}"))
                for i in range(3)
            ]
            ...
            for i, result in enumerate(results):
                assert not isinstance(result, Exception), f"Workflow {i} failed with exception: {result}"
                assert result.final_result == WorkflowResult.APPROVED_AND_APPLIED
    ...
    def test_workflow_configuration_validation(self):
        ...
        workflow_with_defaults = FileModificationWorkflow(config=default_config)
...
class TestApprovalFlowEdgeCases:
    ...
    async def test_extremely_large_file_handling(self):
        ...
        mock_display = AsyncMock(return_value={'approved': True, 'reason': 'Large file approved for testing'})
        mock_apply = AsyncMock(return_value={'success': True, 'applied': [large_change], 'failed': []})
        
        with patch.object(self.workflow, 'approval_manager', self.mock_approval_manager), \
             patch.object(self.workflow.changes_integration, 'display_proposed_changes', mock_display), \
             patch.object(self.workflow.changes_integration, 'apply_proposed_changes', mock_apply):
            result = await self.workflow.execute_workflow(changeset)
            
            assert result.current_state == WorkflowState.COMPLETED
            # Should handle large files without memory issues
            assert len(result.applied_changes) <= 1  # May fail due to size limits
    ...
    async def test_binary_file_handling(self):
        ...
        binary_change = ProposedChange(
            operation_type=ChangeType.FILE_CREATE,
            file_path=str(test_file),
            modified_content=binary_content.decode('latin-1'),  # Preserve bytes as string
            risk_level=RiskLevel.MEDIUM
        )
        ...
        # Mock the changes integration display method since batch_approval_threshold=1
        mock_display = AsyncMock(return_value={'approved': True, 'reason': 'Binary file approved'})
        mock_apply = AsyncMock(return_value={'success': True, 'applied': [binary_change], 'failed': []})
    ...
    async def test_filesystem_permission_errors(self):
        ...
        # Mock the changes integration display method since batch_approval_threshold=1
        mock_display = AsyncMock(return_value={'approved': True, 'reason': 'Approved despite permissions'})
        ...
        with patch.object(self.workflow, 'approval_manager', self.mock_approval_manager), \
            ...
            assert result.current_state == WorkflowState.ERROR
            assert len(result.failed_changes) >= 1  # Should have at least the failed permission change
    ...
    async def test_concurrent_modification_conflicts(self):
        ...
        # Mock the changes integration display method since batch_approval_threshold=1
        mock_display = AsyncMock(return_value={'approved': True, 'reason': 'Concurrent changes approved'})
        mock_apply = AsyncMock(return_value={'success': True, 'applied': [change1, change2], 'failed': []})
        ...
        with patch.object(self.workflow, 'approval_manager', self.mock_approval_manager), \
            ...
            assert result.current_state in [WorkflowState.COMPLETED, WorkflowState.ERROR]