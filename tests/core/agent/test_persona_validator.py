...
from omnimancer.core.agent.persona import (
    AgentPersona, PersonaConfiguration, PersonaCapability, PersonaCategory,
    PersonaStatus, PersonaMetadata
)
...
class TestPersonaValidator:
    ...
    def test_validator_initialization(self, validator):
        ...
        assert validator.required_persona_fields == {'id', 'name', 'description', 'category'}
        assert validator.required_configuration_fields == {'template_id', 'primary_provider'}
    ...
    def test_validate_persona_missing_required_fields(self, validator):
        ...
        assert any("'id'" in msg and "missing or empty" in msg for msg in error_messages)
        assert any("'description'" in msg and "missing or empty" in msg for msg in error_messages)
    ...
    def test_validate_configuration_missing_fields(self, validator):
        ...
        config.temperature_override = None  # Add missing attributes that validator checks
    ...
    def test_validate_configuration_invalid_constraints(self, validator):
        ...
        error_messages = [issue.message for issue in result.errors]
        assert any("Temperature override" in msg and "outside valid range" in msg for msg in error_messages)
        assert any("Max tokens override" in msg and "must be positive" in msg for msg in error_messages)
        assert any("Timeout override" in msg and "must be positive" in msg for msg in error_messages)
    ...
    def test_provider_availability_caching(self, validator):
        ...
        assert validator.provider_registry.get_provider_info.call_count == 1  # No additional calls
    ...
    def test_model_availability_basic_models(self, validator):
        ...
        assert validator._is_model_available("claude", "claude-3-sonnet-20240229") is True
