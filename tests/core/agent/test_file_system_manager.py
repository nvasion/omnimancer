...
async def mock_security():
    ...
    async def mock_secure_file_access(path, operation, content=None):
        return {'success': True, 'operation_id': 'test_op', 'session_id': 'test_session'}
...
async def mock_approval_manager():
    ...
    approval_workflow = Mock(spec=ApprovalWorkflow)
...
class TestFileSystemManager:
    ...
    async def test_glob_files(self, fs_manager, temp_dir):
        ...
        # Test recursive glob
        all_txt = await fs_manager.glob_files(str(temp_dir / '**/*.txt'), recursive=True)
    ...
    async def test_list_directory(self, fs_manager, temp_dir):
        ...
        assert expected_items.issubset(actual_items), f"Missing expected items: {expected_items - actual_items}"
...
class TestFileSystemManagerIntegration:
    ...
    async def test_complete_file_workflow(self, fs_manager, temp_dir):
        ...
        result = await fs_manager.write_file(original_file, modified_content, backup=True)
        assert result['success'] == True
        assert result['backup_path'] is not None
    ...
    async def test_directory_operations_workflow(self, fs_manager, temp_dir):
        ...
        # Test glob patterns
        txt_files = await fs_manager.glob_files(str(main_dir / '**/*.txt'), recursive=True)
        assert len(txt_files) == 2
        assert all(f.suffix == '.txt' for f in txt_files)
...
class TestFileSystemManagerApprovalFlow:
    ...
    @pytest.mark.asyncio
    async def test_create_directory_with_approval(self, fs_manager_with_approval, temp_dir):
    ...
    async def test_operation_data_structure(self, fs_manager_with_approval, temp_dir):
        ...
        fs_manager_with_approval.approval_manager.request_single_approval = AsyncMock(side_effect=capture_operation)
