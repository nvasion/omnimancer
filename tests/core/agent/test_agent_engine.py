...
from omnimancer.core.agent_engine import (
    AgentEngine, ProgramExecutor, WebClient,
    MCPIntegrator, ApprovalManager, ProviderFallback,
...
class TestProgramExecutor:
    ...
    async def test_execute_operation(self, executor):
        ...
        with patch.object(executor, '_execute_command') as mock_execute:
            mock_execute.return_value = OperationResult(success=True, data={'stdout': 'files'})
...
class TestWebClient:
    ...
    async def test_make_request_success(self, web_client):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            result = await web_client._make_request('https://httpbin.org/get', 'GET', {}, None)
    ...
    async def test_make_request_failure(self, web_client):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            result = await web_client._make_request('https://httpbin.org/status/404', 'GET', {}, None)
    ...
    async def test_execute_operation(self, web_client):
        ...
        with patch.object(web_client, '_make_request') as mock_request:
            mock_request.return_value = OperationResult(success=True, data={'status_code': 200})
            
            result = await web_client.execute_operation(operation)
            
            assert result.success is True
            mock_request.assert_called_once_with('https://httpbin.org/get', 'GET', {}, None)
...
class TestProviderFallback:
    ...
    @pytest.mark.asyncio
    async def test_execute_with_fallback_providers(self, provider_fallback, core_engine):
...
class TestAgentEngine:
    ...
    def test_get_manager_for_operation(self, agent_engine):
        ...
        assert agent_engine._get_manager_for_operation(file_op) == agent_engine.file_system
        ...
        assert agent_engine._get_manager_for_operation(web_op) == agent_engine.web_client
        
        # MCP operations
        mcp_op = Operation(OperationType.MCP_TOOL_CALL, "MCP", {'tool_name': 'test'})
        assert agent_engine._get_manager_for_operation(mcp_op) == agent_engine.mcp_integrator
    ...
    async def test_generate_preview(self, agent_engine):
        ...
        with patch.object(agent_engine.file_system, 'preview_operation') as mock_preview:
    ...
    async def test_execute_operation(self, agent_engine):
        ...
        with patch.object(agent_engine.file_system, 'execute_operation') as mock_execute:
    ...
    async def test_rollback_operation_file_write(self, agent_engine, temp_dir):
        ...
        result = OperationResult(
            success=True,
            rollback_data={'backup_content': original_content}
        )
