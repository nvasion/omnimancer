...
class TestWorkflowOrchestrator:
    ...
    def mock_executor(self):
        ...
        mock = Mock()
        mock.execute_command = AsyncMock(return_value={"success": True, "output": "test output"})
        return mock
    ...
    async def test_project_analysis_workflow(self, orchestrator, sample_context):
        ...
        with patch('pathlib.Path.iterdir') as mock_iterdir:
            ...
            result = await orchestrator.execute_workflow("project_analysis", sample_context)
            
            assert result is not None
            assert len(result.history) > 0
            
            # Check that steps were executed
            completed_steps = [s for s in result.history if s.status == WorkflowStatus.COMPLETED]
