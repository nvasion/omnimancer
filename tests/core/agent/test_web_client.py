...
from omnimancer.core.agent.web_client import (
    WebClient, WebResponse, RequestMethod, RateLimiter, 
...
class MockResponse:
    ...
    def __init__(self, status=200, headers=None, content=b"test content", 
                 content_type="text/html", charset="utf-8", cookies=None, history=None):
        self.status = status
...
class TestWebClient:
    ...
    async def test_request_failure_after_retries(self, mock_session_class, web_client):
        ...
        # All requests fail
        mock_session.request.side_effect = lambda *args, **kwargs: AlwaysFailingContextManager()
...
class TestWebClientIntegration:
    ...
    async def test_error_handling_workflow(self, web_client):
        ...
        mock_session.request.side_effect = lambda *args, **kwargs: FailingContextManager()
