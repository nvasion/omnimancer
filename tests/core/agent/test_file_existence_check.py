...
class TestFileExistenceCheck:
    ...
    @pytest.mark.asyncio
    async def test_check_file_exists_no_follow_symlinks(self, file_system_manager, temp_dir):
        ...
        result = await file_system_manager.check_file_exists(symlink_file, follow_symlinks=False)
    ...
    @pytest.mark.asyncio
    async def test_check_file_exists_security_denied(self, file_system_manager, temp_dir):
        ...
        with patch.object(file_system_manager.security, 'secure_file_access', 
    ...
    @pytest.mark.asyncio
    async def test_check_file_exists_security_error(self, file_system_manager, temp_dir):
        ...
        with patch.object(file_system_manager.security, 'secure_file_access', 
            ...
            assert result['exists'] is True
            assert result['is_file'] is True
            assert result['error'] is None  # Security error is logged but doesn't affect result
    
    @pytest.mark.asyncio
    async def test_check_file_exists_metadata_error(self, file_system_manager, temp_dir):
    ...
    async def test_file_exists_simple_with_error(self, file_system_manager, temp_dir):
        ...
        with patch.object(file_system_manager.security, 'secure_file_access', 
            ...
            assert exists is True  # File exists, security error is logged but doesn't affect result
    
    @pytest.mark.asyncio
    async def test_check_file_exists_path_resolution(self, file_system_manager, temp_dir):
        ...
        relative_path = temp_dir / 'level1' / 'level2' / '..' / 'level2' / 'nested_file.txt'
        
        result = await file_system_manager.check_file_exists(relative_path)
        
        assert result['exists'] is True
        assert result['path'] == str(relative_path)  # Returns original path, not resolved
        assert result['is_file'] is True
        assert result['error'] is None
...
class TestFileExistenceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_existence_check_before_operations(self, file_system_manager, temp_dir):
