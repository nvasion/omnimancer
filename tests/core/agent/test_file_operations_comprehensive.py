...
from omnimancer.core.agent.file_system_manager import FileSystemManager, FileOperationError
...
<root>
    ...
    @pytest.mark.asyncio
    async def test_read_file_with_special_characters_in_path(self, file_system_manager, temp_dir):
    ...
    async def test_read_file_encoding_error(self, file_system_manager, temp_dir):
        ...
        result_correct = await file_system_manager.read_file(test_file, encoding='latin-1')
        assert result_correct == content
        
        # This might cause issues with wrong encoding, but should be handled gracefully
        try:
            result_wrong = await file_system_manager.read_file(test_file, encoding='ascii')
    ...
    @pytest.mark.asyncio
    async def test_read_file_return_type_consistency(self, file_system_manager, temp_dir):
...
class TestFileReadingEdgeCases:
    ...
    async def test_read_file_concurrent_access(self, file_system_manager, temp_dir):
        ...
        tasks = [
            file_system_manager.read_file(test_file)
            for _ in range(5)
        ]
...
class TestWriteOperationsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_write_file_with_read_before_write_new_file(self, file_system_manager, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_write_file_with_read_before_write_existing_file(self, file_system_manager, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_write_file_without_read_before_write(self, file_system_manager, temp_dir):
        ...
        assert 'operation' not in result or result.get('operation') != 'read_before_write'
        assert test_file.exists()
        assert test_file.read_text() == content
    
    @pytest.mark.asyncio
    async def test_write_file_with_confirmation_and_read_before_write(self, file_system_manager, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_write_file_user_rejection_in_read_before_write(self, file_system_manager, temp_dir):
