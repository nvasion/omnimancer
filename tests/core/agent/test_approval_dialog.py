...
class TestSecurityFlags:
    ...
    def test_security_flags_with_values(self):
        ...
        flags = SecurityFlags(
            requires_sudo=True, accesses_network=True, executes_code=True
        )
...
class TestOperationDetails:
    ...
    def test_add_file_change(self):
        ...
        details.add_file_change(
            "/test/new_file.py", FileOperationType.CREATE, "print('hello')"
        )
    ...
    def test_get_operation_summary(self):
        ...
        details1 = OperationDetails(
            operation_type="file_write", description="Custom description"
        )
...
class TestApprovalDecision:
    ...
    def test_basic_approval_decision(self):
        ...
        decision = ApprovalDecision(approved=True, reason="Operation is safe")
...
class TestApprovalContext:
    ...
    def test_basic_approval_context(self):
        ...
        operation_details = OperationDetails(
            operation_type="test_operation", description="Test operation"
        )

        context = ApprovalContext(
            agent_name="Test Agent", operation_details=operation_details
        )
...
class TestApprovalDialog:
    ...
    @pytest.mark.asyncio
    @patch(
        "omnimancer.core.agent.approval_dialog.InteractiveInputHandler.handle_input_loop"
    )
...
class TestUtilityFunctions:
    ...
    async def test_show_quick_approval(self):
        ...
        with patch(
            "omnimancer.core.agent.approval_dialog.ApprovalDialog.show_approval_dialog"
        ) as mock_show:
            # Mock the dialog to return approval
            mock_show.return_value = ApprovalDecision(
                approved=True, reason="Quick approval"
            )
...
class TestEdgeCases:
    ...
    async def test_dialog_with_exception(self):
        ...
        with patch.object(
            dialog, "_render_header", side_effect=Exception("Test error")
        ):
            decision = await dialog.show_approval_dialog(context)
