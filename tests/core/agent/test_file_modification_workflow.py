...
class TestFileModificationWorkflow:
    ...
    async def test_execute_file_modification_workflow_success(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                ...
                with patch.object(self.workflow, '_apply_approved_changes', side_effect=mock_apply_changes):
                    result = await self.workflow.execute_file_modification_workflow("test-op")
    ...
    async def test_execute_file_modification_workflow_denied(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            with patch.object(self.workflow, '_handle_change_approval') as mock_approval:
                ...
                result = await self.workflow.execute_file_modification_workflow("test-op-denied")
    ...
    async def test_execute_file_modification_workflow_no_changes(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            result = await self.workflow.execute_file_modification_workflow("test-op-empty")
    ...
    async def test_execute_single_file_workflow_create(self):
        ...
        self.file_system_manager._original_write_file = AsyncMock(return_value={'success': True})
        
        with patch.object(self.workflow.unified_display, 'display_file_creation', new=AsyncMock(return_value={'approved': True})) as mock_display:
    ...
    async def test_execute_single_file_workflow_modify(self):
        ...
        with patch.object(self.workflow.unified_display, 'display_file_modification', new=AsyncMock(return_value={'approved': True})) as mock_display:
    ...
    async def test_execute_single_file_workflow_delete(self):
        ...
        with patch.object(self.workflow.unified_display, 'display_file_deletion') as mock_display:
    ...
    async def test_execute_single_file_workflow_denied(self):
        ...
        with patch.object(self.workflow.unified_display, 'display_file_creation') as mock_display:
    ...
    async def test_handle_change_approval_batch_interface(self):
        ...
        with patch.object(self.workflow.changes_integration, 'display_proposed_changes') as mock_display:
            mock_display.return_value = {'approved': True, 'all_changes': True}
            
            result = await self.workflow._handle_change_approval(workflow_context, change_set)
    ...
    async def test_apply_approved_changes_success(self):
        ...
        with patch.object(self.workflow.changes_integration, 'apply_proposed_changes') as mock_apply:
    ...
    async def test_cancel_workflow(self):
        ...
        with patch.object(self.workflow, '_finalize_workflow') as mock_finalize:
            success = await self.workflow.cancel_workflow("cancel-test", "User requested")
    ...
    async def test_state_transitions(self):
        ...
        await self.workflow._transition_state(workflow_context, WorkflowState.DISPLAYING_CHANGES)
        
        assert workflow_context.current_state == WorkflowState.DISPLAYING_CHANGES
        assert len(workflow_context.state_history) == 1
        assert workflow_context.state_history[0]['from_state'] == WorkflowState.INITIALIZED.value
        assert workflow_context.state_history[0]['to_state'] == WorkflowState.DISPLAYING_CHANGES.value
    ...
    async def test_state_change_callback(self):
        ...
        workflow_context = WorkflowContext(
            operation_id="callback-test",
            operation_type="test"
        )
        
        await self.workflow._transition_state(workflow_context, WorkflowState.AWAITING_APPROVAL)
    ...
    async def test_workflow_error_handling(self):
        ...
        with patch.object(self.workflow.changes_integration, 'fetch_proposed_changes') as mock_fetch:
            ...
            result = await self.workflow.execute_file_modification_workflow("error-test")
