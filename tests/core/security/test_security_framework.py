...
from omnimancer.core.security.permission_controller import PermissionOperation, PermissionLevel
...
class TestApprovalWorkflow:
    ...
    def test_approve_request(self):
        ...
        # Approve it
        success = self.workflow.approve_request(request.id, "admin", "Approved after review")
        
        assert success == True
        assert request.id not in self.workflow.pending_requests
        assert request.id in self.workflow.completed_requests
        assert self.workflow.completed_requests[request.id].status == ApprovalStatus.APPROVED
    ...
    def test_deny_request(self):
        ...
        assert request.id not in self.workflow.pending_requests
        assert request.id in self.workflow.completed_requests
        assert self.workflow.completed_requests[request.id].status == ApprovalStatus.DENIED
        assert self.workflow.completed_requests[request.id].denial_reason == "Too risky"
    
    def test_risk_assessment(self):
        """Test risk level assessment."""
        # Low risk
        low_risk = self.workflow.assess_risk_level("file_read", {"path": "./normal_file.txt"})
...
class TestAuditLogger:
    ...
    def test_event_filtering(self):
        ...
        self.logger.log_event(AuditEventType.PERMISSION_CHECK, AuditLevel.INFO, "Permission check")
        self.logger.log_event(AuditEventType.SECURITY_ALERT, AuditLevel.WARNING, "Security alert")
        self.logger.log_event(AuditEventType.SYSTEM_EVENT, AuditLevel.ERROR, "System error")
