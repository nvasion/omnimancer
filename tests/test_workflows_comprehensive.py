...
from omnimancer.core.models import Config, ProviderConfig, EnhancedModelInfo, ChatSettings, ChatMessage, MessageRole
...
from tests.conftest import MockProviderFactory, create_chat_response
...
def mock_providers():
    ...
    for provider_name in ["claude", "openai", "gemini", "perplexity", "xai", "mistral", "cohere", "ollama", "claude-code"]:
        ...
        mock_provider.send_message = AsyncMock(return_value=MagicMock(
        ...
        mock_provider.get_available_models_enhanced = AsyncMock(return_value=available_models)
...
def integrated_system(temp_workspace, mock_providers):
    ...
    provider_factory.create_provider.side_effect = lambda name, **kwargs: mock_providers.get(name)
...
class TestCompleteSetupWorkflow:
    ...
    async def test_first_time_setup_workflow(self, integrated_system):
        ...
        with patch.object(health_monitor, 'provider_factory', system['provider_factory']):
    ...
    async def test_interactive_setup_workflow(self, integrated_system):
        ...
        with patch.object(setup_wizard.config_manager, 'is_first_run', return_value=True), \
             patch.object(setup_wizard.provider_setup, 'select_provider', return_value="claude"), \
             patch.object(setup_wizard.provider_setup, 'configure_provider', new_callable=AsyncMock) as mock_configure, \
             patch.object(setup_wizard.validation, 'test_configuration', new_callable=AsyncMock) as mock_test_config, \
             patch.object(setup_wizard.provider_setup, 'save_configuration') as mock_save, \
            ...
            mock_save.assert_called_once_with("claude", provider_config, system['config_manager'])
...
class TestProviderSwitchingWorkflows:
    ...
    async def test_provider_switching_workflow(self, integrated_system):
        ...
        health_monitor = system['health_monitor']
        with patch.object(health_monitor, 'provider_factory', system['provider_factory']):
            health_status = await health_monitor.check_provider_health("openai", config.providers["openai"])
            assert health_status['status'] == 'healthy'
    
    @pytest.mark.asyncio
    async def test_switch_between_tool_capable_providers(self, mock_engine, mock_provider_factory):
    ...
    @pytest.mark.asyncio
    async def test_conversation_flow_with_provider_switch(self, mock_engine, mock_provider_factory):
        ...
        with patch.object(cli, '_show_user_message'), \
            ...
            mock_show_assistant2.assert_called_once_with("Second provider response", "claude-3-sonnet")


class TestConfigurationWorkflows:
    """Test configuration-related workflows."""
    
    @pytest.mark.skip(reason="/config command removed as redundant - use /setup instead")
    @pytest.mark.asyncio
    async def test_configuration_display_workflow(self, mock_engine):
        """Test displaying configuration information."""
        mock_engine.get_current_config = MagicMock(return_value={
    ...
    async def test_configuration_template_workflow(self, integrated_system):
        ...
        result_path = generator.generate_template_config("research", str(research_config_path))
        ...
        assert any("perplexity" in provider or "search" in str(providers).lower() 
...
class TestErrorHandlingWorkflows:
    ...
    async def test_provider_error_recovery(self, mock_engine, mock_provider_factory):
        ...
        mock_engine.send_message = AsyncMock(side_effect=[
            Exception("Temporary API error"),
            create_chat_response(content="Recovered successfully", model_used="gpt-4")
        ])
        ...
        with patch.object(cli, '_show_user_message'), \
            ...
            await cli._handle_chat_message(command)
            mock_show_assistant.assert_called_once_with("Recovered successfully", "gpt-4")
...
class TestCLIIntegrationWorkflows:
    ...
    async def test_cli_setup_workflow(self, integrated_system):
        ...
        with patch.object(setup_wizard.config_manager, 'is_first_run', return_value=True), \
            ...
            with patch.object(setup_wizard, 'provider_factory', system['provider_factory']):
    ...
    async def test_model_listing_workflow(self, mock_engine):
        ...
        mock_engine.get_all_models = MagicMock(return_value=[
            {"name": "gpt-4", "provider": "openai", "supports_tools": True},
            {"name": "claude-3-sonnet", "provider": "claude", "supports_tools": True},
            {"name": "gemini-1.5-pro", "provider": "gemini", "supports_tools": True}
        ])
...
class TestPerformanceWorkflows:
    ...
    async def test_large_message_handling(self, mock_engine, mock_provider_factory):
        ...
        with patch.object(cli, '_show_user_message'), \
             patch.object(cli, '_show_assistant_message') as mock_show_assistant:
            await cli._handle_chat_message(command)
            mock_show_assistant.assert_called_once_with("Response to large message", "gpt-4")
...
class TestBackwardCompatibilityWorkflows:
    ...
    async def test_conversation_history_compatibility(self, mock_engine):
        ...
        mock_engine.list_conversations = MagicMock(return_value=[
            {
