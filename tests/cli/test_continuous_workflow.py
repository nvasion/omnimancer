...
class TestContinuousWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_execute_continuous_workflow_with_operations(self, cli_interface, mock_agent_manager):
        ...
        first_response = Mock()
        first_response.content = "I'll analyze the files. [COMMAND_EXEC] ls -la [/COMMAND_EXEC]"
        ...
        second_response.content = "Analysis complete. The workspace has been analyzed successfully."
        ...
        with patch.object(cli_interface, '_parse_and_execute_operations') as mock_parse:
            ...
            with patch.object(cli_interface, '_show_assistant_message') as mock_show:
                await cli_interface._execute_continuous_workflow("analyze workspace", first_response)
    ...
    @pytest.mark.asyncio
    async def test_execute_continuous_workflow_no_operations(self, cli_interface, mock_agent_manager):
        ...
        with patch.object(cli_interface, '_parse_and_execute_operations') as mock_parse:
            ...
            with patch.object(cli_interface, '_show_assistant_message') as mock_show:
                await cli_interface._execute_continuous_workflow("analyze workspace", response)
    ...
    @pytest.mark.asyncio
    async def test_workflow_max_iterations_prevention(self, cli_interface, mock_agent_manager):
        ...
        response_with_ops.content = "Continuing work. [COMMAND_EXEC] echo test [/COMMAND_EXEC]"
        ...
        with patch.object(cli_interface, '_parse_and_execute_operations') as mock_parse:
            ...
            with patch.object(cli_interface, '_show_assistant_message'):
                with patch.object(cli_interface, '_show_warning') as mock_warning:
                    await cli_interface._execute_continuous_workflow("test task", response_with_ops)
                    
                    # Should hit max iterations and show warning
                    mock_warning.assert_called_once_with("Workflow stopped after maximum iterations to prevent infinite loop")
    
    @pytest.mark.asyncio
    async def test_workflow_completion_detection(self, cli_interface, mock_agent_manager):
        ...
        completion_response.content = "The task is complete. Analysis finished successfully."
        ...
        with patch.object(cli_interface, '_parse_and_execute_operations') as mock_parse:
            ...
            with patch.object(cli_interface, '_show_assistant_message') as mock_show:
                await cli_interface._execute_continuous_workflow("test task", first_response)
    ...
    @pytest.mark.asyncio
    async def test_workflow_engine_failure_handling(self, cli_interface, mock_agent_manager):
        ...
        with patch.object(cli_interface, '_parse_and_execute_operations') as mock_parse:
            ...
            with patch.object(cli_interface, '_show_assistant_message'):
                with patch.object(cli_interface, '_show_error') as mock_error:
                    ...
                    mock_error.assert_called_once_with("Workflow continuation failed: API connection failed")
    ...
    def test_operation_pattern_detection(self, cli_interface):
        ...
        for content, expected_has_ops in test_cases:
            has_operations = any(re.search(pattern, content, re.DOTALL) 
...
class TestWorkflowEdgeCases:
    ...
    async def test_workflow_stops_on_completion_keywords(self, cli_interface_edge):
        ...
        for phrase in completion_phrases:
            ...
            with patch.object(cli_interface_edge, '_parse_and_execute_operations', new=AsyncMock(return_value="✅ Command executed")) as mock_parse:
                with patch.object(cli_interface_edge, '_show_assistant_message') as mock_show:
                    await cli_interface_edge._execute_continuous_workflow("test task", first_response)
                    
                    # Should show final completion response
                    assert any(phrase.lower() in str(call).lower() for call in mock_show.call_args_list), \
    ...
    async def test_workflow_handles_empty_responses(self, cli_interface_edge):
        ...
        with patch.object(cli_interface_edge, '_parse_and_execute_operations', new=AsyncMock(return_value="✅ Command executed")) as mock_parse:
            with patch.object(cli_interface_edge, '_show_assistant_message'):
                await cli_interface_edge._execute_continuous_workflow("test", first_response)
    ...
    async def test_workflow_handles_malformed_operations(self, cli_interface_edge):
        ...
        for malformed_content in malformed_cases:
            ...
            with patch.object(cli_interface_edge, '_parse_and_execute_operations', new=AsyncMock(return_value=malformed_content)) as mock_parse:
                with patch.object(cli_interface_edge, '_show_assistant_message'):
                    await cli_interface_edge._execute_continuous_workflow("test", response)
