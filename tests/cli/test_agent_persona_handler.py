...
from omnimancer.core.agent.persona import (
    ...
    CodingPersona, ResearchPersona, GeneralPersona
)
...
class TestAgentPersonaHandler:
    ...
    def mock_persona_manager(self):
        ...
        coding_persona.metadata = type('MockMetadata', (), {
            'usage_count': 5,
            'last_used': None,
            'version': '1.0.0',
            'author': 'Omnimancer',
        ...
        research_persona.metadata = type('MockMetadata', (), {
            'usage_count': 2,
            'last_used': None,
            'version': '1.0.0',
            'author': 'Omnimancer',
        ...
        manager.get_all_personas = Mock(return_value={
    ...
    def handler(self, mock_console, mock_persona_manager, mock_agent_switcher):
        ...
        with patch('omnimancer.cli.handlers.agent_persona_handler.get_persona_manager', return_value=mock_persona_manager), \
             patch('omnimancer.cli.handlers.agent_persona_handler.get_agent_switcher', return_value=mock_agent_switcher):
    ...
    @pytest.mark.asyncio
    async def test_handle_use_persona_already_active(self, handler, mock_persona_manager, mock_console):
    ...
    @pytest.mark.asyncio
    async def test_handle_current_persona_active(self, handler, mock_persona_manager, mock_console):
    ...
    @pytest.mark.asyncio
    async def test_handle_switch_history(self, handler, mock_agent_switcher, mock_console):
    ...
    def test_suggest_personas(self, handler, mock_console):
        ...
        suggest_calls = [call for call in mock_console.print.call_args_list 
    ...
    async def test_handle_recommend_persona_no_query(self, handler, mock_console):
        ...
        assert mock_console.print.called
        recommend_calls = [call for call in mock_console.print.call_args_list 
    ...
    async def test_handle_recommend_persona_with_query(self, handler, mock_console):
        ...
        assert mock_console.print.called
        coding_calls = [call for call in mock_console.print.call_args_list 
    ...
    async def test_handle_compare_personas(self, handler, mock_console):
        ...
        from rich.panel import Panel
        table_calls = [call for call in call_args if call[0] and isinstance(call[0][0], Table)]
        panel_calls = [call for call in call_args if call[0] and isinstance(call[0][0], Panel)]
        assert len(table_calls) > 0 or len(panel_calls) > 0  # Either table or legend panel
    ...
    async def test_handle_discover_personas(self, handler, mock_console):
        ...
        assert mock_console.print.called
        discover_calls = [call for call in mock_console.print.call_args_list 
    ...
    async def test_recommend_with_creative_query(self, handler, mock_console):
        ...
        recommend_calls = [call for call in mock_console.print.call_args_list 
                          if any(word in str(call).lower() for word in ["creative", "writing", "general"])]
        assert len(recommend_calls) > 0
    ...
    async def test_recommend_with_performance_query(self, handler, mock_console):
        ...
        # At minimum should show some recommendation
        recommend_calls = [call for call in mock_console.print.call_args_list 
                          if any(word in str(call).lower() for word in ["performance", "speed", "fast", "general"])]
        assert len(recommend_calls) > 0