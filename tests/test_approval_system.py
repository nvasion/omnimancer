...
class TestEnhancedApprovalManager:
    ...
    async def test_risk_assessment_file_operations(self, approval_manager):
        ...
        assert "sensitive file" in sensitive_preview.risk_assessment.lower() or \
               "medium" in sensitive_preview.risk_assessment.lower() or \
               "high" in sensitive_preview.risk_assessment.lower()
    ...
    async def test_command_risk_assessment(self, approval_manager):
        ...
        assert "high" in dangerous_preview.risk_assessment.lower() or \
               "critical" in dangerous_preview.risk_assessment.lower()
...
class TestApprovalIntegration:
    ...
    async def test_batch_approval_workflow(self, approval_manager):
        ...
        operations = [
            Operation(type=OperationType.FILE_READ, description="Read file 1", data={}),
            Operation(type=OperationType.FILE_WRITE, description="Write file 2", data={}),
            Operation(type=OperationType.FILE_DELETE, description="Delete file 3", data={})
        ]
