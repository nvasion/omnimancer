...
from omnimancer.core.models import Config, ProviderConfig, ChatResponse, ModelInfo, EnhancedModelInfo
...
class TestSystemIntegrationSimple:
    ...
    def create_mock_provider(self, provider_name: str, model_name: str, response_text: str):
        """Create a mock provider for testing."""
        provider = MagicMock()
        provider.get_provider_name.return_value = provider_name
        provider.model = model_name
        provider.supports_tools.return_value = True
        provider.supports_multimodal.return_value = provider_name in ["gemini", "openai", "claude"]
        provider.supports_streaming.return_value = True
        provider.validate_credentials = AsyncMock(return_value=True)
        
        # Mock send_message
        provider.send_message = AsyncMock(return_value=ChatResponse(
    ...
    async def test_all_providers_factory_registration(self):
        ...
        expected_providers = [
            "openai", "claude", "gemini", "cohere", "ollama",
        ...
        for provider_name in expected_providers:
            assert provider_name in available_providers, f"Provider {provider_name} not registered"
    ...
    async def test_provider_model_information_retrieval(self):
        ...
        for provider_name, models in all_enhanced_models.items():
            if models:  # Only count providers that have models
                ...
                for model in models:
                    assert isinstance(model, EnhancedModelInfo), f"Model from {provider_name} is not EnhancedModelInfo"
                    assert model.provider == provider_name, f"Model provider mismatch: {model.provider} != {provider_name}"
        ...
        assert provider_count >= 5, f"Expected at least 5 providers with models, got {provider_count}"
    ...
    async def test_provider_capabilities_integration(self):
        ...
        multimodal_models = ProviderFactory.get_enhanced_models_by_capability('multimodal')
        ...
        tool_provider_count = len([p for p, models in tool_capable_models.items() if models])
        multimodal_provider_count = len([p for p, models in multimodal_models.items() if models])
        ...
        assert tool_provider_count >= 3, f"Expected at least 3 providers with tool support, got {tool_provider_count}"
        assert multimodal_provider_count >= 2, f"Expected at least 2 providers with multimodal support, got {multimodal_provider_count}"
    ...
    async def test_concurrent_provider_operations(self):
        ...
        print(f"✅ Concurrent operations completed: {successful_operations}/{len(test_providers)} successful")
        print(f"✅ Total models retrieved concurrently: {total_models}")
        
        # Verify reasonable success rate
        success_rate = successful_operations / len(test_providers)
        assert success_rate >= 0.5, f"Expected at least 50% success rate, got {success_rate:.1%}"
    ...
    async def test_provider_health_checking_integration(self):
        ...
        for provider_name, health_status in health_results.items():
            assert 'status' in health_status, f"Health status missing 'status' for {provider_name}"
            assert 'available' in health_status, f"Health status missing 'available' for {provider_name}"
            
            if health_status.get('configured', True):  # Default to True if not specified
                configured_providers += 1
            
            if health_status['available']:
                available_providers += 1
            
            print(f"✅ {provider_name}: {health_status['status']} (available: {health_status['available']})")
        ...
        assert len(health_results) >= 10, f"Expected at least 10 providers health checked, got {len(health_results)}"
    ...
    async def test_stress_testing_provider_factory(self):
        ...
        async def stress_get_capabilities():
            ...
            for capability in capabilities:
                try:
                    models = ProviderFactory.get_enhanced_models_by_capability(capability)
    ...
    async def test_provider_switching_simulation(self):
        ...
        provider_configs = {
            ...
            "claude": {"model": "claude-3-sonnet-20240229", "response": "Claude response"},
        ...
        for provider_name in mock_providers.keys():
            ...
            if provider_name in config.providers:
                ...
                assert response.is_success, f"Failed to send message with {provider_name}"
                assert provider_name.lower() in response.content.lower(), f"Response doesn't match provider {provider_name}"
        ...
        assert switch_count >= 5, f"Expected at least 5 provider switches, got {switch_count}"
    ...
    async def test_model_catalog_integration(self):
        ...
        all_models_with_capabilities = ProviderFactory.get_all_models_with_capabilities()
        
        assert len(all_models_with_capabilities) > 0, "No models with capabilities retrieved"
        ...
        for provider_name, provider_info in all_models_with_capabilities.items():
            ...
            print(f"✅ {provider_name}: {len(models)} models, tools: {capabilities.get('supports_tools', False)}, multimodal: {capabilities.get('supports_multimodal', False)}")
        
        print(f"✅ Model catalog integration: {providers_with_models} providers with models")
        ...
        assert providers_with_models >= 8, f"Expected at least 8 providers with models, got {providers_with_models}"
        assert total_models >= 20, f"Expected at least 20 total models, got {total_models}"
        assert providers_with_tools >= 5, f"Expected at least 5 providers with tool support, got {providers_with_tools}"
    ...
    async def test_configuration_integration(self):
        ...
        for provider_name in expected_providers:
            assert provider_name in config.providers, f"Provider {provider_name} not in configuration"
            provider_config = config.providers[provider_name]
            assert provider_config.model, f"Provider {provider_name} has no model configured"
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            ...
            print("✅ Task 11.2 - Complete system integration testing completed successfully")
