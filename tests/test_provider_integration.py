...
from omnimancer.core.models import ChatResponse, ModelInfo, ChatContext, ChatMessage, MessageRole
from omnimancer.utils.errors import ConfigurationError, ProviderError, AuthenticationError, RateLimitError
...
class TestProviderSwitching:
    ...
    @pytest.mark.asyncio
    async def test_switch_between_working_providers(self, mock_engine, mock_provider_factory):
    ...
    async def test_switch_to_nonexistent_provider(self, mock_engine):
        ...
        with pytest.raises(ConfigurationError, match="Provider 'unknown' is not available"):
            await mock_engine.switch_model("unknown")
    
    @pytest.mark.asyncio
    async def test_context_preservation_during_switch(self, mock_engine, sample_chat_context):
        ...
        async def mock_switch_model(provider_name, model_name=None):
            if provider_name in mock_engine.providers:
                ...
                mock_engine.current_provider = mock_engine.providers[provider_name]
                mock_engine.chat_manager.set_current_model(mock_engine.current_provider.model)
    ...
    @pytest.mark.asyncio
    async def test_model_switching_within_provider(self, mock_engine, mock_provider_factory):
        """Test switching models within the same provider."""
        # Setup provider with multiple models
        openai_provider = mock_provider_factory.create_working_provider("openai", "gpt-4")
        ...
        with pytest.raises(ConfigurationError, match="Model 'invalid-model' not available"):
...
class TestProviderErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_api_key_authentication_failure(self, mock_engine, mock_provider_factory):
        ...
        failing_provider.send_message = AsyncMock(return_value=create_chat_response(
    ...
    async def test_rate_limiting_error(self, mock_engine, mock_provider_factory):
        ...
        rate_limited_provider = mock_provider_factory.create_rate_limited_provider("openai")
    ...
    async def test_network_timeout_error(self, mock_engine, mock_provider_factory):
        ...
        timeout_provider.send_message = AsyncMock(side_effect=Exception("Network timeout"))
    ...
    @pytest.mark.asyncio
    async def test_provider_unavailable_fallback(self, mock_engine, mock_provider_factory):
    ...
    @pytest.mark.asyncio
    async def test_malformed_response_handling(self, mock_engine, mock_provider_factory):
        ...
        malformed_provider.send_message = AsyncMock(return_value=create_chat_response(
...
class TestProviderValidation:
    ...
    @pytest.mark.asyncio
    async def test_provider_credential_validation(self, mock_engine, mock_provider_factory):
    ...
    @pytest.mark.asyncio
    async def test_model_availability_checking(self, mock_engine, mock_provider_factory):
...
class TestRetryBehavior:
    ...
    async def test_max_retries_exceeded(self, mock_engine, mock_provider_factory):
        ...
        async def mock_engine_send_message(message):
            ...
            for attempt in range(max_retries):
                call_count += 1
                last_response = await mock_engine.current_provider.send_message(message, [])
...
class TestProviderConfigurationIntegration:
    ...
    async def test_configuration_driven_provider_setup(self, mock_engine, test_config):
        ...
        async def mock_initialize_providers():
            ...
            if test_config.default_provider in mock_engine.providers:
                mock_engine.current_provider = mock_engine.providers[test_config.default_provider]
    ...
    async def test_dynamic_configuration_update(self, mock_engine, mock_config_manager):
        ...
        mock_engine.config_manager.update_provider_settings("openai", model="gpt-4-turbo")
