...
class TestRetryHandler:
    ...
    def test_should_retry_network_error(self):
        ...
        assert self.retry_handler.should_retry(NetworkError("Connection failed"), 0) is True
    ...
    def test_should_retry_rate_limit_error(self):
        ...
        assert self.retry_handler.should_retry(RateLimitError("Rate limited"), 0) is True
        assert self.retry_handler.should_retry(RateLimitError("Too many requests"), 1) is True
    ...
    def test_should_retry_boundary_conditions(self):
        ...
        assert handler.should_retry(NetworkError("test"), 2) is True  # Last allowed retry
        assert handler.should_retry(NetworkError("test"), 3) is False  # Exceeds max retries
