...
class TestToolDiscovery:
    ...
    async def test_discover_tools(self, mcp_integrator):
        ...
        file_tools = mcp_integrator.find_tools_by_capability(ToolCapability.FILE_OPERATIONS)
...
class TestToolExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_with_context_enhancement(self, mcp_integrator, mock_mcp_manager):
...
class TestToolMetrics:
    ...
    async def test_metrics_reliability_score(self, mcp_integrator, mock_mcp_manager):
        ...
        result1 = await mcp_integrator.execute_tool_with_context("file_read", {"path": "/unique/path1"})
        assert result1.success is True
        
        # Set up for failure and execute with different arguments to avoid any issues
        mock_mcp_manager.set_tool_error("file_read", "Test error")
        result2 = await mcp_integrator.execute_tool_with_context("file_read", {"path": "/unique/path2"})
...
class TestToolCaching:
    ...
    async def test_result_caching(self, mcp_integrator, mock_mcp_manager):
        ...
        # First execution
        result1 = await mcp_integrator.execute_tool_with_context("file_read", {"path": "/test"})
        
        # Modify mock to return different result
        mock_mcp_manager.set_tool_result("file_read", {"data": "new result"})
        
        # Second execution should use cache
        result2 = await mcp_integrator.execute_tool_with_context("file_read", {"path": "/test"})
    ...
    async def test_cache_expiry(self, mcp_integrator, mock_mcp_manager):
        ...
        # Change mock result
        mock_mcp_manager.set_tool_result("file_read", {"data": "new result after expiry"})
    ...
    def test_cache_key_generation(self, mcp_integrator):
        ...
        key2 = mcp_integrator._generate_cache_key("tool1", {"b": 2, "a": 1})  # Different order
        key3 = mcp_integrator._generate_cache_key("tool1", {"a": 1, "b": 3})  # Different value
        key4 = mcp_integrator._generate_cache_key("tool2", {"a": 1, "b": 2})  # Different tool
    ...
    def test_cacheable_detection(self, mcp_integrator):
        ...
        assert not mcp_integrator._is_cacheable("file_write", {"path": "/test", "content": "data"})
        
        # Operations with dynamic data should not be cacheable
        assert not mcp_integrator._is_cacheable("file_read", {"path": "/test", "timestamp": time.time()})
...
class TestToolSelection:
    ...
    async def test_find_best_tool_for_task(self, mcp_integrator):
        ...
        best_tool = mcp_integrator.find_best_tool_for_task("search for information online")
    ...
    def test_find_tools_by_capability(self, mcp_integrator):
        ...
        mcp_integrator.tools_by_capability[ToolCapability.FILE_OPERATIONS] = ["file_read", "file_write"]
        mcp_integrator.tools_by_capability[ToolCapability.WEB_REQUESTS] = ["web_fetch", "api_call"]
...
class TestHealthChecking:
    ...
    @pytest.mark.asyncio
    async def test_health_check_problematic_tools(self, mcp_integrator, mock_mcp_manager):
