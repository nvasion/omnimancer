...
from omnimancer.core.models import ChatContext, ChatMessage, MessageRole, ModelInfo, ToolDefinition, ToolCall
...
class TestGeminiProviderCredentialValidation:
    ...
    async def test_validate_credentials_forbidden(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(AuthenticationError, match="Gemini API access forbidden"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_model_not_found(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ModelNotFoundError, match="Gemini model 'gemini-1.5-pro' not found"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_network_timeout(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(NetworkError, match="Gemini API validation request timed out"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_network_error(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(NetworkError, match="Network error during Gemini API validation"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_malformed_json_response(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Invalid response from Gemini API during validation"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_unexpected_error(self, gemini_provider):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Unexpected error during Gemini API validation"):
                await gemini_provider.validate_credentials()
    ...
    async def test_validate_credentials_generic_400_error(self, gemini_provider):
        ...
        mock_response.json.return_value = {
            "error": {
                "message": "Bad request - generic error"
            }
        }
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Gemini API validation error: Bad request - generic error"):
                await gemini_provider.validate_credentials()
...
class TestGeminiProviderModelInfo:
    ...
    def test_get_model_info_gemini_1_5_pro(self):
        ...
        assert model_info.description == "Gemini 1.5 Pro - Most capable multimodal model"
    ...
    def test_get_model_info_gemini_1_5_flash(self):
        ...
        provider = GeminiProvider(api_key="AIzaSyTest123456789", model="gemini-1.5-flash")
        model_info = provider.get_model_info()
        
        assert model_info.name == "gemini-1.5-flash"
        assert model_info.provider == "gemini"
        assert model_info.description == "Gemini 1.5 Flash - Fast and efficient multimodal model"
    ...
    def test_get_model_info_gemini_1_0_pro(self):
        ...
        assert model_info.provider == "gemini"
        assert model_info.description == "Gemini 1.0 Pro - Reliable text generation model"
    ...
    def test_model_info_matches_available_models(self):
        ...
        matching_model = next(m for m in available_models if m.name == "gemini-1.5-flash")
        ...
        assert current_model_info.supports_multimodal == matching_model.supports_multimodal
...
class TestGeminiProviderMessageSending:
    ...
    @pytest.mark.asyncio
    async def test_send_message_success(self, gemini_provider, sample_chat_context, mock_successful_response):
    ...
    @pytest.mark.asyncio
    async def test_send_message_empty_candidates(self, gemini_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Empty candidates in Gemini API response"):
    ...
    async def test_send_message_empty_parts(self, gemini_provider, sample_chat_context):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(ProviderError, match="Empty parts in Gemini API response"):
                await gemini_provider.send_message("Hello", sample_chat_context)
    
    @pytest.mark.asyncio
    async def test_send_message_authentication_error(self, gemini_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_rate_limit_error(self, gemini_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_model_not_found(self, gemini_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_network_timeout(self, gemini_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_network_error(self, gemini_provider, sample_chat_context):
...
class TestGeminiProviderToolCalling:
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_tools_success(self, gemini_provider, sample_chat_context, 
                                                  sample_tools, mock_tool_response):
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_tools_no_tool_support(self, sample_chat_context, sample_tools):
        ...
        mock_response.json.return_value = {
            "candidates": [
                {
                    "content": {
                        "parts": [{"text": "I can't use tools, but I can help with math."}]
                    }
                }
            ],
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_tools_empty_tools(self, gemini_provider, sample_chat_context):
...
class TestGeminiProviderResponseHandling:
    ...
    def test_handle_response_with_tools_success(self, gemini_provider, mock_tool_response):
...
class TestGeminiProviderIntegration:
    ...
    async def test_full_conversation_flow(self, gemini_provider):
        ...
        responses = [
            ...
            {
                "candidates": [{"content": {"parts": [{"text": "Sure, I can help with math."}]}}],
                "usageMetadata": {"totalTokenCount": 15}
            }
        ]
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            context.messages.extend([
                ...
                ChatMessage(MessageRole.ASSISTANT, response1.content, datetime.now(), "gemini-1.5-pro")
            ])
            
            # Second message
            response2 = await gemini_provider.send_message("Can you help with math?", context)
...
class TestGeminiProviderEdgeCases:
    ...
    def test_model_capabilities_consistency(self):
        ...
        test_cases = [
            ...
            ("gemini-1.0-pro", False, False, False)  # no tools, no multimodal, not latest
        ]
        
        for model_name, expected_tools, expected_multimodal, expected_latest in test_cases:
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_various_response_formats(self, sample_chat_context):
    ...
    def test_max_tokens_method(self):
        ...
        provider_flash = GeminiProvider(api_key="AIzaSyTest123456789", model="gemini-1.5-flash")
    ...
    def test_cost_estimation(self):
        ...
        # Test with different model
        provider_old = GeminiProvider(api_key="AIzaSyTest123456789", model="gemini-1.0-pro")
...
class TestGeminiProviderComprehensiveValidation:
    ...
    def test_model_info_completeness_for_all_models(self):
        ...
        for model_name in supported_models:
            ...
            assert model_info.provider == "gemini"
            assert isinstance(model_info.description, str) and len(model_info.description) > 0
            assert isinstance(model_info.max_tokens, int) and model_info.max_tokens > 0
            assert isinstance(model_info.cost_per_token, float) and model_info.cost_per_token >= 0
            ...
            ToolDefinition(
                ...
                parameters={
                    ...
                    "properties": {
                        ...
                        "format": {"type": "string", "enum": ["decimal", "scientific"], "default": "decimal"}
                    },
                    "required": ["expression"]
                },
                server_name="math_server"
