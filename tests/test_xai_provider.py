...
class TestXAIProviderInitialization:
    ...
    def test_initialization_with_system_prompt(self, xai_provider_creative):
        ...
        assert xai_provider_creative.system_prompt == "You are a creative writing assistant."
...
class TestXAIProviderMessageSending:
    ...
    @pytest.mark.asyncio
    async def test_send_message_success(self, xai_provider, sample_chat_context, mock_successful_response):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            response = await xai_provider.send_message("Tell me about AI", sample_chat_context)
            
            assert response.content == "Based on the latest information, AI has made significant strides in reasoning capabilities."
            assert response.model_used == "grok-beta"
            assert response.tokens_used == 50
            assert response.timestamp is not None
    
    @pytest.mark.asyncio
    async def test_send_message_with_web_search(self, xai_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_no_web_search(self, xai_provider_precise, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_system_prompt(self, xai_provider_creative, sample_chat_context):
        """Test message sending with system prompt."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "choices": [{"message": {"role": "assistant", "content": "Creative response"}}],
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            await xai_provider_creative.send_message("Write a story", sample_chat_context)
    ...
    @pytest.mark.asyncio
    async def test_send_message_authentication_error(self, xai_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_rate_limit_error(self, xai_provider, sample_chat_context):
    ...
    @pytest.mark.asyncio
    async def test_send_message_model_not_found(self, xai_provider, sample_chat_context):
...
class TestXAIProviderToolCalling:
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_tools_success(self, xai_provider, sample_chat_context, sample_tools, mock_tool_response):
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_tools_payload_format(self, xai_provider, sample_chat_context, sample_tools):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            await xai_provider.send_message_with_tools("Test message", sample_chat_context, sample_tools)
            ...
            assert tools[0]['function']['description'] == 'Search the web for current information'
    ...
    async def test_convert_tools_to_xai_format(self, xai_provider, sample_tools):
        ...
        assert tool1['function']['description'] == 'Search the web for current information'
...
class TestXAIProviderModelInfo:
    ...
    def test_get_model_info_grok_beta(self, xai_provider):
        ...
        assert model_info.description == "Grok Beta - Advanced reasoning with real-time information"
    ...
    def test_get_model_info_grok_vision(self):
        ...
        assert model_info.description == "Grok Vision Beta - Multimodal model with image understanding"
...
class TestXAIProviderTemperatureSettings:
    ...
    def test_get_temperature_for_mode_unknown(self):
        ...
        provider = XAIProvider(api_key="test", model="grok-beta", grok_mode="unknown", temperature=0.5)
...
class TestXAIProviderMessagePreparation:
    ...
    def test_prepare_messages_with_context(self, xai_provider, sample_chat_context):
        ...
        assert messages[1]["content"] == "Recent AI developments include advances in reasoning and multimodal capabilities."
        
        # Check new message
        assert messages[2]["role"] == "user"
        assert messages[2]["content"] == "New message"
    
    def test_prepare_messages_with_system_prompt(self, xai_provider_creative, sample_chat_context):
        """Test preparing messages with system prompt."""
        messages = xai_provider_creative._prepare_messages("Write a story", sample_chat_context)
...
class TestXAIProviderResponseHandling:
    ...
    def test_handle_response_with_tools_success(self, xai_provider, mock_tool_response):
        ...
        assert len(response.tool_calls) == 1
        assert response.tool_calls[0].name == "web_search"
        assert '"query": "latest AI developments 2024"' in str(response.tool_calls[0].arguments)
    ...
    def test_handle_response_with_error_json(self, xai_provider):
        ...
        with pytest.raises(ProviderError, match="xAI API error: Invalid request format"):
